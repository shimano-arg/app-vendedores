...
| **Versión actual** | **SW v492 en prod.** Sistema completo Revisión + Espera + Sidebar-left (v451→v491, 2026-08-11 → 2026-08-12). Ver sección 47 para el detalle completo. Hitos: (v451) sección **Revisión Excel** en header para simular pedido cliente vs stock SAP; (v454) colección Firestore `revision_waitlist` compartida + modal card individual con recálculo dinámico FIFO; (v462) columna **Disponible app** que corrige delay 5-30min del sync SAP → BQ → Firestore; (v472) 4 subtotales desglosados (Con Stock/Sin Stock)×(Inmediato/Pendiente); (v473) botón "Volver a lista de espera" en modal pedido pendiente; (v474) view BQ `v_backorder_lineas` excluye SQs duplicadas con NC descendiente; (v475→v480) 2 recuadros SAP Asignado del cliente (YA en pedido / NO pedidos) + botones "+ Agregar"; (v481→v487) sidebar-left desktop con Facturación Diaria + Cumplimiento del mes + 9 botones proxy (Campañas/Dashboard/Seguimiento/Exportar/Backorder/Stock Asig./Forecast/Revisión/Espera) con misma paleta que sidebar-right; (v482) sync BQ `v_ventas_lineas` → Firestore `facturacion_snapshot` para datos oficiales SAP (misma fuente que PowerBI); (v490) menú cliente Doble-click → Crear/Cargar Excel redirigen a Revisión con cliente pre-seleccionado (para whitelist de 4). Whitelist Revisión: Mariano/Ioannis/Santi/Pablo. |
