...
| **Versión actual** | SW v444 (en rama `dev`; **v443 en prod**). v444: botón "📦 Envío gratis" en cada card de cliente, **solo visible para Ioannis** (gate CSS `body.is-ioannis` seteado en `applyRolePermissions` según email). Default azul marino oscuro; al click se pone verde y persiste en `clientMeta[key].envioGratis` (userData del user). Toggle a azul si se vuelve a tocar. Aparece arriba del "Ver en mapa". |
