...
| **Versión actual** | SW v431 (en rama `dev`; **v430 en prod**). v431 reorg mobile UX: revertido botón Herramientas (los 3 items vuelven a ser botones individuales estilo Dashboard), agregado botón Clientes, agregado botón Rendiciones al toolbar, fix picker cliente que mostraba "Sin resultados" (POINTS bare vs window.POINTS), y unificado el menú hamburguesa: `#header-hamburger-btn` ahora abre directo el drawer nuevo con TODOS los items (Rutas/Alta clientes/Notificaciones/Productos/Targets/Campañas admin/SAP/Master Clientes/Usuarios/Salir con role gating). Eliminado el dropdown `.user-info` viejo del flujo mobile. |
