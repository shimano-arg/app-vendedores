...
41. [Changelog v300 → v792](#41-changelog-v300--v792)
...
## 41) Changelog v300 → v792

### v792 (2026-09-04)

**Fix Pesca v3: 2 columnas separadas por currency + telemetría Bike (COWORK feedback sobre v790).**

- **Root cause v790**: escribí valores USD en una columna llamada `cost_avg_ars`. COWORK marcó que el nombre queda mintiendo para toda una división. Corrección: **NO usar una sola columna agnóstica de moneda — separar en 2 columnas con nombres siempre veraces**.
- **Diseño correcto** (según COWORK): la lista 11 se lee 2 veces (una por moneda), cada valor va a una columna distinta con nombre veraz. El tipo de cambio lo aplica Power BI del lado modelo con `doc_rate` (mismo patrón que ya usa con `target_usd`).
- **Cambio en `flatten_item()` Pesca** (`sync_sap_to_bigquery.py`):
  - `cost_avg_ars` vuelve a filtrar `expected_currency='ARS'` estricto (~2 items en Pesca, semántica intacta).
...
- **Fact aparte anotado por COWORK**: el TC implícito de la lista 11 (ARS) es 1.365 vs `doc_rate` de facturas en 1.482 (sep 2026). El inventario Bike valuado al costo ARS está subvaluado ~8.8% — dato para llevar a Compras, no fix del pipeline.
- Bump `APP_VERSION` + `CACHE_VERSION` v791 → v792. Bundle sin cambios.
