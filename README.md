...
| **Versión actual** | SW v419 (en `main`; **v414 en prod** por outage GH — ver Sprint) |
...
- **En `main` local + remoto**: **v419** (v415, v416, v417, v418, v419 esperando deploy).
...
**Sesión 2026-08-06 tarde (v413 → v419 + fix pipeline GH Pages):**
...
*Backorders — foco en urgentes (v419):*
- [X] **v419** — Filtro "Solo sin stock" en toolbar del modal Backorders. Default ON al abrir (matchea pedido: "quiero ver siempre los SKUs con demanda que no tenemos"). Filtra usando `getStockDisponibleVenta(sku)` (whs 11) + `getStockTransito(sku)` (whs 12) — si ambos son 0, la línea aparece; si hay stock en cualquiera de los dos, se oculta. User destilda para ver el listado completo. Preserva la elección entre aperturas del modal (variable global `backordersSinStockFilter`). Sincroniza el checkbox al reabrir. Semántica coherente con las cards naranjas ya pintadas por la lógica de stock existente (v403).
