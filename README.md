...
| **Versión actual** | SW v427 (en rama `dev`; **v426 en prod**). v427 fix desktop UX: el toolbar principal (Campañas Activas, Dashboard, Exportar, Seguimiento, Backorder Pendiente, Backorder Disponible, Forecast) generaba scroll horizontal en desktop y ocultaba Forecast + Backorder Disponible; ahora los botones caen a 2da fila via `flex-wrap:wrap`. v417→v426 desplegados en el día 2026-08-07 (post-outage GH Actions resuelto ~12 UTC). Sesión maratónica: 10 releases consecutivos con fixes UX + nueva sección FORECAST + typos display. |
