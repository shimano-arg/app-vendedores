...
41. [Changelog v204 → v379](#41-changelog-v204--v379)
...
**TODO pendiente (requiere OK explícito del user)**:

- Activar **branch protection en `main`** para hacer los checks del workflow obligatorios (`gh api -X PUT repos/shimano-arg/app-vendedores/branches/main/protection --input <spec>`). Sin esto, el workflow corre y muestra ✅/❌ en el PR pero el merge no está bloqueado si falla. Se decidió no hacerlo automático porque puede bloquear el flow actual si algo del workflow falla por temas de setup (ej: primera corrida con cache miss).
