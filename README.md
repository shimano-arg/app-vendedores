...
| **Versión actual** | SW v448 (en rama `dev`; **v447 en prod**). v448: fix typo en email de Ioannis usado como gate del botón "Envío gratis" (v444) y como mapping SlpCode 52 → email en `sync_sap_to_firestore.py`. El email correcto es `ioannis.plakoudakis@shimano.com.ar` (con "l" antes de "k"), no `palkoudakis`. 4 lugares corregidos: 2 en index.html (handler + CSS gate), 1 en scripts/sync_sap_to_firestore.py (SLP_TO_VENDOR[52]), 1 en README doc. **Impacto colateral**: las altas SAP creadas por el sync desde 2026-07-08 en adelante para SlpCode 52 quedaron con `ownerUid` sin resolver (email mapeado no existía) → Ioannis no las veía en su lista personal. El próximo tick del sync las backfillea con el email correcto. |
