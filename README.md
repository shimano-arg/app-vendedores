...
| **Versión actual** | SW v447 (en rama `dev`; **v446 en prod**). v447: Master Clientes — el dropdown de vendedor ahora es editable también para **clientes SAP importados del masterfile** (POINTS `tipo='cliente'` que pasan `isSapConfirmed`), no solo altas app (`tipo='sap_alta'`). Antes v446 solo cubría altas app registradas en SAP; los importados masterfile (~600) seguían texto plano. Nuevo handler `saveMcClienteSapVendor` escribe a `vendor_overrides/{SHOP\|prov\|loc\|nombre}` (misma colección que Panel Zonas scope='shop'), sin log de auditoría propio. Refresh automático: el listener global de `vendor_overrides` re-dispara `renderMasterClientesTable` si el modal está abierto. `getEffectiveVendorForClient` aplicado en `getAllStoreEntries` para que el dropdown pre-seleccione siempre el override activo. |
...
- **Gate por email** (`applyRolePermissions`): `document.body.classList.toggle('is-ioannis', email === 'ioannis.palkoudakis@shimano.com.ar')`. CSS `.cli-envio-btn{display:none}` + `body.is-ioannis .cli-envio-btn{display:inline-flex}` → el resto del equipo no ve el botón.
