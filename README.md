...
| **APP_VERSION** | `v1008` frontend (fix upload rendiciones + Planner en header + vistas BO/ASIG BQ). Sincronizada con `sw.js` CACHE_VERSION. Ver §41 Changelog para historial completo. |
...
**Reporte 2026-09-21 09:31 ART**: pedido MARCELO DANIEL BOSCHETTO falló con `callable(functions/unauthenticated): Unauthenticated`. Diagnóstico completo:

- Error del frontend (auth token expirado, retry manual OK con F5)
- Trigger auto server-side `onPedidoConfirmedSendToSap` **hubiera sido el fallback** cuando el client-side falla, pero estaba **roto desde su creación** por un typo.

**Bug** (`functions/index.js:446`): la 7ma lectura del sapConfig usaba `userName: sl.userName || ''` (camelCase-only), pero el doc real de Firestore `app_config/sap_integration.serviceLayer` tiene el field como `username` (lowercase). Las otras 6 lecturas del archivo (líneas 150, 209, 833, 875, 961) sí usan el pattern correcto `sl.username || sl.userName`. Este único caso quedó sin fallback → `sapConfig.userName === ''` → check de línea 449 falla → skip → **NUNCA envió pedidos automáticamente**.

**Impacto histórico**: el trigger no auto-envió NINGÚN pedido desde su deploy inicial. Los pedidos igual llegaban a SAP porque el flow client-side `sap-auto-send-listener.js` cubría cuando algún admin/gerente tenía la app abierta. Si nadie tenía la app abierta post-confirmación, el pedido quedaba con `transferidoSAP: null` esperando que alguien lo confirmara manual. Los logs de Firebase muestran spam constante de `onPedidoConfirmedSendToSap skip: sapConfig incompleto` que hasta hoy no fue investigado.

**Fix** (1 línea): `userName: sl.userName || ''` → `userName: sl.username || sl.userName || ''`.

Verificación post-deploy: buscar `onPedidoConfirmedSendToSap OK` en logs Firebase después de que un VDE confirme un pedido → confirmar SQ real en SAP con `transferidoSAP.via='cf_auto'` (no `service_layer_auto` que es el client-side).

Bundle rebuildeado + bump `APP_VERSION`/`CACHE_VERSION` v996 → v999 para que el push del hotfix trigger CF también invalide el shell cacheado (aunque el frontend NO cambia, mantener versionado coherente).

**Nota adicional descubierta durante el diagnóstico**: el hotfix v996 original (revert `enforceAppCheck: true → false` en las 3 CFs sap*) **fue perdido en los merges de v996-MELI (#665) + v997-PowerBI (#666) + v998-Node22 (#667)**. Las branches fueron creadas ANTES del hotfix y no rebasearon. Blame confirma que `functions/index.js:114,630,715` volvieron al estado v990 (`enforceAppCheck: true`). NO importa en runtime porque el SDK Gen 2 degrada silent (`NEEDS-VALIDATION.md §1`), pero conviene re-plantear si vale mantener el flag `true` de nuevo cuando se retome el rollout gradual de App Check.
...
Descubierto durante el pre-deploy review del v994: el modal Depósito de la app (`index.html:_fetchDeposito`) invoca `setupGetMovimientos({ dias })` **sin `cardCode`** (v836 flow "traeme todos los shipments recientes"). El v994 original devolvía `invalid-argument` en ese caso → rompía completamente el modal para VDEs.

**Fix v995**:
- Si `role === 'vendedor' && filterCardCode == null` → NO throw. En su lugar marca `_scopeAllToVendorCartera = true` y sigue el flow normal.
- Al final del handler (justo antes del `return { movimientos }`), si el flag está activo:
  1. Extraer set de `destinatario` (= cardCode) únicos del array `movimientos` que SETUP devolvió.
  2. Chunked query a `client_master` con `where('__name__', 'in', chunk)` (batch de 30, límite Firestore para `in`).
  3. Construir `Map<cardCode, assignedVendor>`.
  4. `_finalMovimientos = movimientos.filter(m => vendorByCardCode.get(m.destinatario) === callerVendorNorm)`.
  5. Log `vendor scope filter N → M notas` para observability.
- Vendedor con `cardCode` explicito sigue con el check estricto v994 (verifica `client_master/{cardCode}.assignedVendor` == callerVendor).
- Admin/gerente/interno sin cambio.

**Trade-off perf**: para dias=60 típicos (~50-200 cardCodes únicos) son 2-7 chunk queries (10-30ms cada una). Aceptable dentro del timeout 300s del CF. Alternativa evaluada y descartada: pre-cachear el vendor's cartera al inicio (pero cartera es dinámica y client_master crece, cache serviría poco).

**Vector de finding #3 cerrado igual**: vendedor sigue sin poder ver shipments de clientes ajenos, sea con o sin cardCode. El fix v994+v995 es la implementación robusta y compatible con la UX existente.

Bump `APP_VERSION` + `CACHE_VERSION` → v995. Bundle rebuildeado.
...
Reporte VDEs 2026-09-17: no pueden cargar rendiciones porque el OCR (Gemini) falla con "Sesion expirada o sin permisos. Cerra y volve a entrar a la app."

**Root cause**: mensaje engañoso. La causa real casi nunca es "sesión expirada" — es el **throttle 24h de reCAPTCHA v3 App Check** (ver `memory/reference_appcheck_throttle_24h.md`). El SDK Firebase App Check se auto-throttlea cuando recibe repetidos 403 y bloquea todos los requests por-browser por 24hs. El throttle está persistido en IndexedDB → cerrar/reabrir la app NO lo resuelve, hay que borrar datos del sitio.

Además el mensaje se cortaba a 120 chars en la UI, así que aunque el mensaje tuviera instrucciones detalladas, se perdían.

**Fix `src/domains/rendiciones.js`**:

1. **Detección específica del throttle**: si el error match `throttl` o `appCheck`, título cambia a "La verificacion de seguridad esta bloqueada por 24hs (sucede raro)". Sino, título estándar "Sesion expirada o problema de permisos."

2. **Instrucciones completas** en el mensaje (5 pasos numerados que el VDE puede seguir solo):
```
...
3. **Render seguro sin `innerHTML`**: reconstruido el bloque de error con `createElement` + `textContent` para preservar los saltos de línea y evitar XSS. Detección `isLongDetailed = rawMsg.length > 120 || rawMsg.includes('\n')` decide si usar el layout con `<pre>` (instrucciones completas) o el layout corto original.

4. **Log detallado a consola** (`console.warn`) con code + msg + err para diagnostic post-mortem.

**Bump**: APP_VERSION + CACHE_VERSION → `v961`. Deploy: GH Pages auto (frontend-only, sin CF).

**Follow-up**: los VDEs afectados ahora tienen instrucciones para auto-fix. Si el problema sigue reportándose recurrente, considerar (a) detectar el throttle antes de la llamada callable y auto-limpiar IndexedDB, o (b) sacar `enforceAppCheck` del CF `geminiOcrProxy` (revirtiendo SecAudit E1.2 pero eliminando la superficie de bloqueo).
...
Pedido Mariano 2026-09-16: la reserva de stock en líneas ASIG solo se respeta por 15 días. Después la línea sigue siendo ASIG pero deja de bloquear stock (independiente del cliTipo). Además la columna "UNIDADES RESERVADAS" del modal Pedido en Espera debe contar solo unidades que realmente reservan (ejemplo: si de 4u hay 3 con reserva, mostrar 3, no 4).

**Cambios en `src/pure/stock-realmente-disponible.js`**:

`lineReservesStock(line, nowMs)` extendida con expiración:
- Antes: solo miraba `asigReserva`
- Ahora: chequea `asigReserva !== false` **Y** `asigAt <= 15 días atrás`
- Independiente del cliTipo — un P/A también expira

Nuevo parámetro opcional `opts.now` en las 3 funciones exportadas (`getStockRealmenteDisponible`, `getStockPorCliente`, `getStockDesglose`) para inyectar timestamp en tests. Default `Date.now()`.

`lineReservesStock` **exportada** (era interna) — el bundle la expone via `window.__phase0.pure.lineReservesStock` para que el UI la use.

**Fix columna "UNIDADES RESERVADAS"** (`index.html:18290`):

Antes: `desg.breakdown.ASIG` — todas las líneas ASIG.
Ahora: automático — como `getStockDesglose` ya usa `lineReservesStock`, `breakdown.ASIG` solo cuenta ASIG con reserva vigente. **No requiere cambio adicional** en el sitio de la columna.

**Actualización badge modal detalle pedido** (`viewPedido`):
- `isAsigSinReserva` ahora usa `lineReservesStock` en vez de solo `asigReserva === false`
- Tooltip explica el motivo: "Cliente B/C" O "Reserva expirada — pasaron N días desde asigAt (limite 15)"

**Tests**: 403/403 verde en suite completa. 7 tests nuevos específicos v959:
- ASIG con reserva reciente (<15d) sigue reservando
- ASIG con reserva expirada (>15d) deja de reservar
- Límite exacto (15d): sigue reservando
- Expiración aplica a P/A también (no distingue tipo)
- ASIG sin `asigAt` (legacy) sigue reservando por default
- Mix confirmed + ASIG-reciente + ASIG-expirada + ASIG-sin-reserva
- Ejemplo Mariano: 4 unidades → si 1 expirada, `breakdown.ASIG=3`

**Bump**: APP_VERSION + CACHE_VERSION → `v959`. Deploy: GH Pages auto.
...
Pedido Mariano 2026-09-16: reservar stock solo para clientes A y P. B/C aparecen en Stock Asignado pero sin retener stock físicamente. Motivación: que un cliente C con baja frecuencia de compra no bloquee stock que un VDE podría vender a un A/P que sí compra rápido.

**Fase 1 (este PR) — solo CF FIFO**:

`functions/core/fifo-assign-core.js`:
- Nuevo helper `computeClientLocId(prov, locName, tienda)` — portado de `app.bundle.js:7919` (fn `clientLocId2`). Mismo algoritmo que usa la UI para keyar `client_master`.
- Nuevo helper `fetchCliTipo(deps, docId)` — lee `client_master/{docId}.cliTipo`, default `'C'` si no existe o inválido.
- `loadBoCandidatesForSku` ahora resuelve `cliTipo` para cada candidato via `client_master` (cache dentro del batch para no duplicar reads).
- **`computeAssignmentsFifo` reescrito con lógica tier-based**:
  - Agrupa candidatos por tier (P > A > B > C)
  - Procesa tier por tier. Dentro de cada tier, FIFO estricto por `createdAt`
  - Si el primer candidato del tier no cabe, ese tier se corta pero el siguiente tier SÍ procesa con el remaining
  - Setea `asigReserva: true` para P/A, `asigReserva: false` para B/C
- `applyAssignments` escribe 2 fields nuevos en la línea promovida: `asigReserva: bool` + `asigCliTipo: 'P'|'A'|'B'|'C'`.

**Tests**: 32/32 verde. 9 tests nuevos que cubren:
- P/A tienen prioridad sobre B/C aunque C pidió primero
- P y A quedan con `asigReserva=true`, B y C con `asigReserva=false`
- Si el primer P no cabe, tier P se corta pero A puede tomar el stock
- Strict FIFO within tier (un P grande que no cabe bloquea otros P)
- Default `cliTipo='C'` si no hay campo (retrocompat)
- `loadBoCandidatesForSku` mira `client_master` correctamente
- `runFifoAssign` escribe `asigReserva` + `asigCliTipo` en active mode

**Deploy**: `firebase deploy --only functions:onStockChangeFIFOAssign` post-merge.

**Fases restantes** (PRs separados según fue acordado con Mariano):
- Fase 2: `getStockRealmenteDisponible` + `getStockPorCliente*` skipean ASIG con `asigReserva=false`
- Fase 3: UI (Modal Stock Asig con badge CON/SIN RESERVA, Modal detalle pedido)
- Fase 4: Backfill script para las líneas ASIG existentes en `pedidos` collection

Hasta que Fase 2 llegue: las líneas B/C promovidas por esta CF quedarán con `asigReserva=false` pero el cálculo de stock disponible **todavía** las resta. Es un no-op efectivo hasta Fase 2.

**Bump**: APP_VERSION + CACHE_VERSION → `v956`.

### v955 (2026-09-16) — Modal pedido: subtotal disponible + backorder debajo del TOTAL NETO

Pedido Mariano 2026-09-16: en el modal detalle de un pedido confirmado, agregar debajo del TOTAL NETO un breakdown que muestre cuánto del pedido es "disponible" (líneas con stock) vs "backorder" (líneas `state=BO` sin stock). Facilita ver de un vistazo qué parte del pedido va efectivamente a SAP hoy y qué parte queda esperando.

**Cambios en `viewPedido` (`index.html:20724`+)**:

En el forEach de líneas, además de `subtotalCalc` (total gross), acumular:
```js
const _lineIsBo = l.state === 'BO' || (!l.state && !!l.sinStock);
if (_lineIsBo) subtotalBackorderCalc += q * pr;
else subtotalDispCalc += q * pr;
```

Legacy sinStock (pre-v543 sin schema `state`) se cuenta como BO por defecto.

Debajo del TOTAL NETO (`index.html:20898`), si hay backorder, se renderizan 2 rows con formato indent + color:
- **Subtotal disponible (con stock)** — monto en verde
- **Subtotal backorder (sin stock)** — monto en rojo

Solo se muestran cuando `subtotalBackorderCalc > 0` para no meter ruido en pedidos 100% con stock.

**Bump**: APP_VERSION + CACHE_VERSION → `v955`. Deploy: GH Pages auto.
...
**Reporte Mariano 2026-09-16 con screenshots**: VDE pasó pedido de "Lista de Espera" a "Pendientes" y lo confirmó. La card **siguió apareciendo** en Lista de Espera Y en Confirmados aparecen **3 pedidos duplicados** con el mismo `ORDEN 178` (CRISTIAN JOSE SANTORO, DocNums SAP `2000183`, `2000184`, `2000185`, montos $2.284k / $2.464k / $2.644k).

**Root cause combinado**:

1. **`firestore.rules:623` (v926 SecAudit)** — `delete` de `revision_waitlist` restringido a admin/gerente/interno. Un VDE NO puede borrar sus propios docs. Cuando `doConfirmPedido.then()` disparaba el delete post-add, fallaba con `permission-denied`.
2. **Sidebar no ocultaba la card** — el listener de `revision_waitlist` renderizaba todo lo que hay en Firestore. Si el delete fallaba, la card seguía visible.
3. **`_pendingPedidoOrderNumber` se heredaba de la waitlist card** — al re-abrir la card y volver a "Pasar a Pendientes", el flow re-usaba el mismo `orderNumber` (línea 19551) — las 3 duplicaciones quedaron con ORDEN 178.

**Fix triple**:

1. **`firestore.rules:623`** — permitir owner delete:
```
allow delete: if isAdminOrGerente() || isInterno()
              || (isReader() && resource.data.ownerUid == request.auth.uid);
```
Defensa contra wipe masivo se preserva: un attacker solo puede borrar sus propios docs, no los de otros.

2. **`doConfirmPedido.then()` (`index.html:27156`)** — antes del delete, `update` marca el doc como `stage: 'consumed'` + `consumedByPedidoId: ref.id`. Update funciona para cualquier reader (no requiere delete permission). Aunque el delete falle después, el update marca el doc como consumido.

3. **Listener de `revision_waitlist` (`index.html:15008`)** — filtra `stage === 'consumed'` en el snapshot. Los docs consumidos desaparecen del sidebar aunque el delete no haya corrido.

**Cobertura**: incluso si un rules deploy se retrasa o hay racing, el update (fix 2) + filter (fix 3) esconden la card del sidebar y previenen el re-submit.

**Backfill manual**: los 3 pedidos duplicados en SAP (`2000183/4/5`) siguen abiertos como SQ. Coordinar con Santi para cerrar 2 de 3 en SAP manualmente. La card de waitlist (`ORDEN 178`) también sigue viva — hay que borrarla manual desde la app (ahora que rules lo permiten) o desde Firestore console.

**Bump**: APP_VERSION + CACHE_VERSION → `v953`. Deploy: GH Pages auto + `firebase deploy --only firestore:rules`.

### v952 (2026-09-16) — HOTFIX: `orderNumber` en flows Crear Pedido → lista de espera + Pendientes → lista de espera

Reporte Mariano 2026-09-16: "cuando toco CREAR PEDIDO y pasa a la lista de espera no le pone numero de orden".

**Root cause**: v942 arregló `doConfirmPedido` (flow Crear Pedido → Confirmado). Pero hay OTROS 2 flows que también terminan en waitlist y quedaron sin reservar orderNumber:

1. **`enviarBorradorAListaEspera`** (`index.html:27465`) — "flow Crear Pedido manual" enviando borrador directo a lista de espera SIN pasar por Pendientes. Este es el que reportó Mariano.
2. **`volverAListaEspera`** (`index.html:27369`) — Pendientes → lista de espera. Perdía el orderNumber del pedido pending al recrearlo como waitlist doc.

**Fix ambos**:

En `enviarBorradorAListaEspera`: reserva orderNumber via `reserveNextOrderNumber()` post-`confirm()` gate (mismo pattern que `reviewDejarEnEspera` v913). Añade `orderNumber: ordenNum` al payload. Toast actualizado a `"Borrador movido a lista de espera (ORDEN N)"`.

En `volverAListaEspera`: preserva `entry.orderNumber` del pedido pending. Si el entry es legacy sin orderNumber (pre-v913/v942), reserva uno nuevo. Añade al payload.

Con esto los **3 paths que crean docs en `revision_waitlist`** garantizan que el doc tenga `orderNumber`:
1. `reviewDejarEnEspera` (Revisión Excel + Crear Pedido con Review modal) — ya lo hacía desde v913
2. `enviarBorradorAListaEspera` (Crear Pedido directo → lista de espera) — v952 ✅
3. `volverAListaEspera` (Pendientes → lista de espera) — v952 ✅

**Bump**: APP_VERSION + CACHE_VERSION → `v952`. Deploy: GH Pages auto.

### v951 (2026-09-16) — Export mode-aware: distinguir "Stock Asignado" vs "Backorders"

Reporte Mariano 2026-09-16 con screenshot: al clickear "Exportar todo" desde el modal **Stock Asignado** (BO ya cubierto), el archivo descargado se llamaba `Backorders_YYYY-MM-DD.xlsx` con hojas "Backorders" + "Forecast" — inconsistente con el modo. El contenido estaba bien (filtrado a items con stock), pero el naming confundía.

**Fix**: `exportBackordersToExcel` (`index.html:12550+`) usa `isAsigMode` para elegir:

| Elemento | Modo Backorder | Modo Stock Asignado |
|---|---|---|
| Filename | `Backorders_YYYY-MM-DD.xlsx` | `Stock_Asignado_YYYY-MM-DD.xlsx` |
| Hoja principal | `Backorders` | `Stock Asignado` |
| Título row 1 hoja principal | `TOTAL DEL PEDIDO` | `TOTAL STOCK ASIGNADO` |
| Hoja resumen | `Forecast` | `Resumen por SKU` |
| Título row 1 hoja resumen | `TOTAL DEL FORECAST` | `TOTAL RESUMEN` |

Constantes calculadas al inicio del `try`: `WS_NAME_MAIN`, `WS_NAME_RESUMEN`, `TITLE_MAIN`, `TITLE_RESUMEN`, `FILE_PREFIX`. Toast también actualizado con el nombre correcto.

**Bump**: APP_VERSION + CACHE_VERSION → `v951`. Deploy: GH Pages auto.

### v950 (2026-09-16) — Backorders Excel: segunda hoja "Forecast" (1 línea por SKU agregada)

Pedido Mariano 2026-09-16: mantener la hoja "Backorders" actual (1 línea por SKU-cliente, útil para ver quién pidió qué) y sumar una hoja **"Forecast"** con **1 línea por SKU** agregando las cantidades e importes de todos los clientes. Facilita decidir cuántas unidades importar por SKU sin sumar mentalmente por cliente.

**Cambios en `exportBackordersToExcel` (`index.html:12622+`)**:

Antes de escribir el buffer, agregamos:

```js
const forecastMap = {};
filtered.forEach(ln => {
  const skuKey = String(ln.sku || '').toUpperCase();
  // ... acumula qty, importe. Precio: max entre líneas del mismo SKU.
});
const forecastRows = Object.keys(forecastMap)
  .sort((a, b) => forecastMap[b].qty - forecastMap[a].qty)  // DESC por cantidad
  .map(k => [...7 cols...]);
```

Hoja "Forecast" con 7 columnas:

| Col | Header |
|---|---|
| A | SKU |
| B | Producto |
| C | Unidades pendientes totales |
| D | Precio unitario |
| E | Importe pendiente total |
| F | Total vendidas hist. |
| G | Stock dep 11 |

Row 1: "TOTAL DEL FORECAST" merge A1:D1 + total $ en E1 + relleno verde en F1:G1. Row 2: headers. Row 3+: 1 fila por SKU ordenada por unidades pendientes DESC.

**Toast actualizado**: `"Exportado: N líneas + M SKUs (Forecast)"` — así el user ve rápido cuántos SKUs distintos hay.

**Precio unitario en Forecast**: si distintas líneas del mismo SKU tienen precios distintos (raro pero posible por descuentos por cliente), queda el mayor observado — proxy razonable del precio de lista.

Hoja "Backorders" **sin cambios** — sigue con 1 línea por SKU-cliente y las 8 columnas de v948.

**Bump**: APP_VERSION + CACHE_VERSION → `v950`. Deploy: GH Pages auto.

### v949 (2026-09-16) — HOTFIX: exponer `fbDb` como `window.fbDb` (col G export siempre 0)

Reporte Mariano 2026-09-16 con screenshot: la nueva columna "Total vendidas hist." del export Backorders (v948) sale toda en 0.

**Root cause**: `fbDb` está declarado como `const fbDb = firebase.firestore()` en scope inline (`index.html:21404`). Los `const` de top-level **no se attachan a `window`**. Mi código del export en v948 hacía `if (window.fbDb)` — siempre `false` → el fetch de `sku_ventas_snapshot` nunca se ejecutaba → `salesBySku = {}` → col G siempre 0.

**Bug latente adicional**: el bundle (`src/domains/forecast.js:118`, `src/domains/master-clientes.js`, etc) usa `window.fbDb` también — es probable que módulos lazy-loaded del bundle tuvieran el mismo problema silencioso.

**Fix**: `index.html:21404+` — agregar `window.fbDb = fbDb;` inmediatamente después de la declaración. Un one-liner que arregla el export de Backorders V948 + descubre y arregla el mismo bug latente en el bundle.

**Bump**: APP_VERSION + CACHE_VERSION → `v949`. Deploy: GH Pages auto (solo frontend, sin CF).
...
Pedido Mariano 2026-09-16: el Excel "Backorders_YYYY-MM-DD.xlsx" que se descarga desde el modal Backorder → botón "Exportar todo" trae 10 columnas; quiere reducir a 6.

**Cambios en `exportBackordersToExcel` (`index.html:12530`+)**: removidas columnas D (Ciudad), E (Vendedor), I (SQ Doc Num), J (SQ Fecha). Quedan:
...
Ajustes técnicos:
- `rows.push([...])` — solo 6 valores (era 10)
- `ws.columns` — 6 widths (era 10)
- Row 1 "TOTAL DEL PEDIDO": merge `A1:E1` para el label + `F1` con el total (antes era `A1:G1` + `H1`)
- Removido el merge `I1:J1` (columnas ya no existen)
- Loop de alignment: `c <= 6` (era 10)
- `numFmt` de Precio unitario → col 5 (era 7); Importe pendiente → col 6 (era 8)

Nombre del archivo y filtros preservados igual.

**Bump**: APP_VERSION + CACHE_VERSION → `v947`. Deploy: GH Pages auto (solo frontend).

### v946 (2026-09-16) — Fix lag al escribir en buscador Master Clientes (debounce 200ms)

Reporte Mariano 2026-09-16: buscar/escribir en el input de Master Clientes iba re lento — el texto tarda muchísimo en aparecer.

**Root cause**: `index.html:4324` — el `<input>` tenía `oninput="renderMasterClientesTable()"` que re-renderiza la tabla completa (800+ filas, concat string + `innerHTML` grande) en cada keystroke. Cada render bloquea el main thread 200-800ms según cantidad de filas visibles → el input se ve congelado hasta terminar. Con typers rápidos se acumulaban 5-10 renders overlapping.

**Fix**: nuevo wrapper `debouncedRenderMc()` en `src/domains/master-clientes.js:1934` con debounce de **200ms**. El input dispara `debouncedRenderMc()` — al terminar de tipear se ejecuta un solo render. UX percibida: instantánea.

Cambios:
- `src/domains/master-clientes.js:1934` — nueva función `debouncedRenderMc` (IIFE con closure del timer)
- `index.html:4324` — `oninput="renderMasterClientesTable()"` → `oninput="debouncedRenderMc()"`
- `app.bundle.js` regenerado

Los otros filtros (selects de vendor/provincia/localidad) mantienen `renderMasterClientesTable()` directo — cambian una vez por selección, no por keystroke, no necesitan debounce.

**Bump**: APP_VERSION + CACHE_VERSION → `v946`. Deploy: GH Pages auto.

### v945 (2026-09-16) — Fix botón "Eliminar" invisible en Master Clientes

Reporte Mariano 2026-09-16 con screenshot del panel Master Clientes → tab SAP (fila ON FIRE, LANUS): el botón "Eliminar" al lado del "Guardar" no aparece. Solo se veía Guardar.

**Root cause**: `src/domains/master-clientes.js` líneas ~1442 y ~2463 — los dos botones de delete se renderizan como `<button ...></button>` **sin texto**. El botón existe (funcional) pero no tiene contenido visible → 0 width → invisible para el usuario. Bug pre-existente que arrastraba desde v778+.

**Fix**: agregar el texto "Eliminar" dentro de ambos botones + `type="button"` para evitar submit accidental de forms enclosing. Ambos tabs afectados: tab SAP (client_master) y tab Provisorios (client_applications).

Cambios:
- `src/domains/master-clientes.js:1442` (tab Provisorios)
- `src/domains/master-clientes.js:2463` (tab SAP)
- `app.bundle.js` regenerado con `npm run build`

**Bump**: APP_VERSION + CACHE_VERSION → `v945`. Deploy: GH Pages auto.
...
**Reporte Mariano 2026-09-16**: pedidos cargados desde "Crear Pedido" (path directo sin pasar por Revisión Excel / waitlist) quedaban en Confirmados **sin número de orden**. La card mostraba el cliente pero sin el chip `ORDEN N`.

**Root cause**: `doConfirmPedido` (`index.html:26789`) leía `window._pendingPedidoOrderNumber || null`. Ese flag solo lo poblaba `_waitlistPasarAPendientesContinuar` (path Revisión Excel → Waitlist → Confirmar). Cuando el vendedor entraba por "Crear Pedido" directo, el flag quedaba en `null` y el pedido caía sin orderNumber.

**Fix**: `doConfirmPedido` ahora es `async` y reserva el orderNumber via `reserveNextOrderNumber()` (contador atómico Firestore `counters/orderNumber` — v913) si `_pendingPedidoOrderNumber` no fue seteado por el waitlist flow. La reserva ocurre **después del `confirm()` gate** para no gastar contador si el vendedor cancela el diálogo.

Compatible con el flow `waitlistGenerarBackorder` (`index.html:19404-19411`): el chequeo `_boSkipConfirmDialog` es síncrono (antes del primer `await`), así que el `finally` del caller que resetea el flag no crea race condition.

**Bump**: APP_VERSION + CACHE_VERSION → `v942`. Deploy: GH Pages auto (solo frontend, sin CF).

**Follow-up sugerido (no bloqueante)**: los pedidos ya existentes en Firestore sin `orderNumber` no se backfillean automáticamente. Si querés, corro un script que recorra `pedidos` con `orderNumber = null` y les asigne uno leyendo del contador (o dejandolos como legacy).

### v941 (2026-09-15) — Backorder modal: eliminada opción "Todos los backorders" del filtro urgencia

Reporte de Mariano 2026-09-15: en la sección Backorder aparecen SKUs marcados **PARCIAL — hay stock pero repartido** (headers verdes) junto con los urgentes. Expectativa del uso Mariano-importador: el modal Backorder debe mostrar **solo lo "para importar YA"** (SKUs con `dispSap = 0`).

**Root cause**: v864 (2026-09-11) cambió el default de `backordersUrgencyFilter` de `'urgente'` a `'todos'` para arreglar el bug de "cliente en BO invisible cuando el SKU tiene stock parcial". Ese fix se preservó (el filtrado per-cliente por `qtyBackorder > 0` sigue vigente), pero el default `'todos'` metía ruido visual con los parciales.

**Fix v941**:
- `index.html:4890` — eliminada la `<option value="todos">Todos los backorders</option>` del `<select>`. Quedan solo "Solo urgentes (sin stock)" (default) + "Solo parciales (stock repartido)".
- `index.html:12286` — `var backordersUrgencyFilter = 'urgente'` (era `'todos'`).
- `index.html:12326` — fallback del handler ahora es `'urgente'` (era `'todos'`).
- Comportamiento restaurado al pre-v864 para el user (default urgentes). Los parciales siguen siendo accesibles vía dropdown si se necesitan.
- El fix de v864 (mostrar clientes en BO real dentro de SKUs con stock parcial) se preserva completamente — solo cambió cuándo aparecen (ahora requiere seleccionar "Solo parciales" en el filtro).

**Bump**: APP_VERSION + CACHE_VERSION → `v941`. Deploy: sin CF (solo frontend), GH Pages auto-deploy al merge.

### v940 (2026-09-15) — HOTFIX: sapProxy rate limit 300 → 5000/hr

**Reporte de campo 2026-09-15 ~13:02 ART**: mariano.erbino y pablo.gonzalez recibieron `callable(functions/resource-exhausted): sapProxy rate limit alcanzado (300/hr)` al enviar pedidos vía `service_layer_auto`. Screenshots adjuntos en el reporte.

**Root cause**: el threshold del PR #604 (v939, MED-15) asumía "~50 sapProxy calls/día por VDE" pero no consideraba el factor de amplificación del flow `service_layer_auto`: cada pedido enviado a SAP dispara **SL Login + N calls por línea + SL Logout** vía sapProxy. Un VDE mandando 5-10 pedidos batch pega el techo de 300/hr en minutos.

**Fix**: `functions/core/rate-limit-core.js:105` — `RATE_LIMITS.sapProxy.threshold` de 300 → **5000/hr** (17×). Mantiene defensa contra abuse real (>1.4 req/seg sostenido) sin bloquear al uso legítimo. `geminiOcrProxy` queda igual en 100/hr (no fue reportado).

Test `tests/functions/rate-limit.test.js:167` actualizado al nuevo threshold.

**Follow-up sugerido (no bloqueante)**: si vuelve a pegar el techo, evaluar excluir cuentas `auto/*` del rate limit — son backend flows, no superficie brute-force.

**Bump**: APP_VERSION + CACHE_VERSION → `v940`. Deploy: `firebase deploy --only functions:sapProxy`.
...
**Pedido Mariano post-v914**: "que el fondo se pinte de rojo". La v914 usaba un fondo rojo suave (`#fee2e2`, casi rosa) con borde grueso — se veía tímido. Ahora fondo rojo pleno + texto blanco.

**Cambios CSS** en `.confirmed-card.cc-error`:
- `background: #dc2626` (rojo estándar Tailwind red-600).
- `color: #fff` con `.cc-name` blanco y `.cc-meta`/`.cc-summary` en `#fecaca` (rojo muy claro) para jerarquía.
- `.cc-month` (chip mes) invertido: fondo `#7f1d1d` (rojo muy oscuro) + texto blanco.
- Hover: `#b91c1c` (una posición más oscuro).
- Botón `¿POR QUÉ FALLÓ?`: fondo blanco + texto rojo oscuro (invertido vs v914 para destacar sobre el rojo fuerte).
- Chip categoría: fondo `#7f1d1d` + border blanco 1px.

**Bump**: APP_VERSION + CACHE_VERSION → `v915`. Solo CSS — no requiere rebuild del bundle.

### v914 (2026-09-14) — Card roja en Confirmados cuando el envío a SAP falla + botón "¿Por qué?"

**Pedido Mariano**: los vendedores no tenían feedback de qué pedidos habían fallado al pasar a SAP. El error (`HTTP 400 - 10001095 - Lead ... is inactive`) solo aparecía en el `alert()` del envío manual (visible únicamente a admin/gerente) y se perdía al cerrar. Screenshot mostró `GRAN PARANA S.R.L. — ORDEN 146` sin ningún indicio visual del problema.

**Fix**:

1. **Persistencia del error** — cuando el envío a SAP falla, `sap-admin-panel.js` (manual send, línea 945) y `sap-auto-send-listener.js` (auto send, línea 218) ahora escriben en el pedido doc:
   ```js
   transferError: {
     message, at: <iso>, via: 'service_layer' | 'service_layer_auto', attemptedBy
   }
   ```
   En cada envío exitoso (incluyendo el path `app_only` de pedidos 100% BO) el campo se limpia con `FieldValue.delete()` para evitar mostrar errores stale.

2. **Card roja + botón** (`index.html:20003+`) — cuando `transferError` está seteado y `transferidoSAP.docNum` no, la card en el tab Confirmados agrega la clase `.cc-error`:
   - `background:#fee2e2` + `border:2px solid #dc2626` + `border-left:6px solid #991b1b`
   - `cc-name` en rojo oscuro, `cc-summary` también.
   - Botón `⚠ ¿POR QUÉ FALLÓ?` (rojo, uppercase) + chip con la categoría (ej: `Cliente inactivo`).
   - `event.stopPropagation()` en el botón para no abrir el detalle del pedido.

3. **Clasificación del error** (`classifyTransferError`, `index.html:19907`) — regex sobre el mensaje SAP devuelve categoría + hint accionable. Cubre 8 casos frecuentes: cliente inactivo, sin CardCode, producto inválido, precio inválido, sesión SAP caída, HTTP 400/500/timeout. Fallback `Error SAP`.

4. **Modal detalle** (`openTransferErrorModal`, `index.html:19926`) — al clickear el botón, muestra alert con: cliente, tipo, hint, mensaje completo SAP, timestamp, vía y quién intentó el envío.

5. **Listeners actualizados** — `unsubPedidosOwn` (line 25941) y `unsubPedidosAll` (line 26021) ahora proyectan `transferError` desde el doc. Sin esto la card no detectaba el campo aunque estuviera en Firestore.

**Bump**: APP_VERSION + CACHE_VERSION → `v914`. Bundle regenerado (`app.bundle.js` con los cambios de `src/domains/sap-admin-panel.js` + `src/domains/sap-auto-send-listener.js`). Tests: 382/382 unit + 25/25 smoke verdes.

**Alcance**: el fix es defensivo — si el envío exitoso ocurre después del deploy y el pedido ya tenía `transferError` legado, se limpia en el update. Pedidos actualmente fallidos (ej `GRAN PARANA S.R.L.` ORDEN 146) van a mostrar la card roja apenas admin/gerente reintente el envío manual y falle de nuevo, o cuando el auto-send corra otra vez.
...
**Pedido Mariano**: en Master Clientes → Direcciones, cuando falta alguno de los 3 campos críticos (Localidad, Provincia, Vendedor) la fila debe pintarse de rojo para poder ubicarla rápido y completarla.

**Cambio**:
- `src/domains/master-clientes.js:2221` — extendida la lógica `rowStyleV` que ya se usaba para "DUPLICADO SAP" para también cubrir el caso "falta datos". Mismo fondo rojo (`--color-danger-bg`) + border-left grueso (`--color-danger`) + tooltip `title="Faltan datos: Localidad, Provincia, ..."` para saber sin hover cuál falta.
- Placeholders del import SAP `(sin localidad)` y `(sin provincia)` cuentan como faltantes — no engañan al detector.
- Prioridad visual: si un row es **duplicado SAP** el estilo del duplicado gana (más crítico). Si no hay dup y falta uno de los 3 campos, aplica el nuevo estilo.

**No cambia**: layout, filtros, orden, edición inline, guardar/deshacer. Todo el flujo previo intacto.

**Bump**: APP_VERSION + CACHE_VERSION → `v880`. Tests: 382/382 verdes.

### v869 (2026-09-11) — Cleanup masivo: quitar TODOS los emojis de la UI

**Pedido Mariano**: quitar TODOS los emojis de botones, secciones, modales — tanto mobile como desktop.

**Alcance**: 407 emojis removidos en 14 archivos:
- `index.html`: 106 unicode + 188 HTML entities.
- `src/domains/*.js` (13 archivos): 25 unicode + 88 HTML entities.

**Método**: script Python con regex sobre ranges unicode emoji (`\U0001F300-\U0001F5FF`, dingbats, misc symbols, arrows) + HTML entities decimales del rango emoji (`&#127xxx;`, `&#128xxx;`, `&#129xxx;`, `&#9xxx;`). Colapsa dobles espacios preservando indentación.

**Sin cambios de comportamiento**: solo se sustituye el emoji por string vacío. Labels textuales intactos ("🎪 Preliminar" → "Preliminar", "📦 EN PREPARACIÓN" → "EN PREPARACIÓN").

**Bump**: v868 → v869. Bundle rebuilt. 382/382 tests unit verdes. Typecheck OK.

**Post-cleanup**: si en el futuro se quiere agregar un emoji a un botón puntual, se hace individual — el patrón masivo queda descartado.

### v868 (2026-09-11) — 🎪 Pedido Preliminar (cotizador feria)

**Pedido Mariano**: sección temporal para la feria — prospects/clientes preguntan cuánto queda un pedido con distintos descuentos (categoría P/A/B/C + volumen + contado si es P o A).

**100% en memoria — no persiste, no toca stock, no crea pedido en SAP.**

Botón "🎪 Preliminar" en el header (todos los roles). Modal fullscreen con:
- Búsqueda SKU con autocomplete sobre PRODUCTS + precio visible.
- Tabla de líneas editable (qty +/− + input; precio editable inline).
- Panel descuentos: radio categoría (P/A/B/C con % visible) + toggle contado (bloqueado si no es P/A).
- Bloque colapsable "⚙️ Ajustar porcentajes / umbral" para overridear defaults en vivo.
- Resumen dark con breakdown + total en verde grande.
- Botón "📱 Compartir por WhatsApp" → `wa.me/?text=...`.
- "🗑 Vaciar" con confirmación. Cierre con warning si hay líneas.

**Defaults**: P=−15%, A=−10%, B=−5%, C=0%. Volumen ≥ $500.000 → −3%. Contado (P/A) → −5%. Cascada compuesta: bruto → −cat → −vol → −contado.

**Arquitectura**: pure fn `calcularCotizacion` en `src/pure/preliminar-discount.js` con 15 tests unit. Handlers `window.preliminar*` inline en `index.html`. Bundle export `window.__preliminarDiscount`.

**Bump**: v867 → v868. Bundle rebuilt (3.10 MB). 382/382 unit tests (+15 nuevos).

**Post-feria**: para remover, borrar botón + modal + bloque handlers + pure fn + tests. 1 commit chico.

### v867 (2026-09-11) — Fix SAP error 1470000315 "specify a UoM code"

**Bug reportado por Mariano**: envío a SAP falla con `HTTP 400 - 1470000315 - Cannot add or update document; specify a UoM code`. Error global del documento (no de un item específico). SAP dejó de asumir UoM default.

**Fix**: agregar `UoMEntry: 1` explícito en cada línea del SQ. Items Shimano AR = Manual Management + IUoMEntry=1 (Unidad).

Aplicado en:
- `src/domains/sap-service-layer.js:386` — flow client-side ("Enviar a SAP via Service Layer" del panel Admin).
- `functions/core/auto-send-sap-core.js:194` — CF trigger `onPedidoConfirmedSendToSap` (auto-send VDE).

CF deployada 14:26 UTC. Bump v866→v867. Bundle rebuilt. 367/367 tests verdes.

**Nota futura**: si algún item tuviera UoM Group distinto (rare), habría que consultar `/Items('SKU')?$select=IUoMEntry` en runtime + cachear.

### v866 (2026-09-11) — Modal "Nueva versión disponible" restyle apple-design

**Feedback Mariano**: el modal de alerta de versión nueva no tenía el estilo apple del resto de la app. Fix aplicando skill `.claude/skills/apple-design/SKILL.md`:

- **§1 Feedback**: botón responde en `pointerdown` con `scale(0.97)` + shadow reducida (100ms), no en click.
- **§7 Spatial consistency**: entrada spring desde `scale(0.94) + opacity 0` → `scale(1) + opacity 1`, cubic-bezier(0.16, 1, 0.3, 1), 350ms.
- **§12 Material**: backdrop con `backdrop-filter: blur(20px) saturate(1.6)` en vez de flat overlay.
- **§14 Reduced motion**: `matchMedia('(prefers-reduced-motion: reduce)')` respetado — sin springs, fade instantáneo.
- **§15 Typography**: title `letter-spacing: -0.022em` weight 700, body 14px weight 400 line-height 1.5.
- **Icono en container**: círculo gradient con inset shadow (chrome iOS-like) en vez de emoji flat.
- **Botón pill**: border-radius 14px, sin uppercase, weight 600, gradient background, shadow con capa interna.

Sin cambios de comportamiento (sigue bloqueante, mismo trigger cada 3 min).

**Bump**: v865 → v866. Solo cambio inline en `_showVersionAlert` (index.html:5296+). 367/367 tests verdes.
