...
| **Versión actual** | SW v445 (en rama `dev`; **v444 en prod**). v445: fix SlpCode en la Oferta SAP cuando un VDI carga un pedido. Antes (v405) se priorizaba `p.ownerVendor` (VDI si carga Santi/Ioannis) → SAP mostraba el pedido a nombre del VDI aunque el cliente fuera de un VDE. Ahora se prioriza `getVendorForKey(clientKey)` (vendor real del cliente); `p.ownerVendor` queda solo como fallback para clientes SAP huérfanos. |
