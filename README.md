...
| **Versión actual** | SW v440 (en rama `dev`; **v439 en prod**). v440: (1) mobile — el bottom-nav (v429+) tapaba el footer del modal Visitas/Contactos aunque el z-index del modal (10001) es mayor que el del nav (9998); se oculta el nav mientras haya un modal abierto vía `body:has(.modal-overlay.open) #mobile-bottom-nav{display:none}`. (2) Consulta de stock — la línea "STOCK LIBERADO (transito - backorder)" mostraba 0 cuando había más disponible que backorder pero sin tránsito; se reemplaza por "LIBRE PARA VENDER (disp + transito - backorder)" que refleja lo que realmente le sirve al vendedor. |
