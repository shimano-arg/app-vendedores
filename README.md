...
| **Versión actual** | **SW v759 en main (2026-08-31) — FIX bug filtro cliente en `_findAsigDelCliente()`. Mariano reporto que al pasar a Pendientes desde lista de espera de Quesada, el modal Stock Asignado del Cliente mostraba 2 SKUs (CVC66MH4SACO + CDC60HSAA) que NO eran de Quesada — aparecian para todos los clientes. Root cause: el filtro `if (cc && pCC && pCC !== cc) continue` NO excluia pedidos huerfanos sin clientCardCode → esos pedidos con state=BO y stock disponible se colaban en el resultado de CUALQUIER cliente. Fix: strict matching cardCode-first, fallback a clientName SOLO si ninguno tiene cardCode (pedidos legacy). Anterior: SW v758 en main (2026-08-31) — validacion importe>0 en solicitudes+gastos de rendiciones. Root cause: Power Automate flow "Cargar rendiciones aprobadas a SharePoint" fallaba con `OpenApiOperationParameterTypeConversionFailed` en el step "Create item 1" porque una solicitud tenia importe=0 (permitido por HTML5 required + type=number) y SharePoint column Importe es Number/double, no acepta "". Fix en index.html/bundle (submitRendSolicitud + submitRendGasto): validar `parseFloat(read()) > 0` antes del save y bloquear con alert claro si no. Root cause a nivel data anterior queda resuelto. Complementa fix defensive Mariano aplicaria en el flow (`if(empty(...), 0, ...)`). Anterior: SW v756 en main (2026-08-31) — ROOT CAUSE encontrado del bug de contraste titulos dark mode. `apple-design.css` tenia rules con `color: #1d1d1f !important` en `.sidebar h3`, `#pane-notif h3`, `#pane-clients h3#clients-title`, `#pane-pedidos h3`, `#pane-rendiciones h3`, `#pane-altacli h3`, `#pane-rutas h3`. Specificity de ID (1,0,1) ganaba sobre nuclear rule inline (0,1,1). Por eso los blanket rules v752-v754 no aplicaban a esos h3. Fix v756: dark mode overrides al final de apple-design.css con matching specificity + prefix `html[data-theme="dark"]`. Anterior: SW v755 en main (2026-08-31) — Vista Preliminar del flow "Crear pedido" ahora es READ-ONLY: solo boton "Volver a editar", sin "Pasar a Pendientes". Restaura la intencion original v725 que se estaba pisando por `updateReviewFooter()` en el bundle. Nuevo mode `crear-view-only` en openReviewDialog. Waitlist + Excel siguen con mode `crear` (con boton "Pasar a Pendientes") porque necesitan el submit para completar. Anterior: SW v754 en main (2026-08-31) — FIX PERSISTENTE contraste dark mode. Mariano reporto 4x que titulos siguen "oscuros" pese al blanket v752. Fix v754: nuclear rule pasa de #f1f5f9 a #ffffff (blanco puro sin ambiguedad), extiende a h1-h6, agrega capa high-specificity `html[data-theme="dark"] .sidebar/.tab-pane` que ataca inline styles con color:var(--text-primary), #1e3a8a, #1e40af, #0f172a. Sin dependencia de CSS vars ni cascade. Bumped APP_VERSION+CACHE_VERSION. Anterior: SW v753 en dev (2026-08-31) — mapa arranca con zoom 5 en vez de 4. Antes: mostraba toda Sudamerica muy chico (Argentina se veia como sliver). Ahora: Argentina full-viewport prominente (matches el screenshot que Mariano mando como referencia visual). Cambio en 2 lugares: L.map(...).setView([-38,-64], 5) en el init + map.setView([-38,-64], 5) en recenter() cuando filters=ALL. minZoom preservado en 4 por si user quiere retroceder. maxZoom sigue en 14. Anterior: — FIX contraste dark mode NUCLEAR. Mariano reporto que h3 de las tabs (Clientes/Pedidos/Rutas/Alta clientes/Rendiciones - todos dentro de .tab-pane) SEGUIAN sin contraste incluso post-v751. Los v749/v751 usaban CSS vars pero por algun edge case (SW cache stuck? inheritance chain rara?) no aplicaban visualmente. Solucion nuclear: regla [data-theme="dark"] h1,h2,h3 { color:#f1f5f9 !important } LITERAL - no depende de vars ni cascade, blanket completo. Excepciones para .modal-head h3 (mantiene white sobre gradient dark) y containers con bg colored solid. Zero riesgo funcional. Anterior: — FIX contraste dark mode: titulos h3 con color hardcoded #1e40af (indigo-700) invisibles. Mariano reporto (segundo screenshot) que el titulo "Alertas y tareas" seguia negro sobre negro pese al fix v749. Root cause: encontrados 2 h3 mas con color:#1e40af inline (uno en "Backorder asignado (app)" y otro en modal "En construccion"). Ademas el titulo "Alertas y tareas" TENIA el fix v749 (var --text-primary) pero el codemod no capturo los #1e40af de los otros h3. Fix triple: (1) global find+replace color:#1e40af -> color:var(--text-primary) en index.html, (2) regla defensiva blanket [data-theme="dark"] h3[style*="color:#1e40af/1e3a8a/1d4ed8"] + .right-panel h3 + .tab-pane h3 + #pane-notif h3 con !important, (3) bump v750 -> v751 fuerza cache invalidation del SW. Anterior: — Cloud Function tracking de transiciones ASIG (pedido Mariano: medir mes-a-mes cuantas unidades ASIG concretan vs se eliminan para evaluar ROI del flow backorder). Nueva CF onPedidoWriteTrackAsigTransitions (region southamerica-east1, memory 256MiB, timeout 30s, retry:false fire-and-forget) trigger onDocumentWritten sobre pedidos/{id}. Compara before/after de lines[] y detecta cambios de state que involucren ASIG (entrada o salida). Escribe records a nueva collection asig_transitions/{auto-id} con {pedidoId, lineIdx, sku, clientCardCode, clientName, vendor, province, locName, fromState, toState, qty, transitionedAt serverTimestamp, month YYYY-MM}. Logica pura en functions/core/asig-transitions-core.js (16 tests unitarios verdes - 165 tests suite completa). Wrapper Firebase minimo en functions/index.js. Firestore rule nueva: match /asig_transitions/{id} allow read admin/gerente, write false (solo CF Admin SDK). Deployada. CF deployada exitoso. Analytics future: BQ view v_asig_conversion_monthly (proxima iter) leyendo asig_transitions con GROUP BY month + toState. Dashboard tab "Conversion ASIG" (proxima iter). MVP scope: solo transiciones from-now-forward (no backfill del historico pre-v750). Anterior: — 2 fixes de dark mode reportados por Mariano (screenshot): (1) Titulo "Alertas y tareas" del right sidebar era invisible en dark porque tenia color:#1e3a8a hardcoded inline (indigo-800, fuera del top-40 del codemod v743). Fix: cambiado a color:var(--text-primary) - visible en ambos temas. (2) BUG del map tile: CartoDB Dark Matter (usado en v744 E3) ahora exige API key ("API KEY REQUIRED" watermark reportado por Mariano). Approach nuevo: siempre servimos OSM standard + CSS filter en [data-theme="dark"] .leaflet-tile con invert(1) hue-rotate(180deg) brightness(0.95) saturate(0.85). Zero dependencia externa nueva, zero API key, consistente con policy CartoDB post-cambio. Los overlays SVG de provincias no se ven afectados (solo .leaflet-tile). Zoom controls excluidos del filter para no doble-invertir. Version bumpeada v747 -> v749 skipping v748 porque v748 fue usado en bigquery/views.sql comment (rules deploy - no user-facing). Anterior: — FIX bug reportado por Santi (interno): "Error aplicando cambios ASIG: Missing or insufficient permissions" al pasar pedido de lista de espera a pendientes. Root cause: index.html:16685 flow ASIG resolve hace batch update sobre multiples pedidos de OTROS vendedores (para cancelar/reciclar sus lineas ASIG cross-vendor). La rule Firestore para pedidos update solo autorizaba internos donde eran ownerUid/createdByUid propios. Batch fallaba al llegar a pedidos ajenos. Fix: relajada rule pedidos update/delete a `isAdminOrGerente() || isInterno() || (isVendor() && ownerUid==self)`. Justificacion: los 2 internos (Santi/Ioannis) estan en whitelist confiable, ya pueden crear pedidos on-behalf-of vendedores, y necesitan poder resolver ASIG lines cross-vendor. VDEs siguen restringidos a sus propios. Rules deployadas a produccion via firebase deploy --only firestore:rules. Anterior: — PLAN_DARK_MODE E5 EJECUTADO: rollout announcement one-time modal. Nuevo modal #dark-mode-announcement-modal (head slate gradient, emoji luna, copy explicando el toggle + Probar ahora + Mas tarde). Trigger: showDarkModeAnnouncementIfEligible(userDataDoc) enganchado en userDataRef.onSnapshot (per-user branch) Y en el fbDb.collection(userData).onSnapshot (admin/viewer global branch). Checks pre-show: (1) localStorage shimano_darkmode_announced NO =1, (2) userData Firestore hasSeenDarkModeAnnouncement NO =true, (3) localStorage shimano_theme vacio (user no descubrio el toggle solo). Delay 600ms para no atropellar splash. Botones: "Probar ahora" toggeaDarkMode+cierra+marca; "Mas tarde" solo cierra+marca; X cierra+marca. Persist en Firestore + localStorage. Idempotente (cross-device: si vio en un device, no muestra en otro). PLAN_DARK_MODE.md marcado como COMPLETADO (E0-E5 done). Tiempo real: ~1 dia vs estimado 5-7 dias full-time. Anterior: — PLAN_DARK_MODE E4 EJECUTADO: WCAG AA audit + polish. Nuevo script scripts/dark_mode_contrast_audit.py programaticamente computa WCAG 2.1 AA contrast ratios entre 26 pares (bg, text) de los 25 CSS vars del sistema. Pre-fixes: light 16/26 pass, dark 19/26. Post-fixes: light 16/26 (mismo - 10 fails son intencionales: text-disabled + borders sutiles + brand-shimano-blue que es color de marca inmutable), dark 24/26 (2 fails son borders sutiles). Fixes aplicados al bloque [data-theme=dark]: text-muted era #94a3b8 -> #cbd5e1 (4.04 fail vs bg-elevated -> 6.97 pass), border-default era #334155 = igual a bg-elevated invisible -> #475569 (visible), border-strong #475569 -> #64748b (mas contraste), color-success #22c55e -> #4ade80 (5.99 pass sobre success-bg), color-danger #f87171 -> #fca5a5 (5.99 pass sobre danger-bg), color-danger-strong #dc2626 -> #fca5a5 (era 2.07 fail). Tambien: re-agregados --color-success-bg y --color-danger-bg que el codemod v743 removio accidentalmente. Polish extra: toggle button icon rotate anim (hover:20deg + active:-15deg scale.9), 300ms cubic-bezier, respeta prefers-reduced-motion. Anterior: — PLAN_DARK_MODE E3 EJECUTADO: assets + edge cases. (1) Leaflet map tiles con swap dinamico: OSM standard en light, CartoDB Dark Matter en dark (free tier, CSP ya autorizaba *.cartocdn.com). window.applyThemeTilesSwap() invocada en init + en cada toggleDarkMode + en applyThemeFromUserData (cross-device). (2) Custom scrollbars webkit + firefox con paleta dark (bg-secondary track + border-strong thumb + text-muted hover). (3) Focus outline visible (2px solid focus-ring + offset 2px) para keyboard nav accessibility. (4) Text selection color legible (brand-shimano-blue bg + text-inverse). (5) Leaflet controls +/- + attribution adaptados a dark (bg-elevated + text-primary + border-default). (6) Shimano logos preservados con bg blanco explicito en dark (auth-logo/password-gate-logo/header-logo-wrap) - mantener brand recognition, invertir colores rompe identidad. Bundle ceiling smoke test v732 3MB -> v744 3.5MB (bundle actual 3.08 MB post-E3). Anterior: — PLAN_DARK_MODE E2 EJECUTADO: codemod Python property-aware reemplazo 1964 hex hardcoded por var(--token) semantic en index.html (1554 reemplazos) + 21 archivos src/domains/*.js (410 reemplazos). Reduccion 49% de hex en index.html (2799 -> 1424 remaining). Codemod guardado en scripts/dark_mode_codemod.py como tooling reusable con dry-run/apply modes. Es la solucion de raiz vs los overrides layer-on-layer de v738-v742. Los 1424 hex remaining son: solid dark bgs (#0f172a/#1e293b/#334155 preservados intencional para no romper light mode), rgba(), gradients, shadows, SVG stroke/fill, y algunos vivid custom colors fuera del top-40. Approach codemod: property-aware (solo reemplaza en color/background/border/border-color/border-*/border-shorthand con hex del top-40) para no romper cases donde el hex es intencional. Preservados literales: #fff/#ffffff (mantiene white text on solid buttons como .btn-cancel color:#fff bg:#dc2626), #0f172a/#1e293b/#334155 como bg (badges negros/gradient heads). Post-codemod: dark mode tiene ~99% coverage (los overrides css [data-theme="dark"] layer-on siguen para edge cases). Light mode: 0 regressions - todos los vars en light-mode devuelven los mismos hex originales que estaban antes. Anterior: — Dark Mode contraste fixes especificos (Mariano screenshot). Problemas identificados: (1) header buttons (Salir/Usuarios/Productos/etc.) blending con el gradient slate del header porque bg-elevated (#334155) era casi igual al gradient end. Fix: forzar bg-input (#1e293b, mas oscuro) para .logout-btn + selectores altamente especificos .header .logout-btn y .user-info .logout-btn + border-strong. (2) .tab-btn del sidebar-right (Localidades/Clientes/Pedidos/Rutas/Visita/Contactado/etc.) - v740 solo tocaba color, faltaba bg y border. Fix: bg-elevated + border-default + hover con brand-blue border. (3) .slft-wl-* children (PEDIDOS EN ESPERA cards) tenian color hardcoded en base rules (#0f172a/#64748b/#94a3b8) invisible en dark. Fix: overrides explicitos por child (.slft-wl-cli/loc/meta/hdr/orden/count/search-row input) con text-primary/secondary/muted correspondiente. (4) .stat-box refuerzo (v740 debia funcionar). (5) role-badge filter brightness(1.2) para mejor visibilidad. Header h1 color text-primary explicito. #stats-bar-sidebar + stats-bar-left bg-secondary explicito. Anterior: — Dark Mode PASS MASIVO de contraste. Audit revelo 100+ CSS classes con hardcoded light bg no cubiertas. Agregados overrides class-based (con !important) para las mas visibles agrupadas por prefijo semantico: .header (gradient slate), .auth-box/.auth-btn/.auth-logo/.password-gate-box/.password-gate-logo (login/PIN screens), .ruta-card/.ruta-tienda/.ruta-status/.ruta-progress-bar (Rutas panel), .qmodal-box/.qmodal-opt/.qmodal-foot (custom confirm modals), .ac-* (Alta clientes: subtab-btn/.app-card/.section-title + inputs), .rd-* (Rendiciones: subtab-btn/mias-card), .notif-item, .ccm-* (Client contact modal), .cv-section/.cu-pedido/.cr-product (Client detail sections), .sap-input/.sap-int-col/.sap-int-file-info/.mc-addr-input/.mc-prov-count (SAP+Master Cli inputs), .sync-tag (toast), #splash+#auth-overlay bg, .revision-item + [class*="wl-"] (waitlist cards), [class*="pp-"] (product picker), [class*="pc-"] (Panel de Control), labels genericas, tables th/td genericas fuera de .users-table. Hover states para exp-opt/ccm-opt/ruta-card/notif-item. WCAG AA aspirado. Preserva light mode identico. Anterior: — Dark Mode iteration: mejor contraste + class-based overrides. Mariano reporto screenshot con problemas: (1) botones logout-btn del header (Salir/Usuarios/Productos) invisibles porque bg blanco no se pisaba (CSS class no matcheaba attribute selector inline). (2) stat-box del header (461 LOCALIDADES etc.) light bg + dark text = inconsistente con dark mode. (3) waitlist cards de "PEDIDOS EN ESPERA" con bg blanco. (4) hdr-info-badge (v738/SAP down) opacity issue. Fix: bloque extenso agregado al dark mode CSS con overrides class-based (no solo attribute-based): .logout-btn, .stat-box, .stat-box.num/.lbl, .hdr-info-badge + variants sap-ok/warn/err (colores semanticos preservados con paleta dark), .users-card/.users-table, .exp-opt (export dialog), .tab-btn, .waitlist-item, aside/sidebar genericos, sidebar-right, modal-box headings/labels/subt/modal-head, .btn-cancel/.btn-confirm/.danger-btn. Ademas mas attribute selectors para inline pastel colors: green-50 (#f0fdf4), emerald-50, blue-50, violet-100, indigo-100, fuchsia-100, green-100 (#dcfce7), sky-100/200, amber-100, red-100, blue-100. Text color overrides para colores semanticos hardcoded: #166534 (success), #dc2626 (danger), #7c3aed (violet), #92400e (warning), #991b1b (danger-dark). WCAG AA aspirado en semantic states. Anterior: — FIX field names en exports Backorder + Stock Asignado (v737). Bug reportado por Mariano con screenshot: columnas Fecha_Pedido, CardCode, Provincia, Localidad, Vendedor salian vacias en el Excel exportado. Root cause: (1) mismatches de nombres de campo (mi export usaba clientProvince/clientLocality/vendedor|vendorAssigned pero el schema real es province/locName/ownerVendor), (2) globalPedidos es una proyeccion filtrada del snapshot Firestore (line 22685) - copia solo un subset de campos, y NO incluia createdAt, clientCardCode, ownerVendor, closedAt ni migrationSource. Fix: (a) extendida la proyeccion en unsubPedidosAll listener para incluir los 5 campos faltantes, (b) exportBackorderAll+exportStockAsigAll usan ahora p.province, p.locName, p.ownerVendor (nombres reales). Bumps APP_VERSION+CACHE_VERSION v738 -> v739. Anterior: — Dark Mode FUNCIONAL implementado (Mariano pidio arrancar ya vs plan multi-fase). Approach hybrid vs full-refactor del PLAN_DARK_MODE.md: CSS variables para elementos estructurales (body/modales/inputs/cards/borders) + attribute selectors `[style*="background:#XXX"]` para pisar inline styles hardcoded top-15 sin refactorizarlos todos. Coverage estimada ~80% del app sin tocar los 4147 colores. Cambios: (1) init inline en <head> ANTES de <link>/<style> para prevenir FOUC (lee localStorage->OS preference->apply data-theme antes de renderear body). (2) Bloque CSS con 25 vars semanticos (bg/text/border/brand/semantic/interactive) + [data-theme="dark"] con overrides + attribute selectors para inline styles frecuentes. (3) Toggle button real en el header (reemplaza placeholder v736): SVG dinamico luna/sol, click toggleDarkMode() togglea data-theme + localStorage + firestore sync a userData/{uid}.themePreference (per decision Mariano PLAN §10). (4) applyThemeFromUserData() en userData listener: cross-device sync solo aplica si localStorage local esta vacio (precedencia localStorage > Firestore > OS). (5) Transitions suaves .2s en body/modales/inputs, respeta prefers-reduced-motion. Boton toggle sigue hidden en mobile via @media (header apretado; user togglea desde desktop y syncronea cross-device). Limitaciones honestas: algunas secciones con inline styles muy especificos (buttons custom bg colors, badges con hex inline no en top-15) probablemente se van a ver light en dark mode. Se iteran con overrides puntuales en v739+ segun feedback. PLAN_DARK_MODE.md queda vivo para refactor completo E2 (colores semanticos-first + codemod). Anterior: — FIX exports Backorder y Stock Asignado que se descargaban vacios en Excel. Root cause identificado: la migracion BO/ASIG del 2026-08-28 (62 SQs oficina -> pedidos-app sinteticos) seteo createdAt = fecha ORIGINAL del SAP SQ (potencialmente marzo/abril/julio, no la fecha de migracion). El export por mes (showMonthPicker) filtraba pedidos por createdAt del mes elegido, entonces los 62 pedidos migrados quedaban afuera y el Excel salia con 0 filas. Fix: eliminado el mes-picker para BO/ASIG - ahora son SNAPSHOT ACTUAL, iteran todo globalPedidos sin filtro de mes. Nuevas funciones window.exportBackorderAll / window.exportStockAsigAll con diagnostico incorporado (si 0 filas, alerta explica cuantos pedidos hay open, cuantas lineas BO/ASIG con qtyOpen>0, y posibles causas). Descripciones del modal actualizadas: "Backorder (snapshot actual)" y "Stock Asignado (snapshot actual)" en vez de "del periodo". Filename ahora Shimano_Backorder_Snapshot_YYYY-MM-DD.xlsx. Bumps APP_VERSION+CACHE_VERSION v736 -> v737. Colateral: PLAN_DARK_MODE.md tabla de bumps + LOOP_PROMPT_DARK_MODE.md shifted +1 (E1=v738 en vez de v737) porque este v737 se uso para el fix urgente. Anterior: — 2 cambios en la UI del header: (1) FIX FAB WhatsApp oculto en mobile (Mariano reporto que quedaba tapado por la barra inferior del browser mobile - Chrome address bar + tab strip). Fix CSS puro en @media(max-width:700px) con `#wa-fab{display:none !important}`. (2) NEW boton "Dark Mode" en el header top-right junto a Salir (SVG moon icon 14x14 + label "Dark", clase .logout-btn compartida). Click abre nuevo modal-overlay #dark-mode-modal placeholder "En construccion" (mismo pattern que Deposito v735). Boton tambien oculto en mobile via misma @media rule. Full implementacion Dark Mode se difiere - requiere refactor de cientos de colores hardcoded en inline styles y CSS a CSS variables + paleta oscura completa disenada. Estimado 4-8 hs de dev como feature dedicada. Nuevas funciones window.openDarkModeModal / closeDarkModeModal. Anterior: — boton "Espera" del sidebar-left renombrado a "Deposito" y hijackeado su click para abrir nuevo modal "En construccion" (placeholder de la futura integracion con el WMS SETUP del partner logistico). Motivo: reunion tecnica agendada para martes 2026-09-02 con la gente de SETUP para definir API access + estados + endpoints. Post-integracion el modal va a mostrar timeline PICKING -> ARMADO -> DESPACHADO -> ENTREGADO en tiempo real por pedido. Cambios: (1) sidebar-left mapping linea 19088 - reemplazado {id:waitlist-btn,label:Espera} por {id:waitlist-btn,label:Deposito,overrideClick:openDepositoModal} usando el mecanismo overrideClick que ya existia (mismo pattern que Estado v504). (2) nuevo modal-overlay #deposito-modal con head gradient azul + emoji construccion + copy "En construccion" + info-box con status de la integracion. (3) nuevas funciones window.openDepositoModal / closeDepositoModal. El boton waitlist-btn original queda vivo en el DOM (no se rompe la lista "PEDIDOS EN ESPERA" del sidebar-left que replica el contenido). Doc adicional: Brief_Reunion_SETUP_Integracion.md + .docx en la raiz del proyecto (checklist tecnico + business + arquitectura propuesta + estimacion 8-10 dias para llevar a la reunion). Anterior: — quitar emojis rotos de 5 botones del top toolbar (btn-dashboard, btn-clientes, btn-alta-cli-top, btn-rendiciones-top, btn-seguimiento). Mariano reporto que emojis Unicode range U+1F300+ (chart bar, moneybag, busts, etc.) rendizaban como cajones/tofu en algunos fonts del OS/browser. Fix: removidas las 5 rules `::before` con content Unicode; solo texto queda en los botones. Los otros 2 emojis (star U+2605 en Campanas Activas + downwards arrow U+2193 en Exportar a Excel) se mantienen porque son Basic Multilingual Plane y rendizan universalmente. Zero cambio funcional, solo cleanup visual. Anterior: — fix columna WhatsApp del Panel Usuarios: numero de telefono full (ej. 5491126762031, 13 digitos) se veia truncado como "54911:" porque el <td> no tenia width fijo y el input width:100% no forzaba minimo — la columna colapsaba. Fix: agregado class "wa-col" al <th> y <td> del WhatsApp + CSS .users-table th.wa-col{min-width:150px}. Cambio triple-tocado: index.html (nueva rule CSS), src/domains/admin-users.js (2 lugares: TD del loop de rows + TH del header). Zero cambio funcional, solo layout. Anterior: — dataset ML export ahora incluye 3 snapshots BQ->Firestore que antes estaban excluidos: sap_snapshot (fuente de verdad de facturacion REAL SAP - neto NCs, con importeLineasArsNeto sin IVA para no sobreestimar), facturacion_snapshot (agregados hoy/mes/ano en ARS por vendedor + TOTAL_NACIONAL, listos-para-benchmark), backorder_snapshot (demand supression - lineas de SQs pendientes de despacho por vendedor con sku/pendiente/pedido/stock_actual/precio/cliente). Motivo: en la revision del "Dataset para analisis (ZIP)" con Mariano identificamos que sap_snapshot era el mas critico faltante para modelos de negocio (los pedidos-app son confirmations pero SAP tiene ARS neto real post-descuentos+NCs+IVA correcto). Sin esto no se podian hacer regresion de precio ni churn medido por facturacion real ni analisis descuento efectivo. 3 nuevos schemas en DATASET_SCHEMAS + 3 row builders en ROW_BUILDERS (sap_snapshot: 12 cols one_per_doc; facturacion_snapshot: 6 cols one_per_doc; backorder_snapshot: 18 cols flatten_lines - una fila por linea de backorder con header replicado). Manifest exclusions actualizado: sap_snapshot removido del array excludedCollections. Dataset ZIP pasa de 10 a 13 CSVs Firestore + productos.csv + manifest.json. Anterior: — Apple-style segmented control en los tabs del modal Dashboard (Targets/Visuales/Visitas/Finanzas). Bug reportado por Mariano: los tabs se veian "raros" — cajones planos con background celeste ancho, boton activo apenas distinguido, tipografia con text-transform uppercase + letter-spacing agresivo (estilo Bootstrap 2010, no Apple). Fix: nuevo pattern iOS segmented control con container pill gray (#f2f2f7 iOS system gray 6) y boton activo con background white + soft shadow (0 3px 8px rgba(0,0,0,.04)). Tipografia sentence-case (no uppercase), font-weight 600, font-family -apple-system/BlinkMacSystemFont/SF Pro Text/system-ui, letter-spacing negativo (-.01em) matcheando el kerning que Apple usa en display sizes chicos. Interaction feedback per apple-design skill §1: :active con transform scale(.96) para tap-down. Transition 200ms ease-out en bg/color, 150ms en transform. @media prefers-reduced-motion respetado (skill §14). HTML mucho mas limpio: reemplazo 4 buttons con ~400 chars de inline styles c/u por buttons con clase .dash-tab-btn (+ .active). JS setDashboardTab ahora usa classList.toggle("active") en vez de setear inline background/boxShadow. Rediseno acotado a este componente — el resto de los ~332 botones del app siguen con su estilo actual (rediseno full sigue diferido a fase futura). Anterior: — mobile stat cards del header (LOCALIDADES/CLIENTES EN SAP/LEADS/TIENDAS) ahora vertica-centrados con min-height uniforme y wrap seguro del label. Bug reportado por Mariano: en mobile el card "CLIENTES EN SAP" hacia wrap del label a 2 lineas y esa card quedaba visualmente mas alta que las demas + el numero pegado arriba en vez de centrado. Fix CSS puro en @media(max-width:700px): .stats-bar align-items:stretch, .stat-box display:flex column + justify-content:center + min-height:52px + overflow:hidden + text-align:center; .stat-box .lbl overflow-wrap:break-word + word-break:break-word + hyphens:auto para wrap seguro. Cero cambio en desktop. AUDIT extra: verificados TODOS los botones Eliminar/Borrar/Delete del app (via grep) — ya son 100% rojos (#dc2626 via .btn-cancel/.danger-btn/.app-btn-red/.qmodal-btn.danger o inline style). Sin cambio de codigo en botones destructivos, el estado ya era correcto. Rediseno completo estilo Apple para todos los ~332 botones queda como fase futura con plan escrito (writing-plans skill) por scope grande. Anterior: — 2 fixes en Dashboard: (1) INFRA: re-deploy firestore.rules a produccion via firebase CLI. Las rules del repo se venian mergeando desde v641 (PR #321, hace meses) pero NO habia CI workflow que las deployara automaticamente. Sintoma: tab "Visuales" del Dashboard mostraba "Missing or insufficient permissions" porque la rule para `dashboard_visuales` estaba en el file pero no en el server. Fix: firebase deploy --only firestore:rules. Aprovecho para dejar constancia: cualquier merge que toque firestore.rules requiere deploy manual (TODO: agregar CI action). (2) FIX lazy-init del listener de visits en el tab Visitas del Dashboard. El listener canonico vive en visitas.js:openVisitaModal y solo se attachea si el user abrio el modal Visita antes. Sin eso, visitsCache queda vacio y el tab se veia permanentemente sin data. Ahora renderDashboardVisitas attachea el listener idempotente si no esta activo + re-render on snapshot + loading state "Cargando visitas..." en primera carga. Anterior: — nuevo FAB WhatsApp circular fixed bottom-right (56x56, verde #25D366 con icono SVG oficial WhatsApp) visible en TODA la app cuando currentUser esta seteado. Click abre https://web.whatsapp.com/ en nueva pestana sin destino pre-cargado; el user despues elige a quien escribirle. Motivo Mariano: shortcut universal para todos los users logueados (VDE/VDI/admin/gerente) que necesitan saltar rapido a WhatsApp sin abrir otra pestana. Toggle en onAuthStateChanged: display flex on login exitoso, display none on logout. z-index 9990 (debajo del auth-overlay para que quede tapado durante flujo de login). Position bottom:40px right:24px para no chocar con Leaflet attribution del mapa. Hover: scale(1.08) + shadow bump. HTML antes de </body>. Anterior: — cambio de color columna "Libre para la venta" en modal Pedido en Espera: paleta yellow-100/amber-900 (#fef9c3 + #78350f) -> green-100/900 (#dcfce7 + #14532d). Motivo Mariano: verde comunica mejor la semantica (stock disponible = accion positiva = pintar de verde). Cambia en 2 lugares: (1) header inline style linea 4114, (2) CSS class .disp-sap-col linea 1427 aplicada a las celdas. Matchea el pattern del app que ya usa color-100/color-900 (ej. Pedido Final = blue-100/blue-800). Anterior: — rename columna "Pedido" -> "Pedido Original" para diferenciar de "Pedido Final" (v716). Tooltip clarificado: "Cantidad ORIGINAL que pidio el cliente (antes del split contra stock disponible)". Tooltip de Pedido Final actualizado para referenciar Pedido Original en la formula min(Pedido Original, Libre para la venta). Confirmacion Mariano: lo que va a SAP = Pedido Final; split v600 al confirmar crea 1 linea state=confirmed (=Pedido Final) + 1 linea state=BO (=Pedido Original - Pedido Final) que queda en Backorder App. Anterior: — Vista Preliminar (Revisá tu pedido) ahora tiene solo 1 boton "Volver a editar". Removidos "Dejar en Espera" y "Ir directo a Pendientes". La Vista Preliminar es solo REVIEW, no accion: el vendedor guarda desde "Enviar a lista de espera" en el picker de productos, y pasa a Pendientes desde el modal Pedido en Espera post-review. Preserva reviewDejarEnEspera y validateReviewAndPasarAPendientes por compat. Anterior: — consolidar duplicados cliente/SKU en Backorder + StockAsig. Antes: LUIS ZYSMAN con 1u y 11u aparecia como 2 filas. Ahora: 1 fila de 12u (suma). Consolidacion post-FIFO cap por (cardCode+nombre): suma pendiente + qtyAsignada + qtyBackorder. Se preserva un array pedidoIds con {pedidoId, state, qty} por linea original para que el boton Eliminar (que ahora dice ELIMINAR (N) si son N lineas) cancele TODAS via nueva funcion deleteAllBackorderAppLinesForClient (mismo pattern optimistic update + Firestore batch). Anterior: — preservar scroll en renderBackordersTab post Eliminar. Mariano reporto que al eliminar cliente de card SKU, el card entero parecia desaparecer. Root cause UX: renderBackordersTab regenera innerHTML entero. Antes: list.scrollTop se reseteaba a 0. Sort DESC por totalPendiente + reset a top = card visualmente desaparecio para user que estaba en el medio de la lista. Fix simple: capturar list.scrollTop antes del innerHTML=, restaurar despues. Card ahora se queda en su posicion visual (aunque haya bajado en el orden por menos unidades). Anterior: — FIX exportBackordersToExcel roto desde v700. Antes: leia de backorderLines (SAP-source) que v700 dejo vacio permanente al migrar todo a APP-source. Ahora: itera globalPedidos linea por linea state=BO/ASIG con qtyOpen>0, arma rows con SKU/producto/cliente/vendedor/qty/precio/importe/docNum/fecha. Respeta modo del modal (urgente=sin stock, asignacion=con stock) + filtros mes/busqueda. Anterior: — refresh instantaneo post Eliminar en Backorder/StockAsig. Antes Mariano tenia que refrescar la app entera despues de tocar Eliminar. Bug: deleteBackorderAppLine hacia el update a Firestore pero no re-renderizaba el modal. Solo se veia el cambio cuando el snapshot listener (unsubPedidosAll) llegaba (~200-500ms) + el listener no invocaba renderBackordersTab. Fix doble: (1) optimistic update en deleteBackorderAppLine — actualizo globalPedidos local + renderBackordersTab inmediato antes del await Firestore. (2) unsubPedidosAll listener ahora re-renderea renderBackordersTab si el modal esta abierto. Combined: cambio se ve al momento sin refresh. Anterior: — consistencia Unidades Reservadas vs modal STOCK ASIG. Bug: STC66HC2 mostraba Reservadas=50 (comprometido total incluye 46 BO sin stock) pero el modal STOCK ASIG mostraba 4 (unicas con backing real). Fix: (1) Stock Completo cell usa getStockDisponibleVenta (whs 11 only, no transit) para matchear con lo que dice el label dep 11. (2) Unidades Reservadas capadas: min(comprometido, stockDep11). Ahora matches el modal. Tooltip enriquecido con demanda total + demanda sin backing (backorder puro) + breakdown por cliente. Anterior: — rename columna Subtotal (Confirmacion Inmediata) -> Subtotal Final. La formula ya era Pedido Final x Precio (subtConSap = min(qty, r.disponible) * precio) — solo el label estaba desalineado con la nueva columna Pedido Final de v716. Header con fondo verde pastel para reflejar que es el valor confirmed. Anterior: — mejoras seccion Stock Asignado del cliente (banner azul modal Pedido en Espera). (1) Quitar texto (sin catalogo). Ahora si it.desc esta vacio busca en PRODUCTS por code y si no encuentra oculta el div desc completamente. (2) Highlight amarillo cuando el SKU ya esta en el pedido actual (w.items[]). Fondo #fef3c7 + border ambar + label ⚠ TAMBIEN EN EL PEDIDO ACTUAL. Permite al vendedor detectar instantaneamente que el cliente esta duplicando demanda. Anterior: — 2 cambios en tabla Pedido en Espera: (1) Revert de v714 en semantica de columnas Unidades Reservadas/Libre. Mariano pidio consistencia con el modal STOCK ASIG. Ahora Unidades Reservadas = TOTAL comprometido (getStockDesglose.comprometido) matches lo que aparece en STOCK ASIG. Libre para la venta = r.disponible = stock - reservas TOTAL = disponibleReal (matches el split flow). (2) Nueva columna Pedido Final entre Pedido y Precio. Pedido Final = min(Pedido, Libre). Es lo que efectivamente va a SAP como confirmed. Fondo azul pastel. Tooltip explica el calculo. Anterior: — boton Fusionar en tabla Pedido en Espera. Cuando la fila tiene badge SKU repetido (v714), al lado se agrega un boton azul 🔁 Fusionar (+Xu) que consolida las lineas ASIG/BO viejas del cliente al pedido actual. Usa _findAsigDelCliente para obtener las lineas + itera _asigInlineResolve con action=full en cada una. Efecto: qty totales suman via waitlistAddProductWithQty, lineas viejas quedan state=recycled. Anterior: — desglose por cliente en tabla Pedido en Espera + alerta SKU repetido. Nueva funcion pura getStockPorCliente(sku, cardCode) devuelve fisico/reservadasPorCliente/reservadasPorOtros/libreParaCliente/disponibleReal/yaEnOtroPedido. Columnas UNIDADES RESERVADAS ahora muestra reservas de ESTE cliente (no todos). LIBRE PARA LA VENTA = fisico - reservas de OTROS clientes (las suyas cuentan libres para el). Filas con SKU repetido (cliente ya lo tenia en otro pedido open) reciben clase repeat-sku fondo naranja pastel + badge de warning en el codigo + summary aparte con lista. Anterior: — FIX undefined-undefined en tab Pedidos > Pendientes. renderPendientesList antes leia Object.entries(pending) del localStorage donde habia entradas basura con keys undefined|undefined|... (pedidos guardados sin clientProvince/Locality). Ahora lee directo de globalPedidos filtrado por stage=pending + !transferidoSAP.docNum. Mismo pattern que _estadoGetPendientes del modal Estado de pedidos, ya validado en v708. Bonus: la lista deja de mostrar entradas fantasma del cache local viejo. Anterior: — unificar semantica de Eliminar linea (auditable). deleteBackorderAppLine (modal Backorder + StockAsig) antes FILTRABA la linea del array de pedido.lines perdiendo auditoria. Ahora la marca in-place: state=cancelled, qtyOpen=0, qtyCancelled += qtyOpen, cancelledAt=now, cancelledFrom=stock-asig-modal|backorder-modal. Mismo pattern que _asigInlineResolve (v704). Beneficios: (1) reportes historicos de que vendedor cancelo que linea. (2) codigo consistente entre las 2 formas de eliminar. (3) getStockRealmenteDisponible sigue no restando cancelled porque no esta en STATES_QUE_RESERVAN. UX sin cambios visibles. Anterior: — 3 cambios exports: (1) QUITAR Ventas y Rutas del modal Que queres exportar? (Mariano no los usa mas). Elimino del HTML + del allowedByRole. Los handlers exportVentasForMonth/exportRutasForMonth quedan como codigo muerto por si se reactivan. (2) Rendiciones ahora SIEMPRE embebe la foto del ticket: si es dataURL base64 (comportamiento existente) sino fetch del fotoTicketUrl (Firebase Storage) + arrayBuffer + wb.addImage. Fallback a hyperlink solo si el fetch falla (CORS/red). (3) Visitas ya embebia fotos (frenteLocal como dataURL), sin cambios. Descripcion en el modal actualizada para mencionar las fotos embebidas explicitamente. Anterior: — QUITAR filtro Sin/Con/Todos stock del modal Backorder. Antes v420 lo agrego como select con 3 opciones, v523 lo escondio en mode=asignacion. Ahora display:none en ambos modos. La logica del filtro sigue viva (backordersStockFilter): _applyBackordersMode setea sin en urgente y con en asignacion automaticamente. Los 2 modales quedan mas limpios. Anterior: — 3 exportaciones nuevas en modal Que queres exportar?. (1) Backorder: lineas state=BO open filtradas por mes de createdAt del pedido. (2) Stock Asignado: state=ASIG o BO+stock disp virtual filtradas por mes. (3) Pedidos del mes: todos los pedidos creados en el mes/anio (cualquier stage). Cada export tiene columnas contextuales (fecha pedido, vendedor, cliente, SKU, cantidades, precio, subtotal, estado linea, pedido_id, SQ_DocNum). Fuente: globalPedidos en memoria. Roles vendedor+interno ahora pueden acceder a los 3 exports nuevos. Anterior: — FIX pedidos huerfanos invisibles en tab Pendientes. renderPendientesList antes filtraba por visibleKeys.has(k) (POINTS/altas) y saltaba pedidos cuya key no matchea (ej PESCA PLANET y EL GRINGO PESCADOR reportados por Mariano). Fix: incluye huerfanos con badge amarillo sin catalogo — mismo pattern que v695 aplico a renderConfirmadosList. Ahora la tab Pendientes muestra los mismos pedidos que el modal Estado de pedidos > Pendientes. Anterior: — UX modal Targets mensuales. Ancho del modal 680 -> 920 px + inputs mas grandes (min-width 110px, padding 8x12, font 13.5px) + reparto de columnas mas equilibrado (Mes 12%, cada familia 19%, Total 18%, Estado 13%). Antes el nro se cortaba en montos >6 digitos (ej Julio Reel 228000 se veia como 228). Ahora se lee completo. Anterior: — FIX seccion ASIG en modal Pedido en Espera. _findAsigDelCliente antes solo devolvia lineas state=ASIG. Los 62 pedidos migrados hoy siguen state=BO hasta que E4.5 los promueva (proximo tick ~30 min). Ahora _findAsigDelCliente incluye state=BO cuando el SKU tiene stock disponible dep 11 (ASIG virtual), consistente con el modal STOCK ASIG. Cada linea muestra si es ASIG firme o BO+stock disp (ASIG virtual). Al accionar full/partial la linea pasa a recycled/ASIG igual que antes. Anterior: — PICKER usa disponible REAL en tiempo real. product-picker.js ahora usa window.getStockRealmenteDisponible (v701) en lugar de getStockDisponibleVenta crudo. El vendedor ve el stock que realmente puede prometer (fisico - compromisos-app confirmed+BO+ASIG) mientras arma el pedido, no el fisico del snapshot SAP. Nuevo badge amarillo pastel al lado del SKU con "X libres". Tooltip enriquecido con desglose fisico vs reservado. Cierra el gap UX identificado en v701: antes vendedor podia ver 45u en el picker cuando realmente habia solo 5u disponibles porque 40u ya estaban confirmed en otro pedido en flight. Anterior: — 3 opciones ASIG inline en modal Pedido en Espera. La seccion azul wcard-e4b-asig-app antes usaba _e4bAsigDelCliente (virtual FIFO) bajo flag isE4BEnabled y solo tenia botones Aceptar/Rechazar via CF. Ahora usa _findAsigDelCliente (lineas ASIG reales del cliente) sin flag, y muestra 3 opciones inline por linea: Eliminar (state=cancelled), Agregar completo (recycled, waitlistAddProductWithQty), Agregar parcial (prompt qty, resto queda ASIG). Handler _asigInlineResolve aplica directo a Firestore sin CF. Copy del banner actualizado. Reusa el pattern del modal ASIG obligatorio de v701 pero disparado directo desde el modal Pedido en Espera. Anterior: — FILTROS Pedidos Confirmados: (1) nuevo filtro Estado de pedido con opciones Oferta/Orden/Remitido/Facturado/Cobrado/Cobrado parcial/Cobrado completo. COBRADO agrupa PARCIAL+COMPLETO. REMITIDO placeholder (aun no lo trackea el sync SAP). (2) default mes/año actual en la primera carga (Agosto 2026 hoy). El default respeta la eleccion del user si ya toco el select (dataset.userTouched). Anterior: — UX MEJORAS BACKORDER/STOCK ASIG. 3 cambios: (1) Reestructura de columnas del modal Pedido en Espera. Sacadas: Backorder General, Backorder Cliente, Disponible app. Renombradas: Stock total -> Stock completo (dep 11), Disponible -> Libre para la venta. Nueva: Unidades reservadas (compromisos-app confirmed+BO+ASIG con tooltip breakdown). Antes 15 columnas, ahora 13, mas foco. (2) Boton ELIMINAR en modal STOCK ASIG - Con Stock (antes solo en BACKORDER). Extiende deleteBackorderAppLine con param targetState. Aplica a lineas state=ASIG source=app: al eliminar libera stock reservado al siguiente vendedor. (3) Toolbar 3 botones (Exportar todo, Exportar Stock Asignado por cliente, Graficos) uniformados a un mismo estilo primary (slate #1e293b), mismo min-width 160px. Antes tenian naranja/verde/ambar distintos que rompian consistencia visual. Anterior: — STOCK COMPROMETIDO EN TIEMPO REAL + MODAL ASIG OBLIGATORIO. Mariano decidio 2026-08-28: la app es duenya del stock. Al confirmar un pedido, esas unidades quedan comprometidas y NO se ofrecen al siguiente vendedor. Nueva funcion pura  con  que resta compromisos-app (confirmed+BO+ASIG open) del stock fisico. 22 tests unitarios. Integracion en confirm flow:  y  usan la nueva funcion como  del split (v600).  ahora ES el stock real. Ejemplo: 20u fisico + Santiago confirma 8u -> Mauricio ve 12u disponibles. Si Mauricio pide 14, van 12 a SAP + 2 quedan en BO app. Modal ASIG obligatorio pre-Pendientes: si el cliente tiene lineas ASIG open,  bloquea + abre modal con 3 opciones por linea (Eliminar / Agregar completo / Agregar parcial con input numerico). Firestore batch atomico para aplicar cambios (state=cancelled/recycled/ASIG con qtyRecycled). Anterior: — MIGRACION 100% BO/ASIG SAP->APP: apagado del path SAP-source en la UI. ensureBackordersListener ahora es no-op (backorderLines siempre = []). renderBackordersTab removio el early return "Sin datos de backorder" que dependia de SAP; ahora popula desde APP-source (globalPedidos state=BO/ASIG) y muestra placeholder contextual solo si tambien queda vacio. Precondicion: migracion masiva ejecutada 2026-08-28 (scripts/migrate_sap_backorder_to_app.py --apply) que creo 62 pedidos-app sinteticos desde los SQs SAP oficina (985 lineas / 2974u con state=BO). Total post-migracion: 128/129 SQs SAP tienen equivalente en app. Los 30 dups heredados (mismo cliente pidio 2 veces en SAP) quedan visibles en app y se limpian con boton Eliminar. Los SQs SAP quedan abiertos (Mariano decidio NO cancelarlos). Anterior:** **SW v680 en main (2026-08-26) — FIX defensive en sw.js: HTML cache corrupted bug. Mariano reporto ver CSS renderizado como texto en la app (screenshot). Root cause: race condition o response mal cacheado con content-type wrong. Fix: validar Content-Type text/html antes de cachear + antes de servir desde cache. Bump forzoso CACHE_VERSION invalida cache viejo corrupto. Anterior:** **SW v634 en main (2026-08-25) — PERF Fase 3: 3 libs pesadas removidas del initial load y cargadas ON-DEMAND: ExcelJS (~1.1 MB solo para template Stock Asig con logo), JSZip (~90 KB solo para export ZIP DTW), polygon-clipping (~50 KB solo para outlines admin). Total ahorrado ~1.24 MB del initial load. window.loadExcelJS/loadJSZip/loadPolygonClipping helpers en loader.js con cache+dedup. Polygon-clipping usa background load: mapa aparece inmediato, contornos se renderean ~200ms despues via requestIdleCallback. Anterior:** **SW v634 en main (2026-08-25) — PERF Fase 2: apple-design CSS (v650-v676) extraido a `./styles/apple-design.css` (217 KB, 6078 lineas). HTML baja de 2.04 MB → 1.87 MB. Bonus: el CSS externo se cachea independiente (invalidacion granular al cambiar solo apple-design sin bustear HTML) y browser puede paralelizar HTML parse + CSS download. sw.js incluye el nuevo asset en STATIC_ASSETS para offline. Cascade order preservado: `<link>` despues del `</style>` legacy. Anterior:** **SW v634 en main (2026-08-25) — PERF Fase 1: `<link rel=preconnect>` a 8 CDNs criticos (unpkg, jsdelivr, gstatic, cdnjs, sentry-cdn, firestore.googleapis, identitytoolkit, google) + `<link rel=preload as=script>` al app.bundle.js (2.4MB, el mas grande) + Leaflet + Firebase app/firestore. Descarga arranca inmediato en parallel con el HTML parse, TCP+TLS handshake antes de necesitar cada CDN. Impacto -300 a -1500ms en cold connection mobile. Zero riesgo (solo hint tags, cero cambio de comportamiento). Anterior:** **SW v634 en main (2026-08-25) — MOBILE responsive apple-design: @media max-width 700px con modales full-screen radius 0, header padding reducido, h2 17px, botones full-width, sidebars más compactos, sync-tag centrado bottom mobile, access-denied box padding reducido. Tablet 701-900px padding intermedio. Antes overrides v650-v675 !important sobrescribian los border-radius:0 full-screen que ya existian → esquinas cortadas en mobile. Anterior:** **SW v634 en main (2026-08-25) — apple-design BASE UNIVERSAL para todos los .modal-overlay/.modal-box/.modal-head sin override ID-specific. Cubre 28 modales adicionales (audit/backup/cliente/precios/stock/tienda/totp/password-gate/excel-pedido/export-analisis/etc). Overrides existentes v650-v673 ganan por specificity. Ademas toast sync-tag Apple dark pill + access-denied overlay con backdrop blur + card Apple. Anterior:** **SW v634 en main (2026-08-25) — Row hover azul oscuro #1e3a8a con texto blanco en tabla del modal Pedido en espera. Antes gris #fafafa muy sutil. Ahora contraste alto para que Mariano distinga bien la fila que sobrevuela. Inputs preservan bg blanco para editar. Badges rojos +N mantienen rojo con contraste sobre azul. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada al modal Estado del contacto (#contacto-estado-modal): 3 botones RESPONDIO verde/NO RESPONDIO gris/ELIMINAR rojo Apple con tracking negativo, header teal → white, info block gris Apple card, botón Cerrar pill outlined. Anterior:** **SW v634 en main (2026-08-25) — fix bug reportado por Mariano: columnas Provincia+Localidad vacias en el Excel de Graficos Campanias Activas. Ahora meta se captura al vuelo desde snapshot BQ + globalPedidos + fallback approvedAltasList por nombre normalizado (v649 solo cubria snapshot; naming mismatch snapshot vs POINTS clients dejaba tiendas SAP sin meta). Anterior:** **SW v634 en main (2026-08-25) — quitar tab \"Mapeo Productos\" del modal Carga a SAP (mismo patron data-legacy que \"Mapeo Clientes\" v418) + FIX bug en openSapPanel que sobrescribia el `style=\"display:none\"` de tabs legacy con `b.style.display = ""` — ahora respeta `data-legacy` para TODOS los roles. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a .confirmed-card en tab Pedidos vistas PENDIENTES + CONFIRMADOS (v665 solo cubrió CREAR con .pedido-client-card) + modal #zonas-modal Reasignación de zonas con 4 tabs POR TIENDA/LOCALIDAD/PROVINCIA/HISTORIAL, filters row, tabla apple, botones footer Exportar teal / Refrescar celeste / Guardar verde Apple (v670). Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a los 3 modales grandes del flujo de pedidos (v669): #pedido-modal (Crear pedido con filtros catalog + pedido split), #revision-modal (Revisión Excel vs Stock con drop zone + tabla dark), #waitlist-card-modal (Pedido en espera con secciones SAP Asignado / Backorder pendiente + tabla amplia). Bloque compartido reset + overrides por modal. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a modales #export-modal (Que queres exportar? con 7 cards de opciones) + #panel-control-modal (Mariano-only: infraestructura + workflows críticos + top issues Sentry + KPIs pedidos) v668. Header dark preservado en Panel Control (mantiene identidad de \"herramienta admin\") pero con typography Apple + botón REFRESCAR pill translucent. Cards pc-card blancas radius 12px con hover levantado, sections h3 uppercase gris system caption, tablas con th uppercase gris + rows apple. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada al modal #backorders-graficos-modal (Gráficos STOCK ASIG. / Gráficos Backorder - mismo modal en 2 modos, v667). Header inline gradient (verde asig / naranja backorder) → white con h2 negro. Chart bars con track gris Apple pill radius 999px + fill color semántico preservado. Selector vendedor pill 10px focus ring celeste. Botón EXCEL verde Apple pill. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a tabs Rendiciones + Alta clientes + modales Formulario Visita / Registro Contacto (mismo #visita-modal en 2 modos) + Localidad (v666). Sub-tabs pills segmented, forms con labels uppercase gris sistema + inputs pill 10px focus ring celeste, chips vf-multi pills celeste con active, botones bottom footer con submit color-preserving (violeta visita / teal contacto), CERRAR gris → rojo Apple hover. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a tab Pedidos (#pane-pedidos con toggle CREAR/PENDIENTES/CONFIRMADOS + cards con badges pill translucent) y modal client-menu (#client-menu-modal con opciones Última interacción / Ver últimas 3 compras / Ver recomendaciones + botones grandes Crear pedido verde Apple y Cargar pedido por Excel celeste) v665. Ahora tambien cubre secciones backorder pendiente/asignado (app) dentro del modal con estilos Apple destructive/info. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a tab Clientes del sidebar-right (#pane-clients) + modal Alta SAP (#sap-alta-info-modal DOM injected) v664. Cards de cliente con badges pill translucent (verde SAP EN MAPA, amarillo LEAD, naranja CONTACTADO), boton VER EN MAPA celeste Shimano pill. Modal Alta SAP: header verde gradient → white, botones estado LEAD verde/naranja/rojo Apple, Guardar verde Apple pill, Editar direccion outlined celeste, Cerrar gris → rojo hover. Anterior:** **SW v634 en main (2026-08-25) — apple-design batch a 8 modales adicionales (v663): #mis-camps-modal, #dashboard-modal, #seguimiento-modal, #export-format-modal, #backorders-modal (usado tambien por SAP Asignado), #estado-modal, #waitlist-modal, #forecast-modal. Bloque compartido reset overlay+box+head+close + overrides especificos por modal (tabs, filter buttons, cards, badges). Forecast tratado aparte por estructura dinamica. Total apple-design skill cubre ahora 15+ modales + top-bar + ambos sidebars + controles Leaflet. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a controles del mapa Leaflet (v662): botones zoom/refresh/geocode/recalc como pill Apple 12px con hover gris subtle, popups white radius 12px con shadow Apple + tipografía SF Pro, cluster circles con colores translucent Apple (verde/naranja/rojo según tamaño), attribution con backdrop-filter subtle. Perf hint: `#map { will-change: transform; contain: layout paint style }` para aislar el mapa como capa compositor y mejorar pan/zoom cuando sidebar/modales repintan. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a sidebar-right (`.sidebar` con tab grid 3x3 + Alertas y Tareas, v661), sidebar-left (v660), top-bar (`.header` + `.controls`, v659) + 6 modales (v650-v658). Mirror simetrico left/right con pills translucent verde/celeste/indigo, active state pill solido, sub-tabs underline celeste Shimano #49A2DA. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a sidebar-left (grid 3x3 botones + Pedidos en Espera, v660) + top-bar (`.header` + `.controls`, v659) + modales `#product-master-modal` (v658), `#sap-modal` (v658), `#admin-modal` (v657), `#master-cli-modal` (v656), `#targets-modal` (v655), `#campaigns-modal` (v650-v654). Cards blancos unificados, colores brand → pills translucent (verde/celeste/indigo Apple), celeste Shimano #49A2DA para primarios, tipografía SF Pro/system-ui, reduced-motion keyframe alternate. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a top-bar (`.header` + `.controls`, v659) + modales `#product-master-modal` (v658), `#sap-modal` (v658), `#admin-modal` (v657), `#master-cli-modal` (v656), `#targets-modal` (v655), `#campaigns-modal` (v650-v654). Header pills iOS con role-badge sistema (violeta→pill translucent), stats-bar white cards con num tracking -0.03em, filter bar fondo azul→gris `#f5f5f7`, selects pill 10px con focus ring celeste, Salir gris→rojo Apple. Cards blancos unificados, celeste Shimano #49A2DA para primarios, tipografía SF Pro/system-ui, reduced-motion keyframe alternate. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a modales `#product-master-modal` (Master Productos, v658), `#sap-modal` (Carga a SAP con sus 6 tabs, v658), `#admin-modal` (Roles y permisos, v657), `#master-cli-modal` (Master Clientes, v656), `#targets-modal` (Targets, v655), `#campaigns-modal` (Campañas, v650-v654). Cards blancos unificados, celeste Shimano #49A2DA para primarios, pills iOS-style, focus rings 0 0 0 3px rgba(73,162,218,0.20), tipografía system-ui/SF Pro, keyframes shared `appleOverlayFadeIn` + `appleModalSlideIn` + reduced-motion alternate. Anterior:** **SW v634 en main (2026-08-25) — apple-design skill aplicada a modales `#admin-modal` (Roles y permisos, v657), `#master-cli-modal` (Master Clientes, v656), `#targets-modal` (Targets, v655), `#campaigns-modal` (Campañas, v650-v654). Cards blancos unificados, celeste Shimano #49A2DA para primarios, pills iOS-style, focus rings 0 0 0 3px rgba(73,162,218,0.20), tipografía system-ui/SF Pro, keyframes shared `appleOverlayFadeIn` + `appleModalSlideIn` + reduced-motion alternate. Anterior:** **SW v634 en main (2026-08-25) — Sesión maratónica: Panel Control fixes + Backorder semantic + Master Clientes + Campañas Gráficos.** Cambios ordenados: **(v623)** fix scope let→var (Panel Control mostraba 0/sin datos porque STOCK_BACKORDER/globalPedidos/opsLogCache/STOCK_ASIG_APP etc estaban en let inline no expuesto al bundle IIFE — mismo pattern Rule #13 CLAUDE.md). **(v624)** card Sentry Rate Spike renderizada (fn `computeSentryRateSpike` existía desde v616 pero no estaba conectada). **(v625-v627)** card Firestore Quota en Panel (Cloud Monitoring API — reads/writes/deletes 24h vs free tier; v625 script + workflow + card, v627 per-metric try/except porque `storage_bytes` no existe en todas las editions Firestore Native → falla el sync entero). Números iniciales prod: reads=319k/día (638% free tier), writes=106k/día (532% free tier) — proyecto en Blaze, no crisis, ~$10/mes costo Firestore. Sentry configurado en session: token creado con scopes `event:read + org:read + project:read`, project slug real `javascript` (no `app-vendedores`), fix v303 al workflow. **(v626)** fix provincia MC se pierde al refrescar (root cause: `saveMcAddr` tab SAP NO seteaba `provinciaLocSource='manual'` → cron `sync_sap_to_firestore.py` cada 5 min pisaba con SAP vacío. Fix mismatch v291+/v300+ que ya protegían tab Provisorios). **(v628-v631)** refactor semántica BACKORDER/STOCK ASIG modales — Mariano feedback definitivo screenshot: **SKU-level filter** (SKUs sin stock → BACKORDER, con stock → STOCK ASIG, no state-level). v628 fix BUG1 crítico `getStockDisponibleVenta` fallback a `getStockQty` que incluía dep 12 tránsito → over-promise a SAP con SKUs sin stock vendible. v631 revirtió el semantic de v628 (state-level fue demasiado restrictivo — dejaba BACKORDER modal con SKUs con stock) manteniendo el fix del fallback. **(v629)** fix Sentry ReferenceError `renderReviewLines is not defined` (27 events/mes en prod desde v337 — mismo pattern Rule #13: `window.renderReviewLines = renderReviewLines` faltaba en pedidos-modal.js. Otras funciones del módulo como `setOrderQty`/`setOrderPrice`/`removeFromOrder` sí estaban expuestas desde product-picker.js). **(v630)** feature nueva: datos de entrega pre-cargados por cliente. Nuevo campo `client_master.defaultDelivery` (misma shape que `pedido.formaEntrega`). `saveMcAddr` post-add escribe defaultDelivery (fire-and-forget, modo Y "último uso gana"). `openReviewDialog` pre-fill inputs desde `client_master.defaultDelivery`. Master Clientes muestra badge azul "📦 ENTREGA: TIPO" bajo el nombre con tooltip completo (transportista, dirección, DNI/patente). Elegido: opción A (auto-save primer uso) + opción Y (edits auto-actualizan default). **(v632-v634)** botón "📊 GRÁFICOS" naranja en modal Campañas Activas con nuevo modal `mis-camps-graficos-modal` — 2 charts HTML/CSS puros (sin lib externa): (1) unidades por SKU top 30 en orden desc, (2) tiendas que compraron el SKU seleccionado via dropdown. v633 fix delta reportado 406 vs 314 PowerBI — cambio de `qty` (pedido bruto) a `qtyInvoiced` (facturado real, matchea PowerBI, poblado por CF `syncSapInvoicesToApp` cada 15 min). v634 UX crítico: toggle Facturadas/Pedidas + auto-flip a Pedidas si Facturadas vacío + banner amarillo "no hay unidades facturadas todavía (sync SAP Invoice cada 15 min)" + header con ambos totales side-by-side + delta pendiente. Botón "📋 EXPORTAR EXCEL" verde con 2 hojas (Por SKU + Por SKU y Tienda) con ambas métricas + delta. **Ops de sesión**: sesión también arregló ~15 lint errors de biome (`useIterableCallbackReturn` — arrow forEach sin llaves) que empezaron a fallar CI de una release patch de biome, corrección via `biome check --write --unsafe src/`. Limpieza de `deploy-*.log` y `_verify_live_split.mjs` que se colaron en v630 por `git add -A` (removidos + agregados a .gitignore como `deploy-*.log`). Token Sentry compartido en chat: usuario decidió no rotar (cerrando terminal cierra acceso). **v623 previo — v614:** **SW v614 en dev (2026-08-24) — PANEL DE CONTROL completo (iter 4/4 Loop Engineering).** Nueva sección Mariano-only con estado de infraestructura (Stock Snapshot, GitHub Actions, Sentry, CF Invoice Sync), KPIs internos (pedidos por stage, backorder totals, duplicaciones STRICT) y opsLog. Ver sección 15) abajo. **v607-v613 previos**: auto-confirm 100% BO, fix sync SAP overwriting direcciones (v608), rendiciones email corto + Diego (v609), pin google-cloud-firestore<2.29 (regresión Google 2026-08-24, v610), y las 4 iteraciones del Panel: skeleton+KPIs Firestore (v611), GH Actions status card (v612), Sentry issues card (v613), CF Invoice Sync + README (v614). **SW v607 previo (2026-08-24) + fix Python sync (v608) — Guard calle/codigoPostal en `sync_sap_to_firestore.py`.** Bug reportado por Mariano 2026-08-24 (Master Clientes Direcciones): admin cargaba dirección, click GUARDAR, mostraba "Guardado", 5 min después al refrescar volvía a vacío. Root cause: `sync_sap_to_firestore.py:1049` escribía `calle: bp.get('Address', '') or ''` sin guard "pop-if-empty" (a diferencia de localidad/provincia que sí tenían el guard v291+/v300+ de 2026-07-13). El sync corre cada 5 min (`sync-sap-catalog-stock.yml` cron `*/5`), así que <5 min entre save y refresh mostraba el bug. Fix: agregar pop-if-empty para `calle` + `codigoPostal` con misma lógica que localidad. No requiere APP_VERSION bump (script Python-only, no toca PWA). **v607 previo — auto-confirmar pedidos 100% BO.** Problema reportado por Mariano 2026-08-24: 3 pedidos (LUIS ZYSMAN, PESCA ESCOBAR, BROBRO SA) quedaron atascados en PENDIENTES porque el vendedor nunca clickeó "CONFIRMAR DEFINITIVO", y como todos los ítems eran BO (sin stock), nunca aparecieron en Backorder App. Fix: `confirmExcelPedido` post-save chequea si todas las líneas son `state='BO'` → auto-`update` con `stage='confirmed'` + `transferidoSAP={via:'app_only', reason:'all_lines_bo'}` (mismo shape que sap-auto-send-listener:126-148). Elimina el paso manual "definitivo" cuando no hay nada que ir a SAP. Además: `volverAPendientes` + visibilidad del botón `pm-actions-confirmed` ahora permiten a **cualquier vendedor** revertir pedidos con `via='app_only'` (no hay SQ SAP para tocar → inocuo). Los 3 pedidos existentes se movieron manualmente con `scripts/_move_pending_bo_to_confirmed.py`. **SW v606 previo (2026-08-24) — Fix STOCK ASIG. semantica (solo APP-ASIG genuinos).** Bug reportado por Mariano 2026-08-24: ANA LORENA FUENTES aparecía en STOCK ASIG. modal aunque su pedido nunca fue backorder (era una SQ SAP normal con stock disp). Root cause: v576 diseñó el modal como "cualquier línea (SAP+APP) que puede satisfacerse con stock actual" (semántica de allocation), pero el usuario esperaba "líneas que ESTUVIERON en BO y ahora tienen stock" (semántica de recuperación). Fix v606: `renderBackordersTab` FASE 3 filtra STOCK ASIG. a solo `c.source==='app' && c.state==='ASIG'`. `_updateBackorderButtonCounts` también actualizado — SAP-source ya no cuenta para STOCK ASIG. count (badge). Comportamiento BACKORDER (modo urgente) sin cambios: SAP SKUs sin stock + APP SKUs con state='BO'. **SW v605 previo (2026-08-24) — E5 fixes UX split-safe.** Auditoría de `.lines` en 30 sitios detectó 4 bugs UX cuando v600 splittea (2 líneas mismo SKU): (1) `sap-admin-panel.js:1403` "N SKUs" contaba líneas (mostraba "2 SKUs" en split) → cambiado a `new Set(...).size` para SKUs únicos; (2) `sap-admin-panel.js:1259-1261` mismo bug en tarjeta SAP → idem fix; (3) `seguimiento.js:1037` mismo bug en tarjeta seguimiento → idem fix; (4) `product-picker.js:162` stepper `curQty` mostraba solo qty de la primera línea (`find`) → cambiado a `.filter().reduce()` que suma ambas. Resto del codebase mayormente safe: `reduce((s,l) => s + l.qty)` maneja split correctamente. `sap-service-layer.js` filtra `state==='confirmed'` + dedup por ItemCode ya split-aware. **SW v604 previo (2026-08-24) — E4 Badge state por línea en Modal Revisar.** Para que el vendedor entienda visualmente qué va a SAP vs. qué queda en app cuando el flag v600 splittea un pedido en 2 líneas del mismo SKU. Cada línea del Modal Revisar (solo en modo `pending-confirm`) muestra badge de state: `→ SAP` (verde, va como SQ), `BACKORDER` (naranja, queda en app), `ASIG` (violeta, stock asignado), `FACTURADO`/`CANCELADO`/`RECICLADO` (gris). Además indicador `1/2, 2/2` cuando hay múltiples líneas del mismo SKU en el mismo pedido. Los borradores (`orders[]`) no muestran badge porque no tienen state hasta que se confirmen. Fallback: si `state` es null/undefined (pedidos legacy pre-v543), sin badge. **SW v603 previo (2026-08-24) — Cancel automático de SQ SAP al revertir/eliminar pedido.** Antes: al `Volver a Pendientes` o `Eliminar` un pedido confirmed que ya tenía SQ en SAP, la SQ quedaba huérfana en SAP (inflaba `backorderBySku` con demanda que ya no existía en la app). Bug reportado por Mariano: "si el cliente le agregó cosas de último minuto pero el pedido ya se envió a SAP". Fix: `sap-service-layer.cancelQuotation(docEntry)` nueva (POST `/Quotations({docEntry})/Cancel`); `volverAPendientes` y `eliminarPendiente` la llaman ANTES de tocar Firestore. Si SAP falla el cancel → aborta la operación (no split-brain). Solo aplica cuando `via != 'app_only'` (los pedidos 100% BO no crearon SQ). Whitelist WRITE de sapProxy CF: admin/gerente. **SW v602 previo (2026-08-24) — Controles anti-duplicación STRICT (refinados).** El detector v601 comparaba totales SKU-agregados → 156 SKUs "en overlap" (falsos positivos por demandas de clientes distintos con mismo SKU). v602 cambia a STRICT: solo alerta si un pedido-app tiene 2+ líneas del MISMO SKU con estados mixtos ({confirmed/invoiced} + {BO/ASIG}) — el único escenario donde hay duplicación real por unidades del split v600. `_diagBackorderOverlap` runtime itera `globalPedidos`. `window.diagBackorderOverlap(true)` incluye overlaps LOOSE por (cardCode, sku) para investigación manual. `scripts/audit_backorder_overlap.py` reporta STRICT + LOOSE separadamente. Baseline prod 2026-08-24: **0 STRICT, 7 LOOSE** (esperado con flag OFF). **SW v601 previo (2026-08-24) — Controles anti-duplicación backorder SAP+APP (detector v0.1, muy grueso).** Sumado a v600 (split de línea flag OFF por default), agrega detección de overlap entre las 2 fuentes de backorder: (1) `console.warn` automático en ambos listeners `stock_snapshot` y `stock_snapshot_app` cuando algún SKU aparece en ambas fuentes con qty>0; (2) fn global `window.diagBackorderOverlap(true)` para inspección manual desde consola (retorna array + tabla + detalle por cliente); (3) script Python `scripts/audit_backorder_overlap.py` que audita Firestore prod y reporta overlaps con lista de pedidos-app contribuyentes. Escenario que detecta: si por error existe una SQ SAP manual complementaria por el residual BO del split, la UI hoy sumaría (SAP + APP) duplicando. **SW v600 previo (2026-08-24) — E2/E3 split de línea BO al confirmar/editar pedido (feature flag OFF).** Fix del bug confirmado por Mariano 2026-08-24: pedido con qty=100 y disp=70 al confirmar quedaba 100% BO en vez de mandar 70 a SAP + 30 BO. Nueva política en `src/pure/pedido-split.js` (`splitPedidoLine` + `reenrichPedidoLine`, fn puras testeadas con 21 tests unit). Feature flag `window.isPedidoSplitEnabled()` lee `localStorage.split_enabled`. Con flag OFF: comportamiento legacy todo-o-nada (regresión cero). Con flag ON: pedido 100/70 → 2 líneas `{qty:70, state:'confirmed'} + {qty:30, state:'BO'}`. Prerequisito E1 (v600 mergeado 2026-08-24 PR #276) que asegura que `invoice-sync-core` skipea líneas BO/ASIG. Roadmap completo en `PLAN_BACKORDER_SPLIT.md` (E0-E8). **SW v581 previo (2026-08-21) — Modal Revisar (Pasar a Pendientes) usa `_revisionStockFor.disponible` (matches Lista de Espera) en vez de `hasStock` boolean.** Bug reportado por Mariano: LA LOMITA OUTDOORS SRL orden 78, la Lista de Espera mostraba Con Stock=$5.218k, Sin Stock=$14.839k; el modal Revisar tras "Pasar a Pendientes" mostraba Disponibles=$6.571k, Sin stock=$13.486k. Total match ($20.057k) pero split diferente por $1.353k. Root cause: `renderReviewLines` en `pedidos-modal.js:773` usaba `hasStock(sku)` (boolean simple, solo mira stock físico > 0) mientras Lista de Espera usa `_revisionStockFor(sku).disponible = stock - backorderSAP`. Items con backorder SAP que "reserva" el stock aparecían como Disponibles en Revisar. Fix: usar `_revisionStockFor(sku).disponible` con cap por qty (mismo split que hace la columna "Subtotal (Confirmación Inmediata)" del modal Pedido en Espera). Fallback al comportamiento viejo si `_revisionStockFor` no devuelve `hasData`. **v580 previo (2026-08-21) — Dashboard usa `importeLineasArsNeto` (sin IVA) en vez de `facturadoArsNeto` (con IVA) → matches PowerBI.** Bug reportado por Mariano: la app mostraba Santiago Esteban con $40.372.654 facturado y 202% cumplimiento; PowerBI mostraba $33.365.830 y 166.8%. Root cause: la app leía `facturadoArsNeto = SUM(doc_total)` de `v_facturas_sap` que **incluye IVA**. El target del vendedor ($20M) está definido sin IVA. Diferencia = 21% exacto = IVA argentino ($33.365.830 × 1.21 = $40.372.654,3). Fix `src/domains/dashboard.js`: cambiar los 3 usos activos (`_getSapSnapshotYtd`, ranking cards, card "SAP - Mes en curso") de `facturadoArsNeto` → `importeLineasArsNeto` (sale de `v_ventas_lineas.importe_linea_ars` que es sin IVA — mismo campo que usa PowerBI). Label del card actualizado: "importe líneas SAP sin IVA (matches PowerBI)". Los datos del snapshot ya estaban bien; solo cambia qué campo lee la app. Sin cambios en backend/sync. **v579 previo (2026-08-21) — "Archivo Cliente" y "Exportar todo" usan `_revisionStockFor(sku).disponible` (stock − backorder SAP) en vez de `getStockDisponibleVenta` (stock físico crudo).** Pedido de Mariano: si hay 10 en stock + 8 en Ofertas SAP → dispReal=2. Si el cliente pide 5, el Excel debe exportar `min(5, 2) = 2`, no `min(5, 10) = 5`. Bug: `stockDisp11(sku)` en ambos exportadores usaba `getStockDisponibleVenta(sku)` que solo devuelve `whs['11']` sin restar backorder SAP. Fix: cambiar por `_revisionStockFor(sku).disponible` que sí resta (misma lógica que la columna "Disponible SAP" amarilla del modal Pedido en Espera). Coincidencia garantizada entre lo que ve el vendedor en el modal y lo que sale en el Excel. Espera app (informativa) NO resta stock — no genera reserva SAP hasta que se envíe. **v578 previo (2026-08-21) — Solo pedidos con `transferidoSAP` seteado cuentan para BACKORDER/STOCK ASIG.** Bug reportado por Mariano: pedido CARNADAS LOBERIA (banner amarillo "Aún no transferido a SAP") aparecía en el modal BACKORDER con badge APP aunque nunca ingresó a SAP. Regla nueva: solo los pedidos procesados por el auto-send (con `p.transferidoSAP != null`) generan backorder — la lógica es que solo lo que ya pasó por el proceso de confirmar+enviar cuenta como demanda real que necesita reposición. Fix en 3 lugares: (1) `renderBackordersTab` (index.html:10426) skippea pedidos sin transferidoSAP. (2) CF E3 `aggregateForSku` (pedido-snapshot-core.js:107) skippea pedidos sin transferidoSAP → afecta `backorderByClientSkuApp` y por ende las secciones del modal cliente/menú rápido. (3) `rebuild_stock_snapshot_app.py` mismo filter para consistencia con la CF. Requiere **redeploy de la CF** `onPedidoWriteRecalcSnapshot` + rebuild del snapshot. Pedidos `via='app_only'` (100% BO, procesados pero no fueron a SAP porque no había líneas confirmed) SÍ cuentan — están procesados, solo que sin SQ SAP asociada. 1 test nuevo cubre el escenario. **v577 previo (2026-08-21) — Fix duplicate SQ en SAP (race condition auto-send con 2 admins concurrentes).** Bug reportado por Mariano: pedido LUIS JOSE ZYSMAN cargado por Ioannis creó **2 Ofertas en SAP** (#2000079 con 10 líneas + #2000080 con 24 líneas). Root cause identificado inspeccionando Firestore: `transferidoSAP.transferredBy='auto/jonatan.blanco@shimano.com.ar'` (no Ioannis) → 2 admins con listener `sap-auto-send-listener` activo. El lock stale timeout era 60s pero `sapSL.createQuotation` puede tardar más bajo carga → segunda sesión ve el lock como stale y también ejecuta. Fix v577 en `src/domains/sap-auto-send-listener.js`: (1) timeout stale lock aumentado de **60s → 300s (5 min)** para dar margen a operaciones SAP lentas. (2) DOUBLE-CHECK post-`createQuotation`: antes de escribir `transferidoSAP`, verificar en transacción si otra sesión ya lo escribió. Si sí (race real), NO overwrite, log ERROR con el docNum a borrar manualmente en SAP, alerta al admin via `showSyncTag`. Ambas mejoras cubren distintos escenarios: (1) previene el race más común (concurrent admins), (2) detecta y alerta cuando el race igual ocurre. **v576 previo (2026-08-21) — Modales BACKORDER / STOCK ASIG. con FIFO estricto + cap por dispSap.** Pedido crítico de Mariano tras identificar que STOCK ASIG. mostraba SQs enteras (ej: BROBRO 40u de 22CATPRII420) aunque hubiera solo 1u disp. Nueva lógica en `renderBackordersTab` (index.html:10330): fase 1 recolecta TODOS los candidatos (SAP + APP-BO + APP-ASIG) sin filter por stock. Fase 2 aplica cap FIFO por SKU: APP-ASIG consume dispSap primero (reservado por E4.5, no compite), luego competidores (SAP + APP-BO) ordenados por prioridad — durante transición SAP siempre gana a APP; dentro de cada source, fecha ascendente (más vieja primero). Cada cliente lleva `qtyAsignada` + `qtyBackorder`. Fase 3 filtra por modo: STOCK ASIG. muestra solo `qtyAsignada > 0` (con qty ajustado), BACKORDER muestra solo `qtyBackorder > 0`. SKUs vacíos post-cap se eliminan. Ahora el ejemplo BROBRO/22CATPRII420 muestra 1u en STOCK ASIG. (SQ #25938 más vieja) + 19u en BACKORDER de #25938 + 40u de #2000051. Documentación completa del flujo end-to-end agregada en README §8.bis.0. **v575 previo (2026-08-21) — Columna "Disponible SAP" del modal Pedido en Espera: amarillo pastel gana por encima de warnings de fila (`!important`).** Pedido explícito de Mariano. En v566 la política era "warnings ganan" (exceed/warn-quiebra/warn-duplicado/no-data pintaban toda la fila incluyendo la columna Disponible SAP). Ahora la columna Disponible SAP **siempre** se ve amarilla, sin importar el estado de la fila — es la columna clave que define la lógica de asignación y debe quedar visible siempre. Fix: mover la regla `#wcard-rows tr td.disp-sap-col` al final del bloque CSS (después de las warnings) + agregar `!important` al `background:#fef9c3`. **v574 previo (2026-08-21) — Export "Precios + Stock por SKU": columna Stock W11 muestra unidades numéricas en vez de "Disponible"/"Sin stock".** Pedido de Mariano. Fix en `src/domains/exports-core.js:449` (`fmtStock`): antes usaba `hasStock(sku)` (bool) y retornaba texto; ahora usa `getStockDisponibleVenta(sku)` que lee `STOCK_WAREHOUSE_BREAKDOWN[sku]['11']` y retorna número exacto. Retorna `''` (celda vacía) cuando no hay dato de stock (snapshot no cargado); `0` si el SKU no tiene stock. Los números permiten sort/filter/sum en Excel — no perdemos el estado "no dato" vs "0 unidades" gracias al `''`. Las 3 hojas del Excel (Precios y Stock, Stock) usan el mismo helper. La hoja Info sigue mostrando conteos de SKUs con/sin stock/sin dato (usa `hasStock` internamente que no cambió). **v573 previo (2026-08-21) — "Pasar a Pendientes" pasa TODOS los items (los BO viajan como `state='BO'` al modal BACKORDER, no quedan en Lista de Espera).** Cambio de política tras v572: ahora que el modal BACKORDER muestra líneas app-source con badge "📱 APP", el user prefiere que los items sin dispSap NO queden en la Lista de Espera — deben verse en BACKORDER. Fix: revertir el filter+update del v568 en `waitlistPasarAPendientes`. Todos los items del pedido en espera se pasan al pedido nuevo con qty exacta. `pedidos.add()` (línea 19803) marca cada línea con `state='confirmed'` si `dispSap >= qty`, sino `state='BO'`. Las líneas BO NO van a SAP (por E4C filter v544) y el circuito CF E3 las registra en `backorderByClientSkuApp` → aparecen en el modal BACKORDER con badge APP. El waitlist doc se borra entero al confirmar (`_pendingWaitlistDelete` siempre; `_pendingWaitlistUpdate` queda como dead code de safety). Historial de decisiones: v464 capaba → v507 pasaba todo → v568 volvía a filtrar con waitlist update → v573 vuelve a pasar todo pero ahora los BO son visibles vía modal BACKORDER (que antes de v572 no era el caso). **v572 previo (2026-08-21) — Modales BACKORDER / STOCK ASIG. unifican fuentes SAP + APP con badges de origen.** Mariano pidió ver ambas fuentes en la misma sección diferenciando con etiqueta. Fix: `renderBackordersTab` en `index.html:10330` ahora también itera `globalPedidos` filtrando líneas por `state='BO'` (modo Backorder/urgente) o `state='ASIG'` (modo STOCK ASIG./asignacion) con `qtyOpen > 0`. Cada cliente pusheado al `skuMap` lleva un campo nuevo `source: 'sap'|'app'`. En el render, cada fila lleva un badge visible: `🏢 SAP` (verde claro `#dcfce7/#166534`) o `📱 APP` (azul claro `#dbeafe/#1e40af`). Meta line muestra: SAP → `"SAP · OFERTA DE VENTA #NNNN · fecha"`; APP → `"APP · Orden N · fecha"`. Contadores de los botones (`backorders-btn-count`, `asignacion-backorder-btn-count`) y summary (SKUs / unidades / clientes) también consideran ambas fuentes con dedupe por SKU / cardCode. Los filtros existentes (búsqueda, mes) aplican a app-source igual. Sin cambios de backend — usa `globalPedidos` que ya está en memoria + `sapGetClienteCode` para resolver cardCode desde el nombre en pedidos legacy. **v571 previo (2026-08-20) — Swap default modal Revisar: "Dejar en Espera" ahora es primary; "Ir directo a Pendientes" es opción avanzada.** Bug UX reportado por Mariano: al terminar "Crear pedido" para COMERCIAL 4 NARICES, el user tocó el botón verde grande "PASAR A PENDIENTES" pensando que era el default → el pedido saltó Lista de Espera y fue directo a Pendientes. El flujo correcto según Mariano es SIEMPRE `Crear pedido → LISTA DE ESPERA → PENDIENTES → CONFIRMADOS`. Root cause: el botón "Dejar en Espera" existía (naranja, línea 2613) pero visualmente parecía la opción secundaria por su color naranja vs el verde primary del "Pasar a Pendientes". Fix v571: invertir estilos — "Dejar en Espera" pasa a `btn-confirm` primary (verde) sin override de color; "Ir directo a Pendientes" (renombrado desde "Pasar a Pendientes") pasa a `background:#f97316` (naranja secondary) con tooltip explícito de que salta la Lista de Espera. **v570 previo (2026-08-20) — Secciones "⏳ Backorder pendiente (app)" / "📱 Backorder asignado (app)" también en el `client-menu-modal` (menú rápido al tocar un cliente).** Post-v569 detectamos que el flujo real de los VDEs no pasa por el `client-modal` (Historial / Recomendados) — pasa por el `client-menu-modal` que es el menú de acciones rápidas que aparece al hacer click en un cliente (Última interacción / Ver últimas 3 compras / Ver recomendaciones / Crear pedido / Cargar pedido por Excel). Fix: agregar las 2 secciones (BO + ASIG) al `client-menu-modal` con la misma card style + nueva función `_renderClientMenuBoAndAsigApp(name)` espejo de la del `client-modal`. Re-render automático en el listener de `stock_snapshot_app` si el modal está abierto. Ahora vas a ver la sección roja arriba de las opciones del menú antes de tocar "Crear pedido". **v569 previo (2026-08-20) — Secciones "⏳ Backorder pendiente (app)" y "📱 Backorder asignado (app)" agregadas al modal cliente general.** Post-v567 detectamos gap: las secciones estaban solo en el modal "Pedido en Espera" que requiere doc de waitlist activo. Los 5 top clientes con backorder (incluyendo COMERCIAL 4 NARICES con 27 SKUs / 61 u) NO tenían waitlist → invisible para VDEs. Fix v569: agregar las 2 secciones (BO + ASIG) al `client-modal` (el que se abre con "Abrir cliente" desde el pin del mapa o card sidebar). Nueva función `_renderClientModalBoAndAsigApp(clientName)` que resuelve cardCode con `sapGetClienteCode(nombre)` y consulta `STOCK_BO_BY_CLIENT_APP` + `STOCK_ASIG_BY_CLIENT_APP` para popular las secciones. Solo visible si hay data para el cliente (`display:none` por default). Same card style que las del modal Pedido en Espera (roja pastel para BO, azul pastel para ASIG). Re-render automático en el listener de `stock_snapshot_app` si el modal está abierto cuando la CF muta datos. **v568 previo (2026-08-20) — "Pasar a Pendientes" respeta Disponible SAP (revierte política v507).** Bug reportado por Mariano: cliente DANIEL ALEJANDRO BATTISTONI orden 31, SKU `CLC70MHMXCB` con stock 5 + 7 backorder SAP → dispSap=0. En el modal Pedido en Espera el SKU aparecía correctamente con Subtotal $0 (respeta reserva SAP). Pero al tocar "Pasar a Pendientes", el modal Revisar agregaba el SKU con qty=1 y el subtotal saltaba $37.000 más. Root cause: `waitlistPasarAPendientes` (línea 14085) tenía comentario `v507: respetar EXACTO las qty del pedido` (política de 2026-08-13 que quitaba el cap por dispSap del v464). Fix v568: revertir política — items con `dispSap=0` NO se pasan al pedido nuevo (quedan en la lista de espera como reserva futura); items con `dispSap < qty` se cappean a `qty = dispSap`. Nueva var `_pendingWaitlistUpdate` (paralela a `_pendingWaitlistDelete`) para preservar los skipped items en el waitlist doc en vez de borrarlo. Confirm dialog explícito antes de continuar listando los N SKUs skipeados. Ahora los subtotales del modal Revisar coinciden con los de la lista de espera. **v567 previo (2026-08-20) — UI: nueva sección "⏳ Backorder pendiente (app)" en el modal cliente + CF E3 popula `backorderByClientSkuApp`.** Gap UX detectado post-v564: los pedidos-app con líneas `state='BO'` (esperando reposición de stock) no tenían representación visual en la app — solo se podían ver corriendo `scripts/list_bo_pedidos.py` o via Firestore Console. Fix en 2 partes: (1) **Backend** — `functions/core/pedido-snapshot-core.js` extiende `aggregateForSku` con nuevo `boByClient: Map<cardCode, qty>` y `writeSnapshot` popula la key nueva `backorderByClientSkuApp[cardCode::sku]` con stale cleanup paralelo al de `asigByClientSkuApp`. (2) **Frontend** — nuevo global `STOCK_BO_BY_CLIENT_APP` populado por el listener `stock_snapshot_app`, helper `window._e4bBoDelCliente(cardCode)`, función `_renderE4BBoApp(w)` que renderiza la sección debajo de "Backorder asignado (app)" — misma card style pero rojo pastel (`#fef2f2` + `#f87171`) y sin botones (no hay acción aplicable: solo esperar). Cuando entre stock del SKU, E4.5 FIFO promueve BO → ASIG y la fila se mueve automatico a la sección azul con los botones ✓/✗. Requiere **redeploy de la CF** `onPedidoWriteRecalcSnapshot` para que empiece a escribir el nuevo key. 2 tests unitarios nuevos (aggregateForSku boByClient + writeSnapshot backorderByClientSkuApp). **v566 previo (2026-08-20) — UX: columna "Disponible SAP" con fondo amarillo pastel en el modal Pedido en Espera.** Cambio visual pedido por Mariano para distinguir rápido la columna crítica que define la lógica de asignación (Stock total − Backorder SAP). Fix: (1) `<th>` inline `background:#fef9c3;color:#78350f`, (2) celdas `<td>` con clase nueva `.disp-sap-col` + regla CSS `#wcard-rows tr td.disp-sap-col{background:#fef9c3}` definida ANTES de las reglas de warning (exceed/warn-quiebra/warn-duplicado/no-data) para que las clases de fila ganen por orden de definición en filas problemáticas. Solo estética — no cambia comportamiento. **v565 previo (2026-08-20) — Cambio política "Archivo cliente": columna Pedido = `min(qty, dispSap)`.** El botón "ARCHIVO CLIENTE" del modal Pedido en Espera exportaba la columna "Pedido" con `it.qty` entero (política v547 que quería mostrar al cliente lo que pidió integro). Se cambia porque el archivo representa "lo que le vamos a entregar" — si el cliente pide 12 y hay 8 disponibles SAP, el Excel debe decir 8, no 12. Fix: `qtyExport = Math.min(qtyPedida, dispSap || 0)`; si `qtyExport <= 0` la fila se omite (nada para entregar). Subtotal recalculado con `qtyExport * precio`. Tooltip del botón actualizado para reflejar la nueva política. Hoja 2 (Confirmación Backorder) sin cambios. **v564 previo (2026-08-20) — HOTFIX BO/ASIG: `pedidos.add()` ahora popula `clientCardCode`.** Diagnóstico post-v560 (E4B habilitado a todos los roles): los 60 pedidos abiertos en prod NO tenían `clientCardCode` a nivel doc → el circuito `asigByClientSkuApp[cardCode::sku]` nunca matcheaba a ningún cliente → los VDEs no veían nunca la sección "Backorder asignado (app)" aunque las CFs corrieran automáticas. Root cause: `pedidos.add()` (index.html:19756) guardaba `clientName` (nombre) pero no `clientCardCode` — aunque la función `sapGetClienteCode(clientName)` (usada en `volverAListaEspera` línea ~20041 y otros lugares) resuelve el cardCode desde el nombre. Fix trivial: 5 líneas antes del `docData` que resuelven `_clientCardCode = sapGetClienteCode(parts[3])` y lo agregan al doc. Sin backfill de los 56 legacy sin cardCode (se resuelven cuando se facturen; agregar cardCode a un doc legacy no dispara el flow correctamente porque su `state='legacy'`). Post-v564: cualquier pedido NUEVO que arme un VDE para un cliente con match SAP entra al circuito automático — cuando entre stock del SKU, E4.5 FIFO promueve BO→ASIG y el próximo pedido del mismo cliente muestra los botones ✓ Aceptar / ✗ Rechazar. **v563 previo (2026-08-20) — HOTFIX Rendiciones sort: fallback Infinity para docs con `serverTimestamp()` pendiente.** Federico reportó que tras enviar una rendición tenía que refrescar la app para verla en "Mis rendiciones". Root cause: Firestore SDK aplica optimistic writes al cache local antes de que el server confirme el `serverTimestamp()` → durante esa ventana el doc nuevo tiene `createdAt=null` → sort desc con fallback `0` lo mandaba al **final** de la lista en vez del principio. Fix trivial: `misRendiciones.sort` y `todasRendiciones.sort` usan fallback `Infinity` cuando `createdAt` está pendiente → doc nuevo aparece arriba inmediato. **v562 previo (2026-08-20) — HOTFIX Rendiciones: baja compresion fotos ticket/adjunto 1400/0.78 -> 1000/0.7 (~200 KB) y agrega try/catch con alert claro.** Federico (VDE) reportó al enviar rendición: el sistema mostraba "espacio insuficiente" y la tab del PWA moría → volvía al home. Root cause: `compressImage(f, 1400, 0.78)` en `src/domains/rendiciones.js` (`onRendFotoTicket` y `onRendAttach`) decodifica el bitmap de una foto 12MP → ~48 MB de RAM temporal para el canvas + otro pico en `atob()` del upload a Storage → OOM en el WebView iOS/Android de gama modesta → kill del tab (el sistema mostraba "espacio insuficiente" y volvía al home). Precedente idéntico: **v388 (2026-08-04)** ya había bajado la compresión en Visitas Espacio de 800/0.7 → 500/0.5 por el mismo bug reportado por Federico (Firestore 1 MB limit). Rendiciones nunca se ajustó (usa Storage, no Firestore, pero el OOM del navegador ocurre antes del upload). Fix: baja a 1000/0.7 (OCR Gemini sigue funcionando bien con 1000 px de ancho, la foto queda ~200 KB) + try/catch alrededor de `compressImage` que muestra alert("No se pudo procesar la foto (posible falta de memoria)...") en vez del `console.warn` silencioso que dejaba al usuario sin feedback. **v561 previo (backend-only, sin bump SW) — E7 TTL 30 d ASIG.** Nueva CF scheduled `expireAsigLinesTTLCF` (03:00 America/Argentina/Buenos_Aires): query pedidos abiertos → filtra líneas `state='ASIG'` con `asigAt > 30 d` → mutación transaccional: `state='expired'`, `qtyExpired += qty`, `qtyOpen=0`, `expiredAt=now`. Cierra pedido con `closedReason='ttl_expired'` si todas las líneas cerradas. Audit doc en `asig_ttl_log/{isoTimestamp}` (lectura admin/gerente only). E3 recalcula `stock_snapshot_app` automático por el pedido write → asig sale → stock libre para el próximo tick E4.5 FIFO. 14 tests unitarios (cutoff, cierre pedido, edge cases). `inspect_shadow_logs.py` extendido con `show_asig_ttl_log`. **v560 previo — E4B habilitado para TODOS los roles (admin/gerente/vendedor/interno)**, sacamos el flag piloto. Renombrado UI "SAP Asignado - APP (piloto)" → "Backorder asignado (app)". Kill switch: `localStorage.setItem('e4b_disabled','1')`. **v559 previo — bugfix bootstrap listeners**: `ensureAllListeners()` reemplaza el cluster inline en `applyRolePermissions()` con try/catch por listener + hook `visibilitychange` para re-attach al volver del background (fix definitivo para el problema de `unsubStockSnapshotApp` que no arrancaba en el bootstrap si un ensure previo throwaba). **v554 previo (2026-08-19) — BUGFIX E3 writeSnapshot: dotted paths via .update() (evita reset del sub-map).** Bug 2 del piloto E4B: cada vez que un pedido de otro cliente tocaba SKUs sin cardCode, el `asigByClientSkuApp` quedaba pisado con `{}` — perdíamos las keys previas de clientes ASIG. Root cause: Firestore Admin SDK JS `set({field: {}}, {merge: true})` con objeto vacío REEMPLAZA el field entero, NO es no-op como esperaba. Fix: writeSnapshot hace 2 escrituras: (1) `set({sourceApp, updatedAtApp}, {merge: true})` para garantizar doc existe, (2) `update({dotted.paths: value})` — update() SÍ interpreta dotted paths como nested (a diferencia de set). Los SKUs NO tocados en el recalc actual quedan intactos. Bonus: la corrida de bugs consecutivos v551/v553/v554 muestra los 3 gotchas del Firestore Admin JS que fue necesario descubrir empíricamente. Backend-only bugfix. **v553 previo — hotfix E4B step 2: CF invocation con region explicita** (firebase.functions() default us-central1, hay que usar `firebase.app().functions('southamerica-east1')`). **v552 previo — SECURITY: Gemini API key movida de Firestore a Secret Manager.** Bug: al probar los botones ✓ Aceptar / ✗ Rechazar del piloto salia `Error ejecutando RECICLAR: internal` + preflight OPTIONS 404. Root cause: `_e4bHandleAsigAction` invocaba `firebase.functions().httpsCallable('updateAsigLineStateCF')` que usa region default `us-central1`, pero la CF esta en `southamerica-east1` → 404 no existe. Fix trivial: cambiar a `firebase.app().functions('southamerica-east1').httpsCallable(...)` — mismo pattern usado en `src/domains/rendiciones.js:47`. Sin cambios backend. Piloto ahora funciona end-to-end. **v552 previo — SECURITY: Gemini API key movida de Firestore a Secret Manager.** Antes: `app_config/gemini.apiKey` era legible por cualquier `isReader()` @shimano user → cualquier VDE con DevTools exfiltraba la key y quemaba credito Gemini a costa de la cuenta. Ahora vive en Secret Manager, expuesta solo al Cloud Function `geminiOcrProxy`. Cambios: (1) `functions/core/gemini-ocr-core.js` nuevo módulo puro `runGeminiOcr(deps, auth, {imageBase64, mimeType})` con auth check (@shimano only), input validation (MIME whitelist, size limit 10MB, base64 puro sin data: prefix), timeout 45s, error mapping a `HttpsError` codes. (2) `functions/index.js` nuevo callable `geminiOcrProxy` region southamerica-east1 con `defineSecret('GEMINI_API_KEY')`. (3) `src/domains/rendiciones.js` `extractTicketDataWithGemini` reescrito para llamar `firebase.app().functions('southamerica-east1').httpsCallable('geminiOcrProxy')` en vez de fetch directo. `getGeminiApiKey` + `geminiApiKeyCache` eliminados (dead code). Timeout client-side 60s como red final. (4) `src/domains/admin-users.js` `renderGeminiConfigSection` reemplazado por instrucciones CLI (Secret Manager); `saveGeminiApiKey`/`deleteGeminiApiKey` eliminados. (5) `firestore.rules` `app_config/gemini` movido a lista restrictiva (admin/gerente only read). (6) CSP `connect-src` sin `generativelanguage.googleapis.com` (no llamamos directo). (7) `tests/functions/gemini-ocr.test.js` 19 tests (auth + config + input validation + fetch + happy path). (8) rules tests: 3 casos nuevos para app_config/gemini. **Deploy manual post-merge**: `firebase functions:secrets:set GEMINI_API_KEY` (pegar key actual desde Firestore), `firebase deploy --only functions:geminiOcrProxy,firestore:rules`, después borrar campo `apiKey` del doc `app_config/gemini` en Console Firebase. **v551 previo (2026-08-19) — BUGFIX E5 UI merge: E3 escribe a doc separado stock_snapshot_app.** Bug descubierto durante test del piloto E4B: `sync_sap_to_firestore.py` (cron externo cada 30 min) hace `SET` full replace sobre `app_config/stock_snapshot`, PISANDO los keys `backorderBySkuApp`/`asigBySkuApp`/`asigByClientSkuApp` que E3 (CF `onPedidoWriteRecalcSnapshot`) escribe con merge:true. Cada 30 min los keys app-source desaparecen → UI `STOCK_ASIG_BY_CLIENT_APP` queda vacío → piloto no muestra nada. Fix: E3 en active mode ahora escribe a `app_config/stock_snapshot_app` (doc separado que solo E3 toca). Nuevo listener frontend `unsubStockSnapshotApp` en `ensureStockSnapshotAppListener()` popula los mismos globals STOCK_*_APP. Rules ya cubren `stock_snapshot_app` con `isReader()` (fallback de la regla existente). CF E3 requiere redeploy. **Data migration post-deploy**: al primer write a un pedido, E3 popula `stock_snapshot_app`; hasta entonces los globals quedan vacíos. **v550 previo — SECURITY: vendedores ven SOLO clientes asignados explícitamente.** Antes: cualquier `isReader()` leía TODOS los docs de `client_applications` (VDEs veían altas SAP huérfanas + leads de otros vendedores). Fix multi-capa: (1) `firestore.rules` `client_applications.read` restringido: VDE ve solo si `assignedVendor==myVendorKey()` (helper nuevo que lee `roles/{uid}.vendor`) O es owner (`ownerUid==auth.uid`, necesario para tab Mis solicitudes). Admin/gerente/viewer/interno backward compat. (2) `ensureApprovedAltasListener` (index.html:4676) role-aware: para VDE agrega `.where('assignedVendor','==',assignedVendor)` server-side. Sin `assignedVendor` no crea listener. (3) 6 filtros frontend con bug `if (SET && a.assignedVendor && !SET.has(a.assignedVendor))` corregidos a `if (SET) { if (!a.assignedVendor) return; if (!SET.has(a.assignedVendor)) return; }` — antes colaba altas huérfanas a VDEs. Sitios: `drawSapAltaPins`, `filteredPoints` altas check, 4× `sapLocs`/`updateStats`/`renderClients`, `_leadsFiltered`, visitas. (4) Composite index `(status, assignedVendor)` en `firestore.indexes.json`. (5) Tests rules: 5 casos nuevos (VDE ve su asignado / no ve otro / no ve huérfano / SÍ ve su creación / admin+gerente+interno ven huérfano). **INTERNO (VDI) queda con rule laxa** — frontend filtra por partner-vendors pero DevTools podría leakear; full enforcement server-side requiere denormalizar partner list en `roles/{uid}` — TODO post-v550. **Deploy manual requerido post-merge**: `firebase deploy --only firestore:rules,firestore:indexes` (afuera del PR de código). **v549 previo (2026-08-19) — E4B step 2 botones Aceptar/Rechazar (task #20).** Piloto E4B ahora tiene 2 botones por fila: **✓ Aceptar** (reciclar) y **✗ Rechazar** (cancelar). Nueva Cloud Function callable `updateAsigLineStateCF` en `functions/core/asig-recycle-core.js` — muta linea `state='ASIG'` de un pedido-app a `'recycled'` o `'cancelled'` transaccionalmente. Auth: solo `@shimano.com.ar` o `@shimano.uy`. Idempotencia: valida `state='ASIG'` antes de mutar (rerun con misma linea ya cambiada tira error). Frontend handler `_e4bHandleAsigAction`: prompt qty (default = qty pendiente), fetch pedidos del cliente con lineas ASIG del sku, FIFO oldest-first, llama al CF por cada linea hasta consumir la qty pedida. Post-recycle: agrega automaticamente el SKU al waitlist actual via `waitlistAddProductWithQty(w._id, sku, totalApplied)` — un solo click cierra el flow. Mutaciones que llevan a `qtyOpen=0` marcan `state=recycled/cancelled`; parciales quedan en `state='ASIG'` para siguiente iteracion. Cuando todas las lineas cierran, el pedido source se cierra con `closedReason='all_recycled_or_cancelled_or_invoiced'`. 18 tests nuevos (auth + validation + not-found + recycle parcial/total + cancel parcial/total + rerun idempotencia + multi-lineas). Total 102 tests functions. Deploy CF pendiente + merge frontend. **v548 previo — E5 UI merge SAP+app-source (task #19).** Frontend consume las keys nuevas `backorderBySkuApp`/`asigBySkuApp`/`asigByClientSkuApp` que E3 escribe en `stock_snapshot` (post-cutover mode='active'). Nuevos globals `STOCK_BACKORDER_APP`, `STOCK_ASIG_APP`, `STOCK_ASIG_BY_CLIENT_APP`. Alert modal muestra desglose "SAP N + APP M". **Cutover ejecutado 2026-08-19 13:20:57 UTC** — mode='active'. **v547 previo**: fix "Archivo cliente" columna. **v546 previo**: primera version boton Archivo cliente. Frontend consume las keys nuevas `backorderBySkuApp`/`asigBySkuApp`/`asigByClientSkuApp` que E3 escribe en `stock_snapshot` (post-cutover mode='active'). Nuevos globals `STOCK_BACKORDER_APP`, `STOCK_ASIG_APP`, `STOCK_ASIG_BY_CLIENT_APP` populados del listener `unsubStockSnapshot`. `_revisionStockFor(sku)` ahora devuelve `backorderApp` y `asigApp` adicionales. **Merge logic**: por E4C filter (state='BO'/'ASIG' nunca van a SAP), las fuentes son disjuntas y el merge es suma directa sin dedup. Alert modal Pedido en Espera muestra desglose "SAP N + APP M" cuando hay data de ambas fuentes. E4B piloto simplificado: ahora lee sync del global `STOCK_ASIG_BY_CLIENT_APP` en vez de fetch on-demand a shadow_v3 — más rapido y evita el fallback complejo. `disponible` sigue restando solo SAP-BO (app-ASIG es "phantom reserve" hasta que se recicle en E4B step 2). **v547 previo — E2 active + toggle E5 cutover (backend, sin bump SW).** E5 cutover prep del plan BO/ASIG: implementado `applyInvoiceMatch(deps, match)` en `functions/core/invoice-sync-core.js` — cuando `mode='active'`, para cada match Invoice→pedido incrementa `pedido.lines[i].qtyInvoiced += qty`, recalcula `qtyOpen = qty - qtyInvoiced - qtyCancelled - qtyRecycled`, marca `state='invoiced'` si `qtyOpen<=0`, y cierra el pedido (`closedAt`+`closedReason='all_invoiced'`) si todas las lineas quedaron cerradas. **Idempotencia**: cada pedido lleva `sapLinkage.appliedInvoiceDocEntries[]` — rerun con mismo `invoiceDocEntry` retorna null (skip). Multi-lineas SAP con mismo itemCode suman antes de aplicar. Errores individuales van a `errors[]` sin abortar el batch. Nuevo `scripts/e5_toggle.py` con subcomandos `status`/`preflight`/`activate`/`rollback` para flipear `app_config/sap_sync_state.mode` con confirmacion typed ("CUTOVER"/"ROLLBACK"). Preflight chequea (a) E2 corrio ultimos 30 min, (b) 0 errores en ultimos 5 `sap_sync_log`, (c) shadow snapshot `updatedAtApp <24h`, (d) sin orphan flat keys. 8 tests nuevos para `applyInvoiceMatch` (total 84 tests functions). Fix `scripts/rebuild_shadow_snapshot_v3.py`: delete de flat keys requiere backticks para escape de dots en field paths (`` `field.with.dots` ``). **Cutover ejecutado 2026-08-19 13:20:57 UTC**: `mode='active'`. **v547 previo**: fix "Archivo cliente": columna correcta = "Pedido" (no "Disponible SAP"). E5 cutover prep del plan BO/ASIG: implementado `applyInvoiceMatch(deps, match)` en `functions/core/invoice-sync-core.js` — cuando `mode='active'`, para cada match Invoice→pedido incrementa `pedido.lines[i].qtyInvoiced += qty`, recalcula `qtyOpen = qty - qtyInvoiced - qtyCancelled - qtyRecycled`, marca `state='invoiced'` si `qtyOpen<=0`, y cierra el pedido (`closedAt`+`closedReason='all_invoiced'`) si todas las lineas quedaron cerradas. **Idempotencia**: cada pedido lleva `sapLinkage.appliedInvoiceDocEntries[]` — rerun con mismo `invoiceDocEntry` retorna null (skip). Multi-lineas SAP con mismo itemCode suman antes de aplicar. Errores individuales van a `errors[]` sin abortar el batch. Nuevo `scripts/e5_toggle.py` con subcomandos `status`/`preflight`/`activate`/`rollback` para flipear `app_config/sap_sync_state.mode` con confirmacion typed ("CUTOVER"/"ROLLBACK"). Preflight chequea (a) E2 corrio ultimos 30 min, (b) 0 errores en ultimos 5 `sap_sync_log`, (c) shadow snapshot `updatedAtApp <24h`, (d) sin orphan flat keys. 8 tests nuevos para `applyInvoiceMatch` (total 84 tests functions). Fix `scripts/rebuild_shadow_snapshot_v3.py`: delete de flat keys requiere backticks para escape de dots en field paths (`` `field.with.dots` ``). Preflight actual: **PASS PASS PASS PASS**. Cutover pendiente autorizacion Mariano — deploy CF `syncSapInvoicesToApp` y flipear flag. **v547 previo**: fix "Archivo cliente": columna correcta = "Pedido" (no "Disponible SAP"). El Excel del botón "Archivo cliente" ahora usa la columna "Pedido" (`it.qty`, el valor del input editable del modal) en Hoja 1 y "Pedido" (`pend` de la SQ open) en Hoja 2. Subtotales en Hoja 1 = `qty × precio` sin cap por dispSap (para el cliente, mostrar el valor total pedido, no lo confirmable inmediato). Filename `ArchivoCliente_<cliente>_orden<N>.xlsx`. **v546 previo**: primera versión del botón usaba "Disponible SAP" por error de interpretación — corregido en v547. **v545 previo (2026-08-19) — E4B step 1 + fix E3 dotted-paths + inspector shadow.** **BUG FIX 2026-08-19**: `functions/core/pedido-snapshot-core.js:writeSnapshot` escribia `set({"backorderBySkuApp.SKU1": 0, ...}, {merge:true})` — el Admin SDK JS trata dotted paths como nombres literales de campo (NO anida). Consecuencia: el doc `stock_snapshot_shadow_v3` quedaba con claves top-level tipo `backorderBySkuApp.SHMTECHLBLUEXL: 0` y el reader `d.backorderBySkuApp` devolvia undefined. Fix: armar nested map (`{backorderBySkuApp: {SKU1: 0, ...}, asigBySkuApp: {...}, asigByClientSkuApp: {"C001::X": 3}, ...}`) y `set(..., {merge:true})` hace deep merge. Tests actualizados. Las claves flat quedan orfanas en el doc — no dañan (lectores las ignoran) y se sobreescriben progresivamente segun VDEs escriben pedidos. Nuevo script `scripts/inspect_shadow_logs.py` que resume estado de sap_sync_state + sap_sync_log (E2) + stock_snapshot_shadow_v3 (E3) + stock_snapshot prod (trigger E4.5) + stock_assignment_log_shadow (E4.5). Corrida diagnostico revelo: E2 corre cada 15 min con 0 matches / 18-20 orphans (esperable — pedidos historicos ya facturados antes del cursor), E3 escribio 40 SKUs pero con bug flat-key (fix arriba), E4.5 disparo 12:21 con `skusChecked:0` (no hubo deltas positivos en dep 11 en la ventana). **v545 previo (mismo tag)**: E4B step 1 (display-only piloto). E4B del plan BO/ASIG: modal "Pedido en Espera" ahora tiene una tercera seccion "SAP Asignado - APP (piloto)" que muestra las lineas del cliente con `state='ASIG'` desde app-source (`stock_snapshot_shadow_v3.asigByClientSkuApp[cardCode::sku]` en shadow, cae a `stock_snapshot.asigByClientSkuApp` en cutover E5). Feature flag **`localStorage.setItem('e4b_enabled','1')` + reload** — gateado adicionalmente por role admin/gerente (rules impiden el read a vendedores). Fetch on-demand al abrir el modal (`openWaitlistCardModal`), re-render cuando llega el snapshot. Sin botones aun — step 2 sumara Aceptar (reciclar linea a nuevo pedido a precio actual) y Rechazar (liberar stock para siguiente FIFO). Nuevos helpers `window.isE4BEnabled`, `window._e4bFetchAsigApp`, `window._e4bAsigDelCliente`, `window._E4B_ASIG_CACHE`. Nueva funcion `_renderE4BAsigApp(w)` (index.html) llamada desde `_renderSapAsignadoConStock` post-render de las 2 secciones YA/NUEVOS existentes. **v544 previo**: E4.5 CF backend-only. E4.5 del plan BO/ASIG: nueva Cloud Function `onStockChangeFIFOAssign` (trigger on-write `app_config/stock_snapshot`, `southamerica-east1`, 512MiB, 300s). Cuando `sync_sap_to_firestore.py` actualiza `warehouseBreakdown` (cada 30 min), detecta SKUs con delta positivo en dep 11 y corre FIFO estricto (decisión Q9: primeros pedidos se llevan la línea COMPLETA; si no cabe la próxima línea, corte, NO promoción parcial) sobre pedidos-app con `closedAt==null` y `lines[i].state='BO'`, ordenados por `createdAt` ascendente (ties por `pedidoId`). En modo `'shadow'` (default por `app_config/sap_sync_state.mode`) escribe audit doc a `stock_assignment_log_shadow/{isoTimestamp}` **sin** tocar pedidos. En modo `'active'` (E5 cutover) mutará `pedidos.lines[i]={state:'ASIG', asigAt: nowIso}` y logueará a `stock_assignment_log`. Nuevo módulo `functions/core/fifo-assign-core.js` (~230 LOC): `extractSkusWithStockIncrease`, `loadBoCandidatesForSku`, `computeAssignmentsFifo`, `applyAssignments`, `runFifoAssign`. 16 tests unitarios (5 extract + 4 FIFO estricto + 3 load + 3 runFifoAssign shadow/active/no-deltas + 1 audit log). Rules actualizadas: `stock_assignment_log` y `stock_assignment_log_shadow` lectura admin/gerente. Backend-only (sin cambios frontend) → no bump SW. Deploy pendiente autorización explícita del usuario. **v544 previo**: E4C: `sap-auto-send-listener` ya NO envía a SAP líneas `state='BO'`. `buildQuotationPayload` (src/domains/sap-service-layer.js:400) filtra `p.lines` por `l.state==='confirmed'` antes de mapear a `DocumentLines`. Pedidos legacy (state undefined/'legacy') se envían enteros (compat 56 pedidos E1). Si el pedido queda 100% BO tras filtrar, listener marca `transferidoSAP={via:'app_only', reason:'all_lines_bo', docEntry:null}` y no llama a SAP — cuando entre stock (E4.5 FIFO) y el cliente recicle en pedido nuevo (E4B), ese pedido nuevo va con state='confirmed'. Cumple decisión Q7 (solo líneas con stock van a SAP). Zero cambio UX. **v543 previo**: E2+E3 shadow deployables + hybrid mode. E4 parte A: `pedidos.add()` (index.html:19285) y `persistPendingEntry` (14570) ahora asignan `state='confirmed'` a las líneas con `getStockDisponibleVenta(code) >= qty`, y `state='BO'` a las sin stock. `persistPendingEntry` respeta estados "avanzados" (ASIG/invoiced/cancelled/recycled) — no downgrade. `qtyOpen` se recalcula = `qty - qtyInvoiced - qtyCancelled - qtyRecycled`. Con este cambio, los pedidos nuevos empiezan a poblar `app-source` para que la CF `onPedidoWriteRecalcSnapshot` de E3 pueda agregar datos reales (los 56 pedidos históricos siguen con `state='legacy'` porque se migraron pre-E4A, y se ignoran hasta cutover). Rules deployadas a prod 2026-08-18. Deploy de CFs pendiente (comando powershell reintentar). **v542 previo**: alerta anti-duplicado al agregar SKU. E2+E3 armadas + hybrid mode: Santi aprobó el plan pero pidió NO apagar la lectura SAP-SQ actual (`sync_sap_to_firestore.py:571-642`) porque puede haber SQs cargadas fuera de la app. Diseño hybrid: fuente SAP-SQ sigue viva escribiendo `backorderBySku` como hoy; nueva CF E3 `onPedidoWriteRecalcSnapshot` (trigger on-write pedidos/{id}) recalcula keys paralelos `backorderBySkuApp`, `asigBySkuApp`, `asigByClientSkuApp`. Read side hará merge con dedup por `transferidoSAP.docEntry` ⇔ `SQ.DocEntry`. **E3 modo shadow**: escribe a `app_config/stock_snapshot_shadow_v3` (no toca prod snapshot). **E3 modo active** (E5): a `app_config/stock_snapshot` sin pisar `backorderBySku`. Modo controlado por `app_config/sap_sync_state.mode` (misma flag que E2). 15 tests para pedido-snapshot-core cubren aggregation, cliente/SKU keys, exclusión de state=legacy/confirmed, y garantía shadow no toca stock_snapshot. Rules: `stock_snapshot_shadow_v3` admin/gerente. Deploy pendiente autorización explícita. E2 del plan BO/ASIG: nueva Cloud Function `syncSapInvoicesToApp` (scheduled cada 15 min, `southamerica-east1`) que en SHADOW MODE lee Invoices SAP, resuelve lineage `Invoice.Line.BaseType=17 → SO.DocEntry → SO.Line.BaseType=23 → SQ.DocEntry` y matchea con pedidos-app por `transferidoSAP.docEntry`. NO modifica pedidos — escribe log a `sap_sync_log/{isoTimestamp}` con `{matches: [{invoiceDocEntry, sqDocEntry, pedidoAppId, lines: [{itemCode, qty}]}], orphans: [{reason: no_lineage_to_sq | no_pedido_match | multi_pedido_match | so_fetch_failed}], cursorBefore, cursorAfter, mode}`. Cursor en `app_config/sap_sync_state.lastInvoiceDocEntry`. Modo `'shadow'|'active'` — cambia a `active` en E5 cutover (por ahora TODO). Nuevo module `functions/core/sap-sl-client.js` con login/get/logout reutilizable (extraído del pattern sap-proxy). Tests: 8 tests para syncSapInvoices cubren cursor, lineage completo, orphans (no_lineage, no_match, multi_match) + garantía que shadow mode nunca escribe a pedidos. Rules actualizadas: `app_config/sap_sync_state` admin/gerente only; nueva collection `sap_sync_log` lectura admin/gerente. Requiere deploy manual: `firebase deploy --only functions:syncSapInvoicesToApp` cuando querramos activar (E0 buy-in aún pendiente). Alerta anti-duplicado en agregar SKU: cuando el VDE agrega un SKU al pedido (desde `waitlistAddProduct` búsqueda manual o desde `addToOrder` modal Review), si el cliente ya tiene ese SKU asignado (backorder SAP con stock disponible del mes anterior o antes), se muestra confirm() "⚠️ DUPLICADO POSIBLE — cliente ya tenía X unidades de SKU asignadas (pedido del dd/mm, SQ #NNNN). Aceptar=agregar adicional / Cancelar=usar solo lo previo". `waitlistAddProductWithQty` (path desde botón "+ Agregar" del SAP Asignado modal) NO tiene el check porque es exactamente el flow "quiero recuperar lo asignado". Helper `window._sapAsigMatchParaCliente(cardCode, sku)` + `window._confirmDuplicadoAsig(match)` expuestos desde inline para el bundle. Fail-open: si la validación falla (contexto sin cliente, sapGetClienteCode ausente), no bloquea. **v541 previo**: E1 del plan BO/ASIG desde app: schema pedidos backfilleado + `pedidos.add()` incluye campos nuevos (`qtyOpen`, `qtyInvoiced`, `qtyCancelled`, `qtyRecycled`, `state='legacy'`, `asigAt`, `recycledIntoPedidoId`, `priceAtCreation` por línea; `schemaVersion=2`, `sapLinkage`, `closedAt`, `closedReason` a nivel doc). Migración script `scripts/e1_backfill_schema.py` (dry-run default, --apply para aplicar). Aplicado a prod: 56 pedidos migrados. Zero cambios de comportamiento — `state='legacy'` marca que la lógica nueva no interpreta estos pedidos hasta E5 cutover. Plan completo en respuesta al usuario (16 decisiones + 12 casos de uso). Próximo paso: E0 (buy-in Santi/Ioannis) en paralelo con E2 (Cloud Functions sync SAP Invoices en shadow). **v540 previo**: Botón "Borrar de espera" del modal Pedido en Espera ahora visible para cualquier @shimano.com.ar/@shimano.uy (antes: solo owner del doc + admin/gerente). Motivo: los VDE no eran owner (el VDI crea el doc al cargar el Excel del vendedor) → si el VDE se equivocaba, no podía borrar su propio pedido y dependía del admin. Rules Firestore v514 ya permitían delete a cualquier isReader; solo faltaba abrir el gate del frontend. **v539 previo**: timeout Gemini OCR (45s) + Firebase Storage upload (60s) en rendiciones. Fix rendiciones: agregado timeout 45s al fetch de Gemini OCR y timeout 60s al upload Firebase Storage. Antes: si Gemini colgaba o Storage no respondía, el spinner "Analizando ticket con IA..." o el botón "Subiendo foto..." quedaban pegados infinitos y el vendedor no podía cargar la rendición. Reportado por VDE. **v538 previo**: ciclo campañas dashboard cerrado — Card "Campañas activas" del Dashboard muestra facturado REAL SAP match 1:1 vs Power BI (ej. CATANA GONZALO $5.590.803 / $8.500.000 = 66%, chip azul **SAP**). Ciclo v532→v538 completo: **(v532)** nuevo snapshot Firestore `campania_snapshot` alimentado por `sync_campania_snapshot_to_firestore()` desde BQ `v_campanias_progreso` cada 15 min; frontend leaseholder `window.CAMPANIA_SNAPSHOT` + fallback a `globalPedidos` con badge "pedidos app"/"SAP". **(v533)** optimistic update en `waitlistAddProduct` + `waitlistRemoveItem` + buscador stays open al agregar SKU (no cierra dropdown). **(v534 CRITICAL)** race condition en `waitlistUpdateItemQty` fix: mutar cache local antes del await Firestore. **(v535)** dropdown buscador Pedido en Espera muestra 5 mini-badges por SKU (DISP dep 11 / TRAN dep 12 / ASIG min(pendSAP,disp) / BO max(pendSAP-disp,0) / ESPERA suma revision_waitlist). **(v536)** alert STOCK modal Productos idem 5 métricas. **(v537)** eliminada card SAP - Acumulado anual del Dashboard. **(v538)** fix bug diagnostico: listener `campania_snapshot` no se activaba (UNSUB INACTIVO) porque `ensureListeners` corría antes de `fbDb/currentUser` listos → llamada idempotente adicional en `openDashboardModal()` + log `[campania_snapshot] listener OK - N docs`. Firestore rules `campania_snapshot` (allow read isReader) deployadas manual con `firebase deploy --only firestore:rules`. Ciclo v512→v524 (2026-08-13) en 4 áreas: **(A) BQ Power BI**: mapa por departamento con `v_leads_detalle` + tabla `geo_localidad_departamento` + fixes CABA (`prov_depto` cobertura 606/698 → 688/699 = 98.4%). **(B) Template Excel Shimano oficial**: v511 banda BACKORDER + autofilter PEDIDO; v521 filtra solo SKUs con stock; v522/v524 nuevo botón "Exportar Stock Asignado" en modal card Pedido en Espera que exporta el backorder SAP del cliente con stock disponible; v523 rename botones + oculta filtro stock. **(C) UX Revisión / Lista de Espera abierta a todos los VDEs** (v514 rules Firestore + v517 gate de acceso por email @shimano + v518-v519 dropdown Forma de entrega con campos condicionales prellenados al Pasar a Pendientes + v520 botón "Dejar en Espera" en modal Revisar para el flow SKU-by-SKU manual). **(D)** Frecuencia sync: stock **5 min**, BQ **15 min** (v508). |
...
- **Toggle real** en el header top-right (SVG luna/sol dinámico, `.logout-btn` styling).
- **Storage precedence:** localStorage `shimano_theme` > Firestore `userData/{uid}.themePreference` > OS `prefers-color-scheme`.
- **FOUC prevention:** init inline `<script>` en `<head>` ANTES de `<link>`/`<style>` — setea `data-theme` en `<html>` antes de renderear body.
- **Cross-device sync:** el toggle sincroniza a Firestore fire-and-forget; el listener `userDataRef.onSnapshot` aplica preferencia remota vía `applyThemeFromUserData()` solo si localStorage local está vacío.

**Implementación (14 versiones):**
- **v736:** placeholder solo (botón + modal "En construcción")
- **v738:** infra (CSS vars + toggle real + FOUC prevention). Coverage ~60%.
- **v740-v742:** iterations de contraste con class-based overrides `[data-theme="dark"] .class`. Coverage → 95%.
...
- Leaflet map: swap dinámico entre OSM (light) y CartoDB Dark Matter (dark).
- Charts del Dashboard: colores hardcoded (bar chart azul cyan `#0284a0`) sin adaptar a tema — trade-off, priority baja.

**Tooling reusable:**
- `scripts/dark_mode_codemod.py` — property-aware hex → var() migration. Modes `dry-run` y `--apply`.
- `scripts/dark_mode_contrast_audit.py` — WCAG 2.1 AA audit programático. Output text o `--json`.

**Trade-offs aceptados (WCAG fails intencionales):**
- `brand-shimano-blue` (`#00A9E0`) sobre blanco = 2.71 (fail). Brand color de Shimano, inmutable.
- `--border-default` sobre `--bg-base` = 1.48 (light) / 1.72 (dark). Los borders son intencionalmente sutiles (dividers informativos, no controles críticos).
- `--text-disabled` sobre `--bg-base` = 2.56. Los disabled DEBEN verse muted por design.

**Cómo agregar theme-awareness a código nuevo:**
1. Preferir CSS vars sobre hex hardcoded. Consultar tabla en `PLAN_DARK_MODE.md` §2.
...
**Docs: README §46 reescrita como cierre del trabajo Bike Pipeline + tabla completa de vistas BQ para BO/ASIG (desde app).**

- La §46 vieja ("Handoff Bike Pipeline — retomar acá") era una lista de pendientes. Todos resueltos: la reescribo como cierre.
- Nueva §46.1: tabla con **todas** las vistas BQ disponibles agrupadas por tema (Inventario Bike / Inventario Pesca / Backorder+ASIG desde app / Entradas stock).
- Nueva §46.2: naming convention completa de columnas de costo por tabla, resuelta en v794 sin ambigüedad. Aclara que `cost_usd` en Bike ≠ `cost_list11_usd` en Pesca — salen de listas SAP distintas (7 vs 11).
- Nueva §46.3: decisiones ancladas por COWORK (whitelist WH 10, currency por fila, clasificacion_abc no es categoría, etc).
- Nueva §46.4: dato para Compras (TC lista 11 desactualizado, inventario Bike subvaluado ~9.9%).
- Nueva §46.5: mapa de archivos clave con líneas de referencia.
- Nueva §46.6: workaround PowerShell 5.1 para deployar SQL (UTF-8 sin BOM).
- Bump `APP_VERSION` + `CACHE_VERSION` v794 → v795. Bundle sin cambios.
...
## 46) Bike Pipeline BQ + BO/ASIG desde app — CERRADO 2026-09-04

**Estado**: CERRADO. Todas las decisiones ancladas por COWORK. Última versión: **v794**.

### 46.1 Vistas BigQuery disponibles

#### Inventario Bike (nuevo, v777-v794)

| Vista | Filas típicas | Fuente | Uso |
|---|---|---|---|
| `sap_items_bike_raw` | ~9.896 items grupo 100 | SAP SL `/Items` (sync 30min) | Tabla base Bike |
| `v_inventario_bike` | 1 fila por item | `sap_items_bike_raw` | Cards KPI + tabla resumen Power BI |
| `v_inventario_bike_por_warehouse` | 1 fila por item × warehouse | `sap_items_bike_raw` con JSON_KEYS | Breakdown por depósito |

Archivo: `bigquery/bike.sql`.

#### Inventario Pesca (existente, v794 fix costo)

| Vista/Tabla | Filas típicas | Notas |
|---|---|---|
| `sap_items_raw` | 773 items grupo 102 | Post-v794: `cost_avg_ars` (Currency=ARS, 2 items) + `cost_list11_usd` (Currency=USD, ~771 items) |
| `v_inventario` | 773 items Pesca | Consumida por tablero Pesca en producción |

#### Backorder / Stock Asignado — desde la APP (v764-v794)

**Fuente de verdad**: colección `pedidos` en Firestore (campo `state`). Las SQ SAP se están discontinuando.

| Vista | Rol | Archivo |
|---|---|---|
| **`v_backorder_app`** | Todas las líneas de pedidos abiertos (`closedAt IS NULL`) con `state` expuesto. Consumer filtra. | `bigquery/backorder_app.sql` |
| **`v_backorder_lineas_v2`** | Shim con **mismo shape** que la vieja `v_backorder_lineas` SAP. Filtra `state IN ('BO','ASIG')`. Mapea `state='BO'→'SIN ASIGNAR'`, `state='ASIG'→'ASIGNADO'`. **Este es el que consume Power BI** (migración zero-change). | `bigquery/backorder_app.sql` |
| `v_pedidos_lines` | Vista base sobre `pedidos_raw_raw_latest`. Expone `state`, `qty_open`, `qty_invoiced`, `qty_cancelled`. Fuente de `v_backorder_app`. | `bigquery/views.sql` |
| `v_backorder_lineas` | **Vieja** (SAP quotations). Se está discontinuando. NO usar. | `bigquery/views.sql` |
| `v_entradas_stock` | Arribos de mercadería al depósito 11 (unidades recibidas por mes). | `bigquery/backorder_app.sql` |

**Criterios de negocio** (Mariano confirmó 2026-09-01):
...
**Reposición/ETA** (`prox_embarque_date`, `qty_incoming`): siguen viviendo en SAP (`sap_purchase_orders_raw`). Se joinean por sku en `v_backorder_lineas_v2`.

**Stock disponible**: sigue en `v_inventario.stock_total_sellable` (SAP). No se toca.

### 46.2 Naming convention de columnas de costo (resuelto v794)

Cada tabla tiene sus columnas con nombres explícitos que dicen de qué **price list SAP** salen. No confundir entre tablas:

| Tabla | Columna | Fuente SAP | Cobertura |
|---|---|---|---|
| `sap_items_bike_raw` | `cost_avg_ars` | Price list **11** COSTO ARTICULO ARS (Currency=ARS) | ~5.386 items |
| `sap_items_bike_raw` | `cost_usd` | Price list **7** COSTO ARTICULO USD (Currency=USD) | ~5.381 items |
| `sap_items_bike_raw` | `price_bike_usd` | Price list **2** VENTAS PUBLICO (Currency=USD, filtro estricto) | ~6.833 items |
| `sap_items_bike_raw` | `costo_articulo_usd` | UDF `U_COS_ART_USD` | **NO usar** para valuación (data inconsistente) |
| `sap_items_raw` (Pesca) | `cost_avg_ars` | Price list **11** COSTO ARTICULO ARS (Currency=ARS) | 2 items (raro) |
| `sap_items_raw` (Pesca) | `cost_list11_usd` | Price list **11** con Currency=USD | ~771 items |
| `sap_items_raw` (Pesca) | `price_pesca_ars` | Price list **12** VENTAS PESCA | 773 items |

**Ojo**: `cost_usd` en Bike ≠ `cost_list11_usd` en Pesca. Salen de listas SAP distintas. El día que se crucen tablas, usar los nombres explícitos.

**TC en Power BI**: el pipeline devuelve valores **crudos en su moneda nativa** sin convertir. Power BI aplica FX del lado modelo con `doc_rate` (mismo patrón que `target_usd`).

### 46.3 Decisiones ancladas (COWORK)

- **Costo NO vive en campos del Item** (`StandardAveragePrice`/`LastPurchasePrice` null en este SAP). Vive como price list.
- **Bike NO tiene solapa "Ficha Técnica Pesca"**. Los UDFs `U_P_FAMILIA/U_P_SUBFAMILIA/U_P_CATEGORIA` son exclusivos de Pesca. Bike usa UDFs generales sin prefijo.
- **Stock vendible Bike = SOLO warehouse 10** (whitelist, no blacklist). Warehouse 02 (tránsito) en columna separada.
- **`clasificacion_abc`** (U_CATEG) NO es categoría de producto — es ABC de rotación. Renombrada para evitar confusión.
- **Jerarquía real de producto Bike**: `mss` (tipo) → `subcategoria` (familia) → `clase` (disciplina). `marca` corta transversalmente.
- **`costo_articulo_usd` DESCARTADO**: ratio 6-123% del `cost_usd`, no es FOB, carga manual inconsistente. Expuesto solo para drill-down.
- **Currency por fila** de `ItemPrices`, no por número de lista. `_find_price_by_list_currency()` filtra por `(lista, moneda)` estricto.
- **Encoding UTF-8**: `resp.encoding='utf-8'` en `sl_fetch_all()` para arreglar mojibake (MULTIPROPÃSITO → MULTIPROPÓSITO).
- **BO/ASIG desde APP**: la app es la fuente de verdad, las SQ SAP se discontinúan. `v_backorder_lineas_v2` es el shim para migración zero-change en Power BI.

### 46.4 Dato pendiente para Compras (NO fix del pipeline)

**TC implícito** de la lista 11 Bike (ARS) = **1.365** vs `doc_rate` de facturas SAP = **1.482** (sep 2026). Inventario Bike valuado al costo ARS está **subvaluado ~9.9%**:
- Al costo ARS nativo: **$7.697.734.042**
- Al costo USD × doc_rate: **$8.458.459.000**
- Diferencia: **$760.724.958**

COWORK maneja la corrección en el modelo si hace falta. Dato para llevar a Compras (lista 11 desactualizada vs FX real).

### 46.5 Archivos clave

| Archivo | Contenido |
|---|---|
| `scripts/sync_sap_to_bigquery.py` | Pipeline. Constantes Bike líneas 131-197. `probe_bike_udfs()` línea ~335. `resolve_bike_group_code()` línea ~380. `flatten_item_bike()` línea ~560. Pass en `main()` línea ~2270. Fix Pesca `flatten_item()` línea ~640. |
| `bigquery/bike.sql` | `v_inventario_bike` + `v_inventario_bike_por_warehouse` + 10 queries de validación comentadas. |
| `bigquery/backorder_app.sql` | `v_backorder_app` + `v_backorder_lineas_v2` (shim) + `v_entradas_stock`. |
| `bigquery/views.sql` | `v_pedidos_lines` (base para v_backorder_app) + resto de vistas Pesca. |
| `bigquery/validate_bike_q{4,5,6,7}.sql` | Archivos de validación (compatibles con workaround PS 5.1 UTF-8). |

### 46.6 Cómo re-deployar las vistas desde Windows PowerShell

PS 5.1 rompe `Get-Content | bq query` por BOM UTF-16. Workaround:

```powershell
$sql = [System.IO.File]::ReadAllText("bigquery\bike.sql")
$utf8NoBom = New-Object System.Text.UTF8Encoding $false
[System.IO.File]::WriteAllText("$env:TEMP\_deploy.sql", $sql, $utf8NoBom)
cmd.exe /c "bq query --use_legacy_sql=false --location=southamerica-east1 --project_id=app-vendedores-shimano < $env:TEMP\_deploy.sql"
Remove-Item "$env:TEMP\_deploy.sql"
```

Documentado en memoria `feedback_bq_query_ps51_encoding.md`.
