...
| **Versión actual** | SW v446 (en rama `dev`; **v445 en prod**). v446: Master Clientes — el dropdown de vendedor ahora es editable también para SAP registrados (con `cardCodeSap`), no solo para provisorios. Antes el vendedor de un cliente SAP registrado era texto plano y solo podía reasignarse desde Panel Zonas (masiva). Ahora admin/gerente puede cambiar el vendedor inline por cliente. Reutiliza `saveMcProvisorioVendor` porque escribe a `client_applications/{fsId}` sin importar si tiene cardCodeSap. |
