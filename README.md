...
| **Versión actual** | SW v439 (en rama `dev`; **v438 en prod**). v439: fix mobile — botón "Enviar formulario" / "Registrar contacto" quedaba tapado por la barra del navegador en el modal de Visitas/Contactos. Cambio `.modal-box` a `100dvh` (era `100vh`) + `env(safe-area-inset-bottom)` en el footer. Además, se conserva `id="visita-submit-btn"` al re-renderizar el footer (view→form) para que `applyVisitModeUI()` siga actualizando label/color al alternar entre visita y contacto. |
