...
| **Versión actual** | SW v426 (en rama `dev`; **v425 en prod**). v426 fix display: apellido de Federico Castelaneli (1 L) — antes se veía "Castelanelli" (2 L) en badges/dropdowns/exports por typo histórico en la vendor key + email. Nuevo helper `displayVendorName()` traduce solo al mostrar. Vendor key interna + email + datos históricos preservados (requerirían migración + IT). v425 desplegado 2026-08-07 12:5x UTC (chips header admin-only Mariano + fila propia en mobile). |
...
### Sprint actual (2026-08-06)

#### 🔴 BLOQUEANTE ACTIVO — GitHub Actions + Pages en major_outage

**Estado al cierre de sesión 2026-08-06 ~17:00 AR**:
- **GH Actions**: `major_outage` (desde ~15:30 UTC).
- **GH Pages**: `major_outage`.
- **Producción**: sigue en **v414** (última publicada antes del outage).
- **En `main` local + remoto**: **v416** (v415-v416 mergeadas). **v417-v421** siguen en PR #102 (`feat/v417-header-badges`) bloqueado por CodeQL en cola por el outage. **v422** (rama `dev`) es la nueva sección FORECAST — se abrira PR aparte cuando GH vuelva.
- Error literal en los deploys: `Failed to resolve action download info. Error: Service Unavailable`.

**Cambios pendientes de publicar** (ya mergeados a main, esperando GH):
- **v415**: ocultar botón "Masterfile-Base" del toolbar Zonas.
- **v416**: unificar Master Clientes + Zonas (botón Zonas del header removido; nuevo "Reasignar Zonas" dentro de Master Clientes).
- **v417**: 2 badges informativos siempre visibles en el header — chip de versión (para no abrir DevTools/Console) + estado conexión SAP (verde <45min / amarillo 45-90min / rojo ≥90min según edad de `app_config/stock_snapshot.updatedAt`). Con el outage actual, el badge SAP arranca 🔴 hasta que SEIDOR restablezca el Service Layer + GH Actions vuelva.
- **v418**: ocultar tab "Mapeo Clientes" del modal Carga a SAP. Redundante con el modal Master Clientes (que ya cubre la asignación de código SAP a tiendas). Elemento queda oculto con `display:none` + `data-legacy="v418"` — mismo patrón que Zonas en v416.
- **v419**: filtro "Solo sin stock" en modal Backorders (default ON al abrir). Muestra únicamente SKUs con 0 stock en whs 11 (venta) + whs 12 (tránsito) = los urgentes de importar. User destilda para ver el listado completo. Preserva la elección entre aperturas del modal.
- **v420**: nuevo botón "Asignación Backorder" (verde) al lado del actual "Backorders" (naranja). Reúsa el mismo modal con `backordersMode='asignacion'` (título + color header dinámicos) y filtro por default "Con stock" — muestra los SKUs que llegaron para que el admin sepa qué clientes lo tenían pedido y avisarles. Reemplaza el checkbox v419 por un select con 3 opciones: `Sin stock (para importar) / Con stock (para asignar) / Todos`. Contadores separados en cada botón. **Ambos botones son admin-only** (probamos con Mariano; si funciona lo abrimos a vendedores).
- **v421**: en el modal Mis Visitas, cada card mostraba "AGOSTO 2026" (mes+año). Ahora muestra la fecha exacta `dd/mm/yyyy` (ej `26/07/2026`) parseada desde `v.fecha` (ISO string). Fallback a mes+año para records viejos sin `fecha` seteada. Cambio en `src/domains/visitas.js` → rebuild del bundle requerido.
- **v422**: nueva sección **FORECAST** admin-only (whitelist: `mariano.erbino@shimano.com.ar` + `erbinomariano@gmail.com`). Botón violeta en el header abre modal fullscreen que 1) carga snapshot Firestore `sku_ventas_snapshot` (bulk get, ~755 SKUs), 2) espera Excel Sales Plan (SKU + 6 columnas de meses futuros), 3) calcula y muestra tabla con `SKU | Familia | Subfamilia | Ventas 12m | Pedido 6m | Prom/Mes | Política (×3) | Total`. Botón "Exportar Excel" genera XLSX final. Reglas Firestore: read admin-only (datos competitivamente sensibles). Snapshot alimentado por nueva función `sync_sku_ventas_snapshot_to_firestore()` agregada al cron `sync_sap_to_bigquery.py` (paso 12 al final). Bootstrap manual: `python scripts/apply_sku_ventas_snapshot.py`. Chunk lazy `chunks/forecast.js` 50.2 KB — registrado en `build.js` + `src/main.js` + `sw.js`.
- **v426**: fix display del apellido de **Federico Castelaneli** (una sola L, el correcto). Antes se veía "Castelanelli" (2 L) en dropdowns, badges, exports, dashboards, seguimiento — porque la vendor key interna del sistema es `FEDERICO CASTELANELLI` (2 L, hardcoded desde día 1 con typo, cascadeó a email `federico.castelanelli@shimano.com.ar` + SAP SlpCode 54 + todos los datos históricos en Firestore + BQ).
  - **No se cambia la vendor key** (romperlo requiere migración de miles de docs Firestore + snapshots BQ + coordinación con IT/Google Workspace para renombrar email + SEIDOR para SAP SlpCode name).
  - **Nuevo helper** `displayVendorName(vendorKey)` en `src/pure/normalize.js` con lookup table `VENDOR_DISPLAY_OVERRIDES = { 'FEDERICO CASTELANELLI': 'Federico Castelaneli' }`. Fallback a `titleCase()` para las otras 5 vendor keys que no necesitan override.
  - Registrado como `window.displayVendorName` (main.js + inline shim en index.html).
  - Reemplazos: 4 en `index.html` (exports pedidos, targets, vendorPlain del modal Zonas) + 7 en dominios (`campanias.js`, `dashboard.js`, `exports-advanced.js:3x`, `rutas.js`, `visitas.js`, `seguimiento.js:2x`) + label del dropdown `#vendor-filter` option Z2.
  - Sin cambios en lógica de negocio, ni en storage, ni en identity/auth. Cambio 100% display.
- **v425**: los 3 chips del header (`#header-badge` container = role-badge ADMIN + `#app-version-badge` + `#sap-health-badge`) pasan a ser **admin-only Mariano** (whitelist emails, mismo patrón que Análisis y Forecast). Motivación: vendedores/gerente no necesitaban esa info (versión + estado SAP no aportan al flow diario y ADMIN implícito por los botones). Además en mobile los chips rompían la visual apretando el título "MAPA DE VENTAS - ARGENTINA". Cambios:
  - **JS** (`applyRolePermissions`): toggle de clase `.hdr-badge-hidden` sobre `#header-badge` según email. Si Mariano → visible; resto → oculto (incluye ADMIN chip, que en v400 ya se había ocultado a todos).
  - **CSS**: nueva clase `.hdr-badge-hidden{display:none!important}` — el `!important` es necesario para superar el `display:flex!important` del media query mobile (línea 738) que sino re-mostraba los chips a todos.
  - **CSS mobile** (`@media max-width:900px`): `#header-badge` cambia de `order:2;flex:1 1 auto` a `order:5;flex:1 1 100%;margin-top:4px` → cuando Mariano lo ve, cae a su propia fila centrada debajo del título en vez de meterse entre hamburguesa y logo.
- **v424**: layout del modal Backorders (pedido del user en mobile — los 2 selects tenían anchos distintos y el botón Exportar quedaba en la esquina izquierda). Ajustes en `#backorders-toolbar` (index.html):
  - Search input pasa a `flex: 1 1 100%` → siempre ocupa fila propia.
  - Ambos selects (`#backorders-mes-filter` + `#backorders-stock-filter`) reciben `flex: 1 1 260px; min-width: 200px` → **exactamente el mismo ancho** en desktop (lado a lado) y mobile (cada uno 100% por wrap).
  - Botón Exportar se saca del toolbar flex y se envuelve en `<div style="text-align:center">` → centrado horizontalmente en su propia fila. Aumentado padding lateral (`14px` → `18px`) para mejor tap target.
- **v423**: rename UX de los 2 botones Backorders del header (pedido del user, más claros para el equipo comercial):
  ...
  Cambio de texto puro: mismos handlers, mismos modes, mismos IDs (`backorders-btn` / `asignacion-backorder-btn`), mismo modal — solo cambian los labels visibles + los títulos del modal cuando se abre. Sin cambios en el bundle (edición en `index.html` inline).

**Adicional deployado hoy antes del outage**:
- **`.github/workflows/deploy-pages.yml`** creado (PR #98) — GH Pages había sido cambiado a `build_type: workflow` sin el YAML correspondiente, por eso los deploys "legacy" fallaban con `errored duration=0`. El workflow nuevo sirve todo el repo tal cual (equivalente al `.nojekyll` legacy). Este pipeline es el que se destranca cuando Actions vuelva.

**Monitor auto-rerun activo al momento de cerrar sesión**: background task `bu7o2xtyy` con polling cada 2 min durante 2h. Cuando GH Actions vuelva a `operational`, dispara `gh run rerun 31117888822 --failed` automáticamente. Si al retomar la sesión ya venció los 2h, el proceso murió — hay que reintentarlo manual.

**Para retomar en nueva terminal, ejecutar en orden**:

```bash
# 1) Verificar version en produccion (ver si v416 ya llego mientras dormia el monitor):
curl -s "https://shimano-arg.github.io/app-vendedores/index.html?_t=$(date +%s%N)" -H "Cache-Control: no-cache" | grep "const APP_VERSION"
# Si dice v416 -> LISTO, todo deployado. Ignorar el resto.

# 2) Chequear estado GH Actions:
curl -s https://www.githubstatus.com/api/v2/components.json | python -c "import sys,json; d=json.load(sys.stdin); [print(f\"{c['name']}: {c['status']}\") for c in d['components'] if c['name'] in ('Actions','Pages') and not c.get('group_id')]"
# Si dice 'operational' en ambos -> podes deployar.

# 3) Si GH esta operational pero prod sigue en v414, disparar rerun del ultimo deploy:
cd "C:/Users/shimano.sandbox/Desktop/APP VENDEDORES"
gh run list --workflow=deploy-pages.yml --limit 3 --json databaseId,status,conclusion,createdAt
# Buscar el ultimo run con conclusion=failure, y hacer:
gh run rerun <run_id> --failed
# Esperar 2-3 min, re-verificar con el curl del paso 1.

# 4) Si el rerun tambien falla o el runId ya no existe, disparar workflow desde cero:
gh workflow run deploy-pages.yml
# Y monitorear con:
gh run list --workflow=deploy-pages.yml --limit 1 --json databaseId,status
```

**Contexto adicional**: los sync BQ (`sync-sap-to-bigquery.yml`, `sync-sap-catalog-stock.yml`) tambien pueden estar afectados por el outage de Actions. Si al restablecerse Actions se necesitan datos frescos de SAP, disparar `gh workflow run sync-sap-to-bigquery.yml` (siempre y cuando SAP SL de SEIDOR ya este up — ver bullet siguiente).

- [ ] **SAP Service Layer caído** (reportado por SEIDOR 2026-08-05 tarde, sin ETA definido): endpoint `https://shimano-sap.seidor.com.ar:50000/b1s/v1/Login` responde `ConnectTimeoutError` desde GitHub Actions. El cron BQ sync SAP falla en el POST /Login. Impacto: data BQ (SQs, invoices, BPs, backorder) queda con la última sync exitosa (2026-08-05 20:22 UTC). App Firestore no se afecta (independiente). Cuando SEIDOR restablezca, reintentar `gh workflow run sync-sap-to-bigquery.yml`.
