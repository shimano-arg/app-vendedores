...
| **Versión actual** | SW v451 (en rama `dev`; **v450 en prod**). v451: nueva sección **Revisión Excel** en el header. Cargar Excel del cliente (mismo template que "Cargar Pedido por Excel") y ver por SKU: Stock total (dep 11 + 12), Backorder (SQ open), Disponible (stock − backorder), Pedido cliente, Excedente. Filas rojas cuando Pedido > Disponible. Solo lectura, no persiste pedidos. Gate por whitelist de emails: Mariano, Ioannis, Santiago, Pablo (`body.can-revision` seteado en `applyRolePermissions` + runtime guard en `openRevisionModal`). Botón `📁 Descargar revisión como Excel` genera archivo con las 7 columnas para compartir con el cliente. Reutiliza el parser de header auto-detectado + case-insensitive SKU match del flujo Cargar Pedido. |
