...
| **Versión actual** | **SW v523 en prod.** Ciclo v512→v520 (2026-08-13) enfoca en 3 áreas: **(A) BQ Power BI**: mapa por departamento con `v_leads_detalle` + tabla `geo_localidad_departamento` + fixes CABA (`prov_depto` cobertura 606/698 → 688/699 = 98.4%). **(B) Stock Asig template Shimano oficial**: v511 banda BACKORDER + autofilter PEDIDO. **(C) UX Revisión / Lista de Espera abierta a todos los VDEs** (v514 rules Firestore + v517 gate de acceso por email @shimano + v518-v519 dropdown Forma de entrega con campos condicionales que se prellenan al Pasar a Pendientes + v520 botón "Dejar en Espera" en modal Revisar para el flow SKU-by-SKU manual). Frecuencia sync: stock **5 min**, BQ **15 min** (v508). |
