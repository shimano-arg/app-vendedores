...
| **Versión actual** | SW v436 (en rama `dev`; **v435 en prod**). v436: fix picker Pedido que no encontraba clientes SAP con nombre comercial largo (ej "RED OROZA MULTIMARCA S.A"): ahora también itera `approvedAltasList` (fuente 2) y matchea por multi-token contra comercio + titular + fantasía + cardCode + localidad, mismo helper `matchesAllTokens` que usa la tab CLIENTES. |
