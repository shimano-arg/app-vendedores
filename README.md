...
| **Versión actual** | SW v454 (en rama `dev`; **v453 en prod**). v454: **Pedidos en Espera** con recálculo dinámico. Nueva colección Firestore `revision_waitlist` compartida entre los 4 users del whitelist. Botón "Dejar recalculando" en modal Revisión Excel (requiere elegir cliente destino primero). Botón nuevo header "Pedidos en Espera [N]" con badge de conteo. Al abrir una card individual, recalcula en tiempo real: `disponible = stock − backorder SAP − Σ(otros pedidos en espera del SKU)` — FIFO implícito. Muestra columna "Cambio" con el delta respecto a la primer revisión. Botón "Pasar a Pendientes" ajusta cantidades a lo disponible con alerta binaria ("Ajustado X SKUs, ¿continuar?") y abre el modal "Revisa tu pedido" precargado. Botón borrar visible solo para owner + admin/gerente. Listener onSnapshot refresca en vivo cuando otros users tocan la lista. Rules: `read isReader() / create isReader() + ownerUid propio / update+delete isAdminOrGerente() || owner`. Cleanup en `detachFirebaseListeners`. |
