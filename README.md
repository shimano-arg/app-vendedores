...
| **Versión actual** | SW v449 (en rama `dev`; **v448 en prod**). v449: fix crítico SlpCode SAP — `getVendorForKey` (dashboard.js) leía una cache lazy con `p.vendor` **base del padrón**, sin aplicar `vendor_overrides`, y la cache **nunca se invalidaba**. Efecto: tiendas reasignadas desde Panel Zonas (`scope='loc'`) o Master Clientes (`scope='shop'`) quedaban devolviendo el vendor viejo. Cuando un VDI cargaba un pedido de una tienda con override a un VDE, sap-service-layer.js caía al fallback `p.ownerVendor` (el VDI) → SlpCode en SAP salía con el VDI en vez del VDE asignado. **Ejemplo real 2026-08-11**: Ioannis carga pedido de tienda reasignada a Gonzalo → SAP recibió SalesPersonCode 52 (Ioannis) en vez de 50 (Gonzalo). Fix: reemplazar cache por `getEffectiveVendorForClient(p, clientName)` sin cache — misma cascada que color del mapa (shop > loc > prov > hardcode > base). Aplica a: sap-service-layer.js buildQuotationPayload (auto-send SL), exports-sap.js (DTW/ZIP export vendedor), dashboard.js (filtro por vendedor), seguimiento.js. v445 sigue vigente para huérfanos SAP. |
