...
import { BASELINE_DATE, OUTPUT_DIR } from './config.js';
...
function loadReport(scenario, tag) {
  ...
  if (!existsSync(p)) {
    throw new Error(
      `No existe reporte ${p}. Correr lighthouse-baseline.js/trace-map.js con BASELINE_TAG=${tag} primero.`
    );
  }
...
const shellMetrics = [
  ...
  [
    'Transfer',
    shellBefore.metrics.transferSize.median,
    shellAfter.metrics.transferSize.median,
    fmtKb,
  ],
  [
    'Score',
    shellBefore.metrics.performanceScore.median,
    shellAfter.metrics.performanceScore.median,
    (v) => v.toFixed(0),
  ],
];
...
const mapMetrics = [
  ...
  [
    'LT >200ms',
    mapBefore.longTasksOver200msMedian,
    mapAfter.longTasksOver200msMedian,
    (v) => `${v}`,
  ],
  [
    'LT >500ms',
    mapBefore.longTasksOver500msMedian,
    mapAfter.longTasksOver500msMedian,
    (v) => `${v}`,
  ],
];
...
if (lcpDelta > -40) {
  console.error(
    `❌ GATE FAIL: LCP shell mejoró solo ${fmtDelta(lcpDelta)} (target ≥40% reducción).`
  );
  process.exit(1);
}
