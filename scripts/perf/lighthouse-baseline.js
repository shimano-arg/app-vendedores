...
import { findChrome, LOCAL_URL, THROTTLING, RUNS_PER_SCENARIO, OUTPUT_DIR, BASELINE_DATE } from './config.js';
...
async function runOne(url, chromePath) {
  ...
  try { await chrome.kill(); } catch (e) { /* ignore EPERM Windows race */ }
...
async function main() {
  ...
  for (let i = 0; i < RUNS_PER_SCENARIO; i++) {
    ...
    try {
      ...
      console.log(`  → LCP=${Math.round(r.lcp)}ms  FCP=${Math.round(r.fcp)}ms  TBT=${Math.round(r.tbt)}ms  score=${(r.performanceScore * 100).toFixed(0)}  (${dt}s)`);
      runs.push(r);
    } catch (err) {
  ...
  console.log(`[lighthouse] Median LCP: ${Math.round(output.metrics.lcp.median)}ms (min=${Math.round(output.metrics.lcp.min)}, max=${Math.round(output.metrics.lcp.max)})`);
  console.log(`[lighthouse] Median TBT: ${Math.round(output.metrics.tbt.median)}ms`);
  console.log(`[lighthouse] Median transfer: ${(output.metrics.transferSize.median / 1024).toFixed(0)} KB`);
  console.log(`[lighthouse] Median score: ${output.metrics.performanceScore.median.toFixed(0)}/100`);
  console.log(`[lighthouse] Long tasks >500ms (primer run): ${runs[0].longTasks.filter(t => t.duration > 500).length}`);
}
