...
function buildAltaCliShareUrl() {
  ...
  const base =
    window.location.origin + window.location.pathname.replace(/[^/]*$/, '') + 'alta-cliente.html';
...
window.shareAltaCliViaWhatsapp = function () {
  const url = buildAltaCliShareUrl();
  const vname =
    (currentUser && currentUser.displayName) || (currentUser && currentUser.email) || 'Shimano';
  const msg =
    'Hola! Soy ' +
    vname +
    ' de Shimano Argentina. Para darte de alta como cliente, ' +
...
window.setAltaCliSubtab = function (sub) {
  document
    .querySelectorAll('.ac-subtab-btn')
    .forEach((b) => b.classList.toggle('active', b.dataset.acsub === sub));
...
window.submitAltaRapida = async function () {
  ...
  const provincia = ((document.getElementById('ar-provincia') || {}).value || '')
    .trim()
    .toUpperCase();
  ...
  if (comercio.length < 2) {
    alert('Completá el nombre del local.');
    return;
  }
  if (!provincia) {
    alert('Elegí la provincia.');
    return;
  }
  if (localidad.length < 2) {
    alert('Completá la localidad.');
    return;
  }
  ...
  if (dueno.length < 2) {
    alert('Completá el nombre del dueño / contacto.');
    return;
  }
  if (cuit && cuit.length !== 11) {
    if (
      !confirm('El CUIT ingresado tiene ' + cuit.length + ' digitos (esperaba 11). Guardar igual?')
    )
      return;
  }
  if (
    !confirm(
      'Confirmar alta rápida de "' +
        comercio +
        '"?\n\n' +
        'Provincia: ' +
        titleCase(provincia) +
    ...
    geoPromise.then((geo) => {
      if (geo && geo.lat != null && geo.lng != null) {
        ...
        docRef
          ...
          .catch((e) => console.warn('geocode update alta rapida', e));
      }
    });
      ...
      adminsSnap.forEach((d) => {
        fbDb
          ...
          .add({
            ...
            body:
              me +
              ' dio de alta rapida a "' +
              comercio +
              '" (' +
              direccion +
              ', dueño ' +
              dueno +
              '). Hay que cargarlo manualmente en SAP.',
            comercio: comercio,
            status: 'unread',
            createdAt: firebase.firestore.FieldValue.serverTimestamp(),
          })
...
function refreshAltaCliGrid(kind) {
  const gridId =
    kind === 'arca' ? 'ac-arca-grid' : kind === 'iibb' ? 'ac-iibb-grid' : 'ac-fotos-grid';
  const grid = document.getElementById(gridId);
  if (!grid) return;
  let cells = '';
  if (kind === 'fotos') {
    cells = altaCliFiles.fotos
      .map(
        (b64, i) =>
          '<div class="photo-cell"><img src="' +
          b64 +
          '"/><button type="button" class="rm" onclick="removeAltaCliFile(\'fotos\',' +
          i +
          ')">&times;</button></div>'
      )
      .join('');
    if (altaCliFiles.fotos.length < ALTA_CLI_MAX_FOTOS) {
      cells +=
        '<label class="photo-cell add"><input type="file" accept="image/*" multiple onchange="onAltaCliFile(this,\'fotos\')"/>+</label>';
    }
  } else {
    ...
    if (v) {
      cells =
        '<div class="photo-cell"><img src="' +
        v +
        '"/><button type="button" class="rm" onclick="removeAltaCliFile(\'' +
        kind +
        '\')">&times;</button></div>';
    } else {
      cells =
        '<label class="photo-cell add"><input type="file" accept="image/*,.pdf" onchange="onAltaCliFile(this,\'' +
        kind +
        '\')"/>+</label>';
    }
  }
...
window.submitClientApplication = async function () {
  ...
  if (
    !confirm(
      'Enviar la solicitud de alta de "' +
        read('ac-comercio') +
...
function renderAltaCliMisSolicitudes() {
  ...
  if (!altaCliMine.length) {
    cont.innerHTML =
      '<div class="notif-empty">No enviaste solicitudes todavia. Usa la pestaña <b>Nueva solicitud</b>.</div>';
    return;
  }
  let html = '';
  altaCliMine.forEach((a) => {
    ...
    html +=
      '<div class="ac-app-meta">' +
      escapeHtml(a.fantasia || '') +
      ' &middot; CUIT ' +
      escapeHtml(a.cuit || '') +
      '</div>';
    html +=
      ...
      ' &middot; ' +
      escapeHtml(titleCase(a.provincia || '')) +
      ' &middot; Enviada ' +
      escapeHtml(dtStr) +
      '</div>';
    ...
    if (canDelete) {
      ...
      html +=
        '<button onclick="deleteMyAltaCli(\'' +
        safeId +
        "', " +
        safeName +
...
window.openClientApplicationDetail = async function (appId, notifId) {
  ...
  try {
    ...
    h += '</div>';
    h +=
      '<h4 style="font-size:12px;color:#0891b2;text-transform:uppercase;letter-spacing:.5px;margin:14px 0 8px;padding-bottom:5px;border-bottom:1.5px solid #67e8f9">Contacto</h4>';
    h += '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px 14px">';
    ...
    h += '</div>';
    h +=
      '<h4 style="font-size:12px;color:#0891b2;text-transform:uppercase;letter-spacing:.5px;margin:14px 0 8px;padding-bottom:5px;border-bottom:1.5px solid #67e8f9">Documentaci&oacute;n</h4>';
    ...
    (a.fotosLocal || []).forEach((f, i) => {
      h +=
        ...
        '" id="ca-foto-' +
        i +
        '" class="task-img-thumb" style="width:90px;height:90px" onclick="openImgViewer(\'ca-foto-' +
        i +
        '\')"/></div>';
    });
    h += '</div>';
    h +=
      '<h4 style="font-size:12px;color:#0891b2;text-transform:uppercase;letter-spacing:.5px;margin:14px 0 8px;padding-bottom:5px;border-bottom:1.5px solid #67e8f9">Origen</h4>';
    ...
    if (a.status === 'pending_approval' && iAmApprover && !iAlreadyApproved) {
      ...
      h +=
        ...
        '\')">Aprobar</button>';
      h +=
        '<button class="qmodal-btn danger" style="flex:1" onclick="rejectClientApplication(\'' +
        escapeAttr(appId) +
        "','" +
        escapeAttr(notifId || '') +
        '\')">Rechazar</button>';
...
window.approveClientApplication = async function (appId, notifId) {
  ...
  if (!cardCode) {
    if (
      !confirm(
        'No cargaste CardCode SAP. La tienda va a aparecer en el mapa pero NO se va a poder crear pedido hasta que se cargue. ¿Aprobar igual?'
      )
    )
      return;
  }
  if (!assignedVendor) {
    alert(
      'Tenes que elegir un vendedor para asignar la tienda. Sin vendedor no se sabe en qué zona del mapa aparece.'
    );
    return;
  }
    ...
    if (apN >= 2 && a.status === 'pending_approval') {
      ...
      if (a.ownerUid) {
        await fbDb.collection('notifications').add({
          ...
          message:
            'Tu solicitud para dar de alta a "' +
            (a.comercio || '') +
            '" fue aprobada por los 2 revisores. Ya podes cargarle pedidos.',
          applicationId: appId,
          status: 'unread',
          createdAt: firebase.firestore.FieldValue.serverTimestamp(),
        });
  ...
  try {
    ...
    if (a.ownerUid) {
      await fbDb.collection('notifications').add({
        ...
        message:
          'Tu solicitud para "' + (a.comercio || '') + '" fue rechazada.\nMotivo: ' + reason.trim(),
        applicationId: appId,
        status: 'unread',
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
...
window.setNotifsTab = function (tab) {
  ...
  document
    .querySelectorAll('.ntab-btn')
    .forEach((b) => b.classList.toggle('active', b.dataset.ntab === tab));
  document.getElementById('notifs-pane-recibidas').style.display =
    tab === 'recibidas' ? '' : 'none';
  document.getElementById('notifs-pane-realizadas').style.display =
    tab === 'realizadas' ? '' : 'none';
...
function notifItemHtml(n, opts) {
  ...
  if (type === 'task') {
    ...
    if ((n.images || []).length) {
      ...
      n.images.forEach((img, i) => {
        ...
        h +=
          '<img src="' +
          safe +
          '" class="task-img-thumb" onclick="openImgViewer(\'' +
          escapeAttr('task-' + n._fsId + '-' + i) +
          '\')" id="task-' +
          n._fsId +
          '-' +
          i +
    ...
    if (n.status !== 'done' && !opts.readonly) {
      ...
      h +=
        '<button class="btn-read" onclick="completarTask(\'' +
        escapeAttr(n._fsId) +
    ...
    if (n.status !== 'read' && n.status !== 'done' && !opts.readonly) {
      h += '<div class="notif-item-actions">';
      h +=
        '<button class="btn-read" onclick="openClientApplicationDetail(\'' +
        escapeAttr(n.applicationId || '') +
        "','" +
        escapeAttr(n._fsId) +
        '\')" style="background:#0891b2">Ver detalle y decidir</button>';
      h += '</div>';
    }
    ...
    if (n.status !== 'read' && n.status !== 'done' && !opts.readonly) {
      h += '<div class="notif-item-actions">';
      h +=
        '<button class="btn-read" onclick="openRendicionDetail(\'' +
        escapeAttr(n.rendicionId || '') +
        "','" +
        escapeAttr(n._fsId) +
        '\')" style="background:#be185d">Ver detalle y decidir</button>';
      h += '</div>';
    }
    ...
    h +=
      '<div class="nf"><span>' +
      escapeHtml((n.localidad || '') + ' &middot; ' + (n.provincia || '')) +
      '</span><span>' +
      escapeHtml(dtStr) +
      '</span></div>';
    if (n.status !== 'read' && !opts.readonly) {
      h += '<div class="notif-item-actions">';
      h +=
        '<button class="btn-read" onclick="contactarDesdeNotif(\'' +
        escapeAttr(n._fsId) +
        '\')">Contactar</button>';
      h +=
...
function renderNotifsList() {
  ...
  html +=
    ...
    '</div>';
  html +=
    '<button onclick="markAllNotifsRead()" style="background:#0d9488;color:#fff;border:none;padding:7px 12px;font-size:11px;font-weight:800;border-radius:5px;cursor:pointer;text-transform:uppercase;letter-spacing:.3px;white-space:nowrap">&#10003; Marcar todas como leidas</button>';
  html += '</div>';
...
function renderNotifsRealizadas() {
  ...
  if (!cerradas.length) {
    cont.innerHTML =
      '<div class="notif-empty">Aca van a aparecer las alertas y tareas que ya cerraste.</div>';
    return;
  }
...
function renderMySentTasks() {
  ...
  if (!mySentTasks.length) {
    cont.innerHTML =
      '<div class="notif-empty">No enviaste tareas todavia. Usá la pestaña <b>Crear tarea</b>.</div>';
    return;
  }
  let html = '';
    ...
    Object.entries(usersMap || {}).forEach(([uid, u]) => {
      ...
      opts.push(
        '<option value="' +
          ...
          escapeAttr(u.email || '') +
          '" data-name="' +
          escapeAttr(u.displayName || u.email || '') +
          '">' +
          escapeHtml(label) +
          '</option>'
      );
    ...
    .then((snap) => {
      ...
      fbDb
        ...
        .catch((e) => {
          ...
          sel.innerHTML =
            '<option value="">Seleccionar... (sin usuarios disponibles, pedile al admin que entre 1 vez al Panel Usuarios)</option>';
        });
    })
    .catch((e) => {
...
function refreshTaskImageGrid() {
  ...
  if (!grid) return;
  const cells = taskFormImages
    .map(
      (b64, i) =>
        '<div class="photo-cell"><img src="' +
        b64 +
        '"/><button type="button" class="rm" onclick="removeTaskFormImage(' +
        i +
        ')">&times;</button></div>'
    )
    .join('');
  const addCell =
    taskFormImages.length < 5
...
window.markNotifRead = async function (fsId) {
  try {
    await fbDb
      .collection('notifications')
      .doc(fsId)
      .update({ status: 'read', readAt: firebase.firestore.FieldValue.serverTimestamp() });
  } catch (e) {
