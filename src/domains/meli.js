...
function _paintHeader(el, state) {
  ...
  if (!state) {
    el['innerHTML'] = '<div style="padding:12px 16px;background:#0f172a;color:#fff">🛒 MERCADOLIBRE · sin data</div>';
    return;
  }
  const syncAt = state.last_sync && state.last_sync.toDate ? state.last_sync.toDate() : null;
  const snapshotDate = escape(state.snapshot_date || '—');
  ...
  el['innerHTML'] =
    '<div style="padding:12px 16px;background:linear-gradient(90deg,#0f172a,#1e293b);color:#fff;display:flex;justify-content:space-between;align-items:center">' +
      '<div>' +
        ...
        '<div style="font-size:10px;opacity:0.8;margin-top:2px">Snapshot ' + snapshotDate + ' · sincronizado ' + ageStr + '</div>' +
      '</div>' +
...
function _paintSubtabs(el, cache, active) {
  const pill = (name, label, count) => {
    ...
    return '<span onclick="setMeliSubtab(\'' + name + '\')" style="cursor:pointer;padding:6px 12px;background:' + bg + ';color:' + color + ';border-radius:6px;font-weight:' + weight + ';font-size:11px">' + label + ' <b>' + count + '</b></span>';
  };
  el['innerHTML'] =
...
function _paintMapSection(el, alerts) {
  ...
  html +=
    '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:14px">' +
      '<div style="background:var(--bg-elevated);border:1.5px solid ' + (sorted.length ? '#fecaca' : '#e5e7eb') + ';border-radius:8px;padding:12px">' +
        '<div style="font-size:10px;color:var(--text-secondary);text-transform:uppercase;font-weight:700">Violaciones</div>' +
        '<div style="font-size:26px;font-weight:800;color:' + (sorted.length ? '#dc2626' : '#16a34a') + '">' + sorted.length + '</div>' +
      ...
      '<div style="background:var(--bg-elevated);border:1.5px solid var(--border-subtle);border-radius:8px;padding:12px">' +
        ...
        '<div style="font-size:26px;font-weight:800">' + avgPct + '%</div>' +
      '</div>' +
      '<div style="background:var(--bg-elevated);border:1.5px solid var(--border-subtle);border-radius:8px;padding:12px">' +
        '<div style="font-size:10px;color:var(--text-secondary);text-transform:uppercase;font-weight:700">Sellers involucrados</div>' +
        '<div style="font-size:26px;font-weight:800">' + nSellers + '</div>' +
      '</div>' +
    '</div>';

  html += '<input id="meli-map-filter" placeholder="Filtrar por seller..." oninput="window.__meliFilterMapAlerts(this.value)" style="width:100%;padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:11px;margin-bottom:8px">';
  ...
  html += sorted.map(a => {
    ...
    return '<div class="meli-alert-row" data-seller="' + escape((a.seller_nickname || '').toLowerCase()) + '" style="display:grid;grid-template-columns:120px 100px 1.5fr 80px 80px 60px 40px;padding:8px 10px;border-top:1px solid #f3f4f6;background:' + bg + ';font-size:11px">' +
      '<div>' + escape(a.seller_nickname || '—') + '</div>' +
      '<div style="font-family:monospace;font-size:10px">' + escape(a.sku || '—') + '</div>' +
      '<div style="font-size:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + escape(a.product_name || '') + '</div>' +
      '<div>$' + Math.round(a.precio_publicado).toLocaleString('es-AR') + '</div>' +
      '<div>$' + Math.round(a.precio_sugerido).toLocaleString('es-AR') + '</div>' +
      '<div style="text-align:right;color:' + color + ';font-weight:700">' + a.diferencia_pct.toFixed(1) + '%</div>' +
      '<div style="text-align:center"><a href="' + escape(a.ml_url) + '" target="_blank" rel="noopener" title="Ver publicación">🔗</a></div>' +
    '</div>';
  }).join('');
...
function _paintProductsTable() {
  ...
  let html = '<div style="background:var(--bg-elevated);border:1px solid var(--border-subtle);border-radius:8px;overflow:hidden">';
  html +=
  ...
  if (filtered.length === 0) {
    ...
    html += filtered.map(p => {
      ...
      return '<div style="display:grid;grid-template-columns:2fr 1fr 100px 100px 60px 60px 100px;padding:8px 10px;border-top:1px solid #f3f4f6;font-size:11px">' +
        '<div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + escape(p.name || '') + '</div>' +
        '<div>' + escape(p.category_name || '—') + '</div>' +
        '<div>$' + Math.round(p.price_min || 0).toLocaleString('es-AR') + '</div>' +
        '<div>$' + Math.round(p.price_avg || 0).toLocaleString('es-AR') + '</div>' +
        '<div>' + (p.n_sellers_competing || 0) + '</div>' +
        '<div>' + (p.visits_7d_sum || 0) + '</div>' +
        '<div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + escape(topSeller.nickname || '—') + '</div>' +
      '</div>';
    }).join('');
  }
  html += '</div>';
...
function _paintCategoriesSection(el, categories) {
  ...
  let html = '<div style="padding:14px 16px"><div style="background:var(--bg-elevated);border:1px solid var(--border-subtle);border-radius:8px;overflow:hidden">';
  html +=
  ...
  html += sorted.map(c => {
    ...
    return '<div style="display:grid;grid-template-columns:2fr 60px 60px 1.5fr 100px 100px 60px;padding:8px 10px;border-top:1px solid #f3f4f6;font-size:11px;align-items:center">' +
      '<div><b>' + escape(c.category_name || '—') + '</b></div>' +
      '<div>' + (c.n_listings || 0) + '</div>' +
      '<div>' + (c.n_products || 0) + '</div>' +
      '<div style="display:flex;align-items:center;gap:6px">' +
        ...
        '</div>' +
        '<span style="font-weight:700;color:' + barColor + ';min-width:40px;text-align:right">' + pct.toFixed(1) + '%</span>' +
      '</div>' +
      '<div>$' + Math.round(c.price_min || 0).toLocaleString('es-AR') + '</div>' +
      '<div>$' + Math.round(c.price_avg || 0).toLocaleString('es-AR') + '</div>' +
      '<div>' + (c.n_sellers || 0) + '</div>' +
    '</div>';
  }).join('');
