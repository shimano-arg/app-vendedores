...
function openPedidoModal(clientName, tipo, province, locName){
  ...
  window.currentOrderClient = {name: clientName, tipo: tipo, province: province, locName: locName};
...
window.cancelPedido = function(){
  console.log('[cancelPedido] currentOrderKey=', currentOrderKey, 'client=', currentOrderClient && currentOrderClient.name);
  ...
  const msg = nItems > 0
    ? ('Eliminar los ' + nItems + ' producto(s) del pedido de "' + clientName + '"?\n\nEsta accion no se puede deshacer.')
    : ('Cerrar el pedido de "' + clientName + '" sin cargar nada?');
  if (!confirm(msg)) return;
  try { logOp('cancelar_borrador_pedido', 'pedido', clientName, {key: currentOrderKey, items: nItems, province: currentOrderClient && currentOrderClient.province, locName: currentOrderClient && currentOrderClient.locName}); } catch(e){}
  ...
  console.log('[cancelPedido] orders[key] borrado localmente. orders restantes:', Object.keys(orders).length);
  ...
  if (currentUser && fbDb) {
    try {
      ...
      fbDb.collection('userData').doc(currentUser.uid).update(
        ...
        console.warn('[cancelPedido] no pude borrar la key del cloud (puede ser normal):', e && e.message);
      }).finally(function() {
  ...
  try { if (typeof renderProductPicker === 'function') renderProductPicker(); } catch(e){}
  try { if (typeof renderOrderLines === 'function') renderOrderLines(); } catch(e){}
  closePedidoModal();
  if (typeof showSyncTag === 'function') showSyncTag('Pedido cancelado (' + nItems + ' items eliminados)');
};
...
window.openReviewDialog = function(mode){
  ...
  if (!lines.length) { alert('El pedido esta vacio. Agregue productos antes de confirmar.'); return; }
  // Limpiar banner de error de sesion anterior.
  try { if (typeof showReviewError === 'function') showReviewError(''); } catch(e){}
  ...
  ['rv-transp-nombre','rv-transp-direccion','rv-cliente-direccion','rv-sucursal-direccion'].forEach(id => {
...
window.validateReviewAndPasarAPendientes = function(){
  try {
    ...
    if (!fpVal) {
      ...
      if (fpSel) { try { fpSel.focus(); fpSel.scrollIntoView({behavior:'smooth', block:'center'}); } catch(e){} }
      return;
    }
    ...
    if (!feVal) {
      ...
      if (feSel) { try { feSel.focus(); feSel.scrollIntoView({behavior:'smooth', block:'center'}); } catch(e){} }
      return;
    }
    const transpNombre = ((document.getElementById('rv-transp-nombre') || {}).value || '').trim();
    const transpDireccion = ((document.getElementById('rv-transp-direccion') || {}).value || '').trim();
    const clienteDireccion = ((document.getElementById('rv-cliente-direccion') || {}).value || '').trim();
    const sucursalDireccion = ((document.getElementById('rv-sucursal-direccion') || {}).value || '').trim();
    if (feVal === 'TRANSPORTISTA') {
      ...
      if (!transpNombre || !transpDireccion || !clienteDireccion) {
        ...
        showReviewError('Elegiste "Entregar a transportista": falta completar ' + faltantes.join(' + ') + '.');
        const targetId = !transpNombre ? 'rv-transp-nombre'
                      : !transpDireccion ? 'rv-transp-direccion'
    ...
    try { doConfirmPedido(); }
      ...
      alert('No pudimos pasar el pedido a Pendientes:\n\n' + (err && err.message ? err.message : String(err)));
      try { openReviewDialog('crear'); } catch(e){}
    }
  } catch(err) {
    console.error('[pedido] validateReviewAndPasarAPendientes FATAL', err);
    alert('Error inesperado al pasar el pedido a Pendientes:\n\n' + (err && err.message ? err.message : String(err)) + '\n\nAvisa al admin y adjunta el mensaje.');
  }
};
...
function showReviewError(msg){
  const modalBody = document.querySelector('#review-modal .modal-body') || document.querySelector('#review-modal .review-content') || document.getElementById('review-modal');
  if (!modalBody) return;
  ...
  if (!el) {
    ...
    el.style.cssText = 'background:#fee2e2;border:1.5px solid #fca5a5;color:#991b1b;border-radius:6px;padding:10px 12px;margin:10px 0;font-size:12px;font-weight:600;line-height:1.5';
...
function updateReviewFooter(){
  const footer = document.querySelector('#review-modal .modal-footer');
  if (reviewMode === 'pending-confirm') {
    footer.innerHTML = '<button class="btn-cancel" style="background:#fff;color:#475569;border:1.5px solid #cbd5e1" onclick="closeReviewDialog()">Volver a editar</button>'
...
function renderReviewSuggestions(){
  ...
  if (reviewMode !== 'pending-confirm' || !currentOrderClient) { box.style.display = 'none'; return; }
  ...
  suggestions.forEach(s => {
    ...
    let shopsText = s.shopList.length <= 3 ? s.shopList.join(', ') : s.shopList.slice(0, 2).join(', ') + ' y ' + (s.shopList.length - 2) + ' mas';
    html += '<div class="suggest-row" onclick="addToOrderAndRefreshReview(\'' + escapeAttr(s.code) + '\')">';
    html += '<div class="code">' + escapeHtml(s.code) + '</div>';
    html += '<div><div class="sname">' + escapeHtml(desc) + '</div>';
    html += '<div class="sinfo">Pidieron: ' + escapeHtml(shopsText) + ' &middot; ' + s.qty + ' unid.</div></div>';
    html += '<button class="add-sg" onclick="event.stopPropagation();addToOrderAndRefreshReview(\'' + escapeAttr(s.code) + '\')" title="Agregar">+</button>';
    html += '</div>';
  });
...
window.addSuggestionPrompt = function(code){
  ...
  if (isNaN(qty) || qty < 1) { alert('Cantidad invalida. Usa un numero entero >= 1.'); return; }
  ...
  if (currentOrderKey === '__pending__') {
    ...
    if (existing) {
      ...
      entry.lines.push({code: prod.code, desc: prod.desc, cat: prod.cat || '', fam: prod.fam || '', sub: prod.sub || '', qty: qty, precio: getDefaultPrice(prod.code)});
    }
...
window.doConfirmDefinitivoFromReview = function(){
  closeReviewDialog();
  if (!currentOrderClient || currentOrderClient.stage !== 'pending') return;
  const key = currentOrderClient.viewKey, idx = currentOrderClient.viewIdx;
  const list = pending[key] || [];
  const entry = list[idx];
  if (!entry) return;
  if (entry._fsId && currentUser) {
    fbDb.collection('pedidos').doc(entry._fsId).update({stage: 'confirmed', finalizedAt: new Date().toISOString()})
      .then(() => showSyncTag('Pedido confirmado definitivamente'))
      .catch(e => { console.error('confirm def', e); alert('Error: ' + (e.message || e)); });
  }
...
function renderReviewLines(){
  ...
  if (currentOrderClient) {
    sumEl.innerHTML = '<div class="rs-name">' + escapeHtml(currentOrderClient.name) + '</div>'
      + '<div class="rs-meta">' + escapeHtml(titleCase(currentOrderClient.province)) + ' &middot; ' + escapeHtml(currentOrderClient.locName) + ' &middot; ' + (currentOrderClient.tipo === 'C' ? 'Cliente actual' : 'Prospecto') + '</div>';
  }
  ...
  let okU = 0, okM = 0, okN = 0;
  let noU = 0, noM = 0, noN = 0;
  ord.forEach(l => {
    ...
    if (noDisp) { noU += q; noM += subtotal; noN++; }
    else        { okU += q; okM += subtotal; okN++; }
    ...
    const dot = needsReview
      ...
      : (noDisp
          ...
          : (stockAvail === true ? '<span title="Disponible" style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#16a34a;margin-right:6px;vertical-align:middle"></span>' : ''));
    ...
    const lineStyle = needsReview
      ...
      : (noDisp ? ' style="background:#fef2f2;border-color:#fecaca"' : '');
    ...
    const badgeReview = needsReview
      ...
      : '';
    const badgeNoStock = (!needsReview && noDisp)
    ...
    html += '<input type="number" min="0" step="any" value="' + p + '" placeholder="$" onchange="setOrderPrice(\'' + escapeAttr(l.code) + '\', this.value);renderReviewLines()" title="Precio unitario"/>';
    html += '<input type="number" min="0" step="1" value="' + q + '" title="Cantidad" onchange="setOrderQty(\'' + escapeAttr(l.code) + '\', this.value);renderReviewLines()"/>';
    html += '<button class="rl-rm" onclick="removeFromOrder(\'' + escapeAttr(l.code) + '\');renderReviewLines();renderReviewSuggestions()" title="Quitar">&times;</button>';
    html += '</div>';
  });
  // v281+: si el filtro esta activo y no matchea nada, mostrar mensaje claro.
  if (!ord.length) html = '<div class="no-data">Sin productos. Volve a editar y agrega items.</div>';
  else if (searchActive && visibleN === 0) html = '<div class="no-data">Ningun producto matchea &laquo;' + escapeHtml(searchQuery) + '&raquo;.</div>';
  ...
  let totalsHtml = '';
  if (noN > 0 && okN > 0) {
    totalsHtml = '<div style="display:flex;flex-direction:column;gap:8px;width:100%">'
      ...
      +   '<span style="display:flex;align-items:center;gap:6px;font-size:11px;font-weight:700;color:#86efac"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#16a34a"></span>Disponibles &middot; ' + okN + ' prod &middot; ' + okU + ' u</span>'
      +   '<span style="color:#86efac;font-size:14px;font-weight:800">' + fmt(okM) + '</span>'
      + '</div>'
      + '<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;padding-bottom:8px;border-bottom:1px solid #334155">'
      +   '<span style="display:flex;align-items:center;gap:6px;font-size:11px;font-weight:700;color:#fca5a5"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#dc2626"></span>Sin stock &middot; ' + noN + ' prod &middot; ' + noU + ' u</span>'
      +   '<span style="color:#fca5a5;font-size:14px;font-weight:800">' + fmt(noM) + '</span>'
      + '</div>'
      + '<div style="display:flex;justify-content:space-between;align-items:center;gap:8px">'
      +   '<span style="font-size:12px;font-weight:800">Total pedido &middot; ' + ord.length + ' prod &middot; ' + totalU + ' u</span>'
      +   '<span class="rt-money">' + fmt(totalM) + '</span>'
      + '</div>'
      + '</div>';
  } else {
    totalsHtml = '<span>' + ord.length + ' productos &middot; ' + totalU + ' unidades</span><span class="rt-money">' + fmt(totalM) + '</span>';
  }
    ...
    if (noN > 0) {
      ...
      splitEl.innerHTML =
        '<div style="background:#ecfdf5;border:1.5px solid #6ee7b7;border-radius:8px;padding:10px 12px">' +
          ...
          '</div>' +
          '<div style="font-size:16px;font-weight:800;color:#047857">' + fmt(okM) + '</div>' +
          '<div style="font-size:10px;color:#065f46;margin-top:2px">' + okN + ' producto' + (okN === 1 ? '' : 's') + ' &middot; ' + okU + ' u</div>' +
        '</div>' +
        '<div style="background:#fef2f2;border:1.5px solid #fecaca;border-radius:8px;padding:10px 12px">' +
          ...
          '</div>' +
          '<div style="font-size:16px;font-weight:800;color:#dc2626">' + fmt(noM) + '</div>' +
          '<div style="font-size:10px;color:#991b1b;margin-top:2px">' + noN + ' producto' + (noN === 1 ? '' : 's') + ' &middot; ' + noU + ' u</div>' +
        '</div>';
    } else {
...
window.onFormaPagoChange = function(){
  renderReviewLines();
  try { revalidateReviewSilently(); } catch(e){}
};
...
window.onFormaEntregaChange = function(){
  ...
  if (val !== 'TRANSPORTISTA') {
    const n = document.getElementById('rv-transp-nombre'); if (n) n.value = '';
    const d = document.getElementById('rv-transp-direccion'); if (d) d.value = '';
    const cd = document.getElementById('rv-cliente-direccion'); if (cd) cd.value = '';
  }
  if (val !== 'SUCURSAL') {
    const s = document.getElementById('rv-sucursal-direccion'); if (s) s.value = '';
  }
  try { revalidateReviewSilently(); } catch(e){}
};
...
window.openConfirmDialog = function(){
  ...
  const formaPagoYaElegida = (document.getElementById('rv-forma-pago') && document.getElementById('rv-forma-pago').value) || '';
  const fpLine = formaPagoYaElegida ? ('<br><span style="font-size:11px;color:#475569">Forma de pago: <b>' + escapeHtml(formaPagoYaElegida) + '</b></span>') : '';
  ...
  document.getElementById('cd-text').innerHTML = 'Desea confirmar el pedido de <b>' + escapeHtml(clientName || '(sin nombre)') + '</b>?<br><span style="font-size:11px;color:#64748b">Seleccione el mes/a&ntilde;o:</span>' + fpLine;
  // Selector "Crear en nombre de" para VDI con parejas VDE.
  try { renderPedidoActingAsSelect(); } catch(e) { console.warn('renderPedidoActingAsSelect fallo', e); }
...
function renderPedidoActingAsSelect(){
  ...
  wrap.style.display = '';
  const opts = ['<option value="">Mí mismo</option>']
...
function refreshPedidoActingAsInfo(){
  ...
  if (p) {
    info.style.display = '';
    info.innerHTML = '&#9888;&#65039; El pedido queda registrado como del VDE <b>' + escapeHtml(p.displayName) + '</b>. Le va a llegar una notificacion automatica.';
  } else {
