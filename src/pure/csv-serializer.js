...
export function firestoreValueToCsv(v) {
  ...
  if (typeof v === 'number') {
    if (!isFinite(v)) return ''; // NaN, Infinity -> vacio (no confundir pipelines)
    return String(v);
  }
  if (typeof v === 'boolean') return v ? 'true' : 'false';
  // Firestore Timestamp
  if (typeof v === 'object' && v !== null && typeof (/** @type {any} */(v)).toDate === 'function') {
    try {
...
export function computeNullRates(schema, rows, requiredCols) {
  ...
  schema.columns.forEach((c, i) => { colIndex[c.col] = i; });
...
export function buildPedidoRows(doc) {
  ...
  return lines.map((/** @type {any} */l, /** @type {number} */idx) => header.concat([
    idx,
    l ? l.code : null,
    l ? l.desc : null,
    l ? l.qty : null,
...
export function buildCustomRouteRows(doc) {
  ...
  return stops.map((/** @type {any} */s) => header.concat([
    s ? s.order : null,
    s ? s.key : null,
    s ? s.tipo : null,
    s ? s.provincia : null,
...
export function buildProductoRowsFromStockJson(stockJson) {
  ...
  let breakdown = {};
  try { quantities = sj.quantities ? JSON.parse(sj.quantities) : (sj.quantities_map || {}); } catch (_) {}
  try { breakdown = sj.warehouseBreakdown ? JSON.parse(sj.warehouseBreakdown) : (sj.warehouseBreakdown_map || {}); } catch (_) {}
